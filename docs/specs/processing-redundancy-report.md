...
- aggregate_claims_across_sources - deterministic regex claim reconciliation + source-agreement scoring over fetched news text -> tabular dict. KIND: tabular. V: KEEP. Not a spatial op (none of b/c/d fit); SHARED PRIMITIVE `_extract_contaminants` (modflow contamination); the LLM could reason over the text itself, but that is a different axis than the spatial replacement surfaces.

...
## Plugin dependencies: matplotlib, per OS
...
`install_dependencies.py` (pure stdlib, ships inside the plugin) is
...
("No module named pip") -- not a PATH problem, not a
missing package, pip is simply absent from that interpreter, so nothing can
be installed into it, by any method. The fix: use your

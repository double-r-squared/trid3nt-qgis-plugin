...
> STATUS 2026-08-09: EXECUTED. The 2026-08-08 data block was cleared by
> the AORC + LTER fetchers, and the replication was run computed-vs-observed
...
> paper comparison were recorded with the run. Historical context (the STOP +
> unblock) below.
>
> PRIOR STATUS 2026-08-08: NATE signed off the sourcing (Coweeta site + USFS
...
> gauge. STOPPED per protocol. Cheapest unblock (TAKEN): build an AORC
...
> weir->sub-catchment mapping, and the pour-point/basin spatial caveat: section 1.
...
Primary site: the **Coweeta Creek watershed**, Nantahala Mtns, NC.
- [KNOWN] Delineated catchment 30.03 km2 (pysheds `delineate_watershed` on 3DEP
  ...
  paper's small steep catchment. Mesh already built (4956 nodes / 9727
...
- Observed-discharge gauge options:
  ...
  - [RESOLVED 2026-08-08] Coweeta Creek proper via EDI: the
    ...
    catchment exceeds the 21.85 km2 gauged basin.
...
- [NOT GRADEABLE 2026-08-08: post-dates the gauge (ends 2019-01-14) by
  ...
  hydrograph.
- [NOT GRADEABLE 2026-08-08: post-dates the gauge by 5.7 yr]
...
- [NOT GRADEABLE 2026-08-08: any 2021-2024 window post-dates the gauge.
...
- The template consumes the hourly MRMS hyetograph via the rainfall-excess
  preprocessing path, since the native runoff model is
...
4. Sign off the CN/Manning per-class bands (paper Table 1 analog) as calibration
   inputs.
5. THEN build the registered template (the deferred build wave) and run.

## Status update -- 2026-08-10 (fidelity-ladder-2)

The two residuals above were attacked with two levers on the same Ball Creek
...
throughout. The full account, ladder table, store formulation and mass audit

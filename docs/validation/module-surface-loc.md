...
| --- | ---: | ---: | ---: | --- |
| `telemac/authoring/` | 4,597 | 1,545 | **-3,052** | `author.py` (1,859) and the three JSON-config hops `{agitation,open_water,stratified}.py` (1,363) die; `assembler.py` keeps settling and staging, and the harbour settle grew the walk that turns two node sets into runs; `serializer.py` (80) is the whole writer. |
...
(source.yaml 135 + corpus.yaml 9) is declared data, not code - which is the
point of a declarative fetcher, and why the level cost the tree no fetcher.

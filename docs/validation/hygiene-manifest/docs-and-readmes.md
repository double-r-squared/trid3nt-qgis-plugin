...
| docs/validation/demo-physics-defaults-audit.md (rowed as docs/design/demo-physics-defaults-audit.md) | 454 | The ADR 0285 law-9 audit: every baked demo physics constant, its consequence class and its derive-or-refuse disposition. | Rows 8-11, 16, 19, 23, 27 and the P6-P8 tail all name Landlab/GeoClaw/SWMM/SCHISM/HEC-RAS workflows that left the tree. | ROW RECONCILED IN ROUND 4. Verdict was DELETE on measured staleness; the layout stage MOVED it to `docs/validation/` instead, byte-identical, because the wave's instruction was the move and a measurement belongs beside the other measurements. The divergence is reported, not decided. Since the file is KEPT, the stray `</content>` authoring artifact this row found at its tail is REMOVED - a paste artifact is not a finding, so removing it does not touch what the record found. |
...
| docs/validation/external-fetch-audit.md (rowed as docs/design/external-fetch-audit.md) | 702 | The audit that found every network call outside the router - the basis for the no-double-middleware law and the fetch migration. | Its site list is mostly engine workers that left; the LAW it established is live and stated in AGENTS.md. | ROW RECONCILED IN ROUND 4. Verdict was DELETE; the layout stage MOVED it to `docs/validation/` byte-identical instead. Divergence reported, not decided. |
| docs/validation/fallback-audit.md (rowed as docs/design/fallback-audit.md) | 338 | The audit of every silent substitution site, the parked register, and the loudness gate it produced. | The parked register it describes now has one row ("11"). The `_router/stratified.py` claim this row carried is not in the file - it was the eval's note, not the audit's text. | ROW RECONCILED IN ROUND 4. Verdict was DELETE; the layout stage MOVED it to `docs/validation/` byte-identical instead. Divergence reported, not decided. |
...
| path | lines | what it is | stale claims | fate |
|---|---|---|---|---|
| docs/specs/calibration-seam.html | 163 | Spec for the calibration seam. | ROUND 4: its section 12 acceptance gate read "five slices + contracts + plugin at zero". This is an UNEXECUTED charter, so that line is an instruction the next wave will be held to rather than a record of a past walk; it now reads "all six slices at zero", which is the standing line and already contains contracts and plugin. The two other live specs (`mesh-recipe.html`, `module-surface.html`) carry the same phrase inside close-out tables that a frozen conformance record walks clause by clause, so they are left verbatim. | KEEP (unexecuted charter, not a claim about the tree) |
...
| docs/READABILITY_LEDGER.md | 278 | THE READABILITY LEDGER: every comment or docstring a better NAME or a small EXTRACTION would make unnecessary, as file / line / the comment as it stands / the change that would remove it / risk class (rename, extract, restructure). Written by this wave, applied by NOTHING. | None - it records proposals, so it carries no claim about the tree beyond its line references, which were spot-checked. Its standing condition is the ruling's own: after the docs wave NATE reads it and either takes a batch as its own reviewed change per module, where behaviour is verified as code, or moves on. | KEEP |
| docs/validation/hygiene-docs-conformance.md | 199 | The hygiene and docs wave's conformance table: every charter entry CONFORMS / DEVIATES with evidence, the four verify deviations with their resolutions, the LOC table on the corrected instrument. | Current (written at the remedy leg; rowed here by the orchestrator at close). | KEEP |
...
5. **`demo-physics-defaults-audit.md` carries a stray `</content>` tag** at its
   tail - a literal authoring artifact left in a committed file. CLOSED in round
   4: the file was moved rather than deleted, so the artifact was removed from
   it at `docs/validation/demo-physics-defaults-audit.md`.

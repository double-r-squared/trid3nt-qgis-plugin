...
- [CAND-S] `generic_passive_tracer_mass_conservation` [S] [US] - Injecting a conservative generic tracer at a boundary, does the domain-integrated mass stay conserved over the run (numerical-scheme sanity check)?
...
- [CAND-S] `tidal_constituent_extraction_inrun` [S] [US] - Over a multi-week tidal run, what are the extracted M2/K1/O1/... amplitude and phase at every node, without a separate post-processing harmonic-analysis pass?
...
- [CAND-S] `harmonic_analysis_ccrm_wiki_method_notes` [S] [US] - What does the harme.53 -> combine_outHA -> ad2tct.f post-processing chain produce, and how does it compare to an offline t_tide/UTide harmonic fit?
...
Today: roster note says steady flow is already signed (alongside levee and rain-on-grid) - candidates below extend it toward calibration and regulatory-floodway automation
...
- [CAND-S] `mixed_regime_multi_profile_solve` [S] [US] - Across a reach with alternating subcritical/supercritical reaches, does the steady solver correctly locate hydraulic jumps via the mixed-flow-regime algorithm?
...
- [CAND-S] `storage_area_network_flow_reversal` [S] [non-US] - On a flat multi-reach network with junctions and storage areas, does the unsteady solver correctly reproduce flow reversal and lateral-weir activation as tailwater exceeds headwater?
...
- [CAND-S] `pump_station_trigger_and_ramp_control` [S] [US] - For an interior-drainage pump station lifting flow from a 2D floodplain over a levee into a storage area, does the pump correctly stage-trigger on/off and ramp through its startup-transition time rather than stepping instantaneously?
...
- [CAND-S] `wq_module_smoke_test_suite` [S] [US] - Do HEC's own bundled water-quality test data sets (temperature/GCSM/nutrient toy cases) run to completion and match the manual's stated expected output, as a pre-replication smoke gate before attempting Mohawk-scale calibration?
...
- [CAND-S] `simple_breach_geometry_setup` [S] [US] - Can a minimal SA/2D-connection breach (crest elevation, auxiliary spillway, low-level outlet, progressive breach growth) be configured and run to completion as a bring-up smoke test before attempting the full Sayers Dam PMF case?
...
- [CAND-S] `classical_psha_nontrivial_logic_tree` [S] [US] - Given competing source models and multiple GMPEs per tectonic region, what is the mean hazard curve and 5/50/95 quantile spread across all logic-tree realizations?
...
- [CAND-S] `classical_psha_gr_uncertainty_logic_tree` [S] [US] - How sensitive is the hazard curve to Gutenberg-Richter a/b-value and Mmax epistemic uncertainty (LogicTreeCase2, 324 realizations)?
...
- [CAND-S] `classical_psha_uhs_and_hazard_map` [S] [US] - For a US site, produce a Uniform Hazard Spectrum and hazard map at 10%/2% probability of exceedance in 50 years alongside the curve.
...
- [CAND-S] `scenario_rupture_gmf_realization_set` [S] [US] - For a specified single-plane rupture, generate 10+ Monte Carlo ground-motion-field realizations across a site grid for downstream loss/secondary-peril use.
...
- [LANDED] `region_based_amr_refinement_windows` [S] [US] - How do I control AMR refinement level and duration with explicit lat/lon/time regions instead of relying on default error flagging? [REVISE 2026-08-05: NATE-flagged. Depth-COG postprocess emits ONE uniform-value rectangular patch (0.756/0.732 m over 21x46 px = a coarse AMR cell smeared through the land mask) - not real inundation; likely PRE-EXISTING in the geoclaw postprocess (check the ADR 0123 depth layer too). Proof gauges also sited where the knob has no visible effect. Revision wave dispatched.]
...
- [LANDED] `manning_friction_by_region` [S] [US] - How do I set a spatially-varying Manning's n bottom-friction coefficient (e.g. different value onshore vs offshore) instead of a single global n? [REVISE 2026-08-05: NATE-flagged. Depth-COG postprocess emits ONE uniform-value rectangular patch (0.756/0.732 m over 21x46 px = a coarse AMR cell smeared through the land mask) - not real inundation; likely PRE-EXISTING in the geoclaw postprocess (check the ADR 0123 depth layer too). Proof gauges also sited where the knob has no visible effect. Revision wave dispatched.]
...
- [CAND-S] `okada_1d_dtopo_smoke_case` [S] [non-US] - How do I run a minimal 1D Okada-generated-dtopo tsunami case with a fast, checkable output as a build/regression smoke test?
...
- [CAND-S] `fgmax_island_runup_maxima` [S] [non-US] - How do I set up an fgmax grid over an island/shelf domain to record max wave height and first-arrival time for runup analysis?
...
- [CAND-S] `sequential_stationary_snapshot_batch` [S] [US] - Give me wave conditions at several discrete times during an event without paying for a full nonstationary run.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node23.html (swan_user_manual_node23_mode)
  knobs: repeated MODE STATIONARY / COMPUTE STAT blocks with updated wind/boundary each pass
  notes: Pure orchestration of the existing stationary solve TRID3NT already runs; no new physics.
...
- [CAND-S] `gen3_westhuysen_default_wind_growth` [S] [US] - What is the standard, operationally-tuned wind-generation physics for a coastal storm run?
  ...
  notes: Confirms/pins the implicit default already presumably in use; makes it an explicit, auditable knob.
...
- [CAND-S] `gen_formulation_sensitivity_sweep` [S] [US] - How sensitive is significant wave height to the choice among GEN1/GEN2/Komen/Janssen/Westhuysen for this domain?
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: swap GEN1/GEN2/GEN3 KOMEN/JANSSEN/WESTH keyword across otherwise-identical runs
  notes: Pure knob sweep on an existing run; batching candidate for uncertainty/QA workflows.
...
- [CAND-S] `whitecap_dissipation_scheme_toggle` [S] [US] - Compare the default Alves-Banner whitecapping against the classical Komen formulation for the same storm.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: WCAPPING AB [cds2=5.0e-5] [br=1.75e-3]  vs  WCAPPING KOMEN [cds2=2.36e-5] [stpm] [powst] [delta] [powk]
  notes: Straight knob swap on existing generation setup.
...
- [CAND-S] `quad_integration_method_for_ambient_currents` [S] [US] - Get accurate quadruplet transfer in a domain with strong ambient tidal/inlet currents.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: QUADRUPL [iquad]=3 (fully explicit DIA per iteration, recommended with currents) vs default 2
  notes: Single-integer knob change on the existing quadruplet term.
...
- [CAND-S] `constant_breaker_index_surfzone_default` [S] [US] - Standard surf-zone wave-height dissipation for a beach or barrier-island run.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: BREAKING CONSTANT [alpha=1.0] [gamma=0.73]
  notes: Makes the implicit default an explicit, documented knob.
...
- [CAND-S] `jonswap_friction_regional_coefficient_calibration` [S] [US] - Calibrate bottom friction for a specific US shelf - sandy Atlantic/Gulf shelf vs the smoother Gulf of Mexico bottom the manual explicitly calls out.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: FRICTION JONSWAP [cfjon]=0.038 (default, sandy) or 0.019 (manual-recommended for Gulf of Mexico-type smoother beds)
  notes: Manual gives a named, US-specific (Gulf of Mexico) alternate default - directly reusable calibration knob.
...
- [CAND-S] `dcta_default_triad_surfzone_harmonics` [S] [US] - Standard shallow-water triad energy transfer for a surf-zone or tidal-flat run.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
  knobs: TRIAD DCTA [trfac=4.4] [p=4/3] COLL (default) or NONC for noncollinear
  notes: Makes the implicit default explicit and exposes the COLL/NONC switch.
...
- [CAND-S] `biphase_parametrization_eldeberky_vs_dewit_calibration` [S] [US] - Tune the phase-coupling assumption underlying triad energy transfer for a specific site's Ursell-number regime.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node28.html (swan_user_manual_node28_physics)
...
- [CAND-S] `parametric_boundary_spectrum_from_hindcast_or_buoy` [S] [US] - Force the domain boundary with a simple Hs/Tp/direction/spread set from a buoy observation or hindcast point.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node27.html (swan_user_manual_node27_boundary)
  knobs: BOUNDSPEC SIDE [side] PAR [hs] [per] [dir] [dd]
  notes: Simple parametric knob; NDBC buoy data is the natural US fetcher pairing.
...
- [CAND-S] `full_partition_parameter_set_windsea_swell_export` [S] [US] - Report the complete per-partition parameter set (not just Hs) - period, wavelength, direction, spread, energy fraction, steepness - for each wind-sea/swell train.
  ...
  notes: Extends the already-signed partition capability to the full published parameter set rather than a subset.
- [CAND-S] `netcdf_block_output_for_qgis_cog_pipeline` [S] [US] - Get SWAN's gridded output directly in a format that slots into TRID3NT's QGIS-native raster/COG publishing path.
  src: https://swanmodel.sourceforge.io/online_doc/swanuse/node32.html (swan_user_manual_node32_output)
  knobs: BLOCK 'sname' NOHEADER '<file>.nc' LAYOUT ... (netCDF output option)
  notes: Format-selection knob on the existing BLOCK output; aligns with the drop-TiTiler/QGIS-native-raster direction already in the sprint plan.
...
- [CAND-S] `closed_form_damage_state_probability_check` [S] [US] - Does pelicun's Monte-Carlo damage-state sampling match the analytic lognormal closed-form probability for a 2-DS sequential component?
...
- [CAND-S] `mixed_fragility_and_loss_function_aggregation` [S] [US] - How do fragility-driven damage consequences and direct loss-functions combine in one P-58 assessment, and how do AcrossFloors/AcrossDamageStates aggregation settings change portfolio Cost and Time?
...
- [CAND-S] `auto_populated_building_type_seismic_run` [S] [US] - Given a single building's AIM (asset inventory model) attributes and a response.csv of EDPs, does DL_calculation.py with --auto_script correctly infer the HAZUS building type/design level and reproduce the checked-in damage/loss response?
...
- [CAND-S] `hazus_eq_v5_vs_v6_dataset_comparison` [S] [US] - How much do portfolio-level damage-state probabilities and repair-cost estimates shift between Hazus v5.1 and v6.1 building fragility/consequence datasets for the same building type?
...
- [CAND-S] `foundation_type_depth_damage_sweep` [S] [US] - How does the FloodRulesets.py auto-population logic assign flood building class from foundation type and first-floor elevation, and how sensitive is the resulting repair-cost loss function to that assignment?
...
- [CAND-S] `standalone_vs_coupled_surge_consistency` [S] [US] - Does the standalone HAZUS-flood loss for a given depth match the storm-surge branch used inside the coupled wind+surge module for the same building/depth (consistency check across the two integration points)?
...
- [CAND-S] `loss_function_only_1to1_validation` [S] [US] - For a component defined purely via a loss-function (no fragility/damage-state layer), does the resulting loss distribution exactly reproduce the input EDP distribution as pelicun's own closed-form CI test expects?
...
- [CAND-S] `correlation_structure_sensitivity` [S] [US] - How much does portfolio loss uncertainty (Cost/Time distribution spread) change between a perfectly-correlated and an independent EDP correlation structure for the same component inventory?
...
- [CAND-S] `rid_from_pid_inference` [S] [US] - Given only peak interstory drift (PID) samples, does pelicun's RID-inference feature produce residual-drift estimates usable to trigger P-58 irreparable-damage consequences without a separate RID demand file?
...
- [CAND-S] `replacement_threshold_override_sweep` [S] [US] - At what aggregate damage/RID threshold does the irreparable-damage override switch a building's reported loss from summed repair consequences to full replacement cost/time, and how does moving that threshold change the portfolio loss curve?
...
- [CAND-S] `loss_model_sample_save_load_consistency` [S] [US] - Does a LossModel's aggregated decision-variable output remain numerically identical after saving samples to disk and reloading them into a fresh session (needed for staged/batch regional runs)?
...
- [CAND-S] `unconfined_newton_dry_rewet_channel` [S] [US] - Can the solver handle a staircase-shaped impervious base drying and rewetting an unconfined channel without oscillation/failure?
...
- [CAND-S] `maw_crossaquifer_nonpumping_analytical` [S] [US] - Does a non-pumping multi-aquifer well equilibrate to the Sokol (1963) analytical water level between two confined aquifers?
...
- [CAND-S] `hfb_barrier_wall_containment_knob` [S] [US] - Can a defined-thickness barrier between two specific cells reduce flux across a wall to a target hydraulic characteristic, independent of grid refinement?
...
- [CAND-S] `prt_forward_structured_steady_vs_modpath7` [S] [US] - Do native PRT pathlines/travel-times on a structured grid with a well+river match an equivalent MODPATH 7 run exactly (same input, cross-tool validation)?
...
- [CAND-S] `advection_scheme_toggle` [S] [US] - Should momentum advection use the current default upw1 scheme or the legacy 'original' scheme (Leijnse et al. 2021) for backward-compatible runs?
...
- [CAND-S] `manning_roughness_zonation_mode` [S] [US] - Should Manning's n be spatially uniform, land/sea-differentiated, or fully spatially varying per cell?
...
- [CAND-S] `momentum_smoothing_theta_tuning` [S] [US] - Does this domain need reduced implicitness (theta between 0.8-1.0) to damp instabilities on steep/complex bathymetry?
  src: https://sfincs.readthedocs.io/en/latest/parameters.html (sfincs-parameters-theta)
  knobs: theta (0.8-1.0, default 1.0)
- [CAND-S] `horizontal_viscosity_smoothing` [S] [US] - Should horizontal eddy viscosity smoothing be enabled (viscosity=1, nuvisc) to stabilize flow around sharp topographic gradients?
...
- [CAND-S] `subgrid_stability_limiter_tuning` [S] [US] - For numerically stiff subgrid domains (thin channels, steep slopes), should uvmax/hmin_cfl/uvlim/slopelim/wiggle_suppression be tightened from defaults to avoid CFL-driven blowups?
...
- [CAND-S] `subgrid_qtable_weighting_method` [S] [US] - Should the subgrid flux lookup table use the legacy (q_table_option=1) or improved SFINCS>=2.1.1 (q_table_option=2, default) weighting method?
...
- [CAND-S] `grid_type_regular_vs_quadtree` [S] [US] - Should the model grid be built as a uniform regular grid or a quadtree grid (grid_type='quadtree') for domains with mixed resolution needs?
...
- [CAND-S] `quadtree_mesh_netcdf_qgis_output` [S] [US] - Should sfincs_map.nc be written/read as native quadtree mesh netcdf (outputformat=1) for direct QGIS mesh-layer loading, instead of resampled regular-gridded netcdf (0, default)?
...
- [CAND-S] `incident_wave_setup_toggle` [S] [US] - Should incident-wave-induced setup/forcing be included or disabled (snapwave_waveforces_factor=0) for a run where only surge/riverine drivers matter?
...
- [CAND-M] `cn_infiltration_from_gridded_curve_number` [M] [US] - Should net rainfall be derived from a gridded global Curve Number dataset (e.g. gcn250) with a chosen antecedent moisture condition, rather than a single uniform qinf value?
...
- [CAND-S] `spatially_uniform_constant_infiltration` [S] [US] - For a quick screening run, should a single spatially-uniform constant infiltration rate (qinf) be used instead of a gridded method?
...
- [CAND-S] `weir_discharge_coefficient_tuning` [S] [US] - Should the weir discharge coefficient (par1/cd, default 0.6) be adjusted per structure type (sharp-crested vs broad-crested levee)?
...
- [CAND-M] `reanalysis_gridded_precip_forcing` [M] [US] - Should the model be forced with gridded hourly reanalysis precipitation (e.g. ERA5) over the domain rather than a single uniform rain gauge time series?
...
- [CAND-S] `uniform_precip_timeseries_forcing` [S] [US] - For a quick single-gauge or design-storm run, should spatially uniform precipitation (sfincs.prcp, mm/hr time series) be used?
...
- [CAND-S] `gridded_precip_interpolation_mode` [S] [US] - Should gridded precipitation rates be treated as block-constant per timestep (ampr_block=1, default) or linearly interpolated between timesteps (ampr_block=0)?
...
- [CAND-L] `spiderweb_tropical_cyclone_wind_forcing` [L] [US] - For a hurricane/tropical storm scenario, should wind (and implied pressure) be forced from a spiderweb vortex file (track + radii of max wind) rather than a uniform or gridded field?
  src: https://sfincs.readthedocs.io/en/latest/input_forcing.html (sfincs-input-forcing-spiderweb)
  knobs: spwfile / netspwfile
- [CAND-M] `gridded_reanalysis_wind_forcing` [M] [US] - Should the model be forced with a gridded time-varying wind field (amu/amv or netCDF) instead of a single uniform wind vector?
...
- [CAND-S] `wind_drag_coefficient_curve_tuning` [S] [US] - Should the wind drag coefficient curve (cd_nr breakpoints of cd_wnd vs cd_val) be adjusted from SFINCS defaults for extreme-wind (hurricane-force) conditions where drag saturates/decreases?
  src: https://sfincs.readthedocs.io/en/latest/input.html (sfincs-input-wind-drag-curve)
  knobs: cd_nr, cd_wnd[], cd_val[]
- [CAND-S] `uniform_wind_timeseries_forcing` [S] [US] - For a simple screening run, should a single uniform wind speed+direction time series (sfincs.wnd) be used?
...
- [CAND-S] `swmm_infiltration_method_comparison` [S] [US] - For the same subcatchment and storm, how much does the choice of infiltration method (Horton vs Green-Ampt vs Curve Number) change peak flow and total runoff volume?
...
- [CAND-S] `swmm_predev_postdev_runoff` [S] [US] - For the same parcel, how much more and how much faster does post-development (increased imperviousness) runoff arrive compared to pre-development (pasture/undeveloped) conditions?
...
- [CAND-S] `swmm_node_surcharge_ponding` [S] [US] - When the pipe/mesh network is undersized for a storm and backs up, where does water surcharge and pond at the surface, and how much depth/volume accumulates at each flooded node?
...
- [CAND-S] `swmm_weir_orifice_outlet_family` [S] [US] - For an outfall or regulator structure, how does discharge behavior differ across a transverse weir, a V-notch weir, a circular orifice, and a rating-curve outlet at the same pond/node?
...
- [CAND-S] `swmm_flow_diversion_structure` [S] [US] - At a junction where flow needs to split between a main channel/pipe and a relief/diversion pipe (e.g. to protect a downstream WWTP or route excess to a bypass), how much flow goes each way as the incoming flow rises?
...
- [CAND-S] `swmm_pump_curve_wetwell` [S] [US] - For a lift station with a defined pump curve, how does wet-well depth and pump discharge respond over a storm, and at what depths do the pumps turn on and off?
...
- [CAND-S] `swmm_pump_alternation_duty_standby` [S] [US] - With three pumps sharing one wet well, how does a depth-staged duty/standby alternation rule (only one pump running at low depth, two at higher depth) change pump run-time and cycling compared to running all pumps off one fixed setpoint?
...
- [CAND-S] `swmm_rtc_multicondition_rules` [S] [US] - With a rule that only opens a regulator when BOTH upstream depth is high AND downstream link flow is below a threshold, how does that multi-condition logic change regulator behavior versus a single-condition rule?
...
- [CAND-S] `swmm_lid_green_roof_detention` [S] [US] - For a subcatchment with a fraction of its impervious roof area converted to a green roof, how much less and how much slower does runoff leave the subcatchment compared to the same area with conventional roofing?
...
- [CAND-S] `swmm_lid_rainbarrel_rooftop_disconnect` [S] [US] - For a residential lot, how much roof runoff does simple rooftop disconnection (draining onto a pervious yard) capture compared to a rain barrel (fixed-volume cistern with a controlled release), for storms of increasing size?
...
- [CAND-S] `swmm_lid_vegetative_swale_conveyance` [S] [US] - Routing runoff through a vegetated swale (a surface-layer-only LID acting as a conveyance element, not just storage) instead of a conventional lined channel, how much does travel time and peak attenuation change along the same flow path?
...
- [CAND-S] `wind_stress_forcing` [S] [US] - How does sustained wind set up a water-surface slope or drive circulation on a wide reach, embayment, or lake?
...
- [CAND-S] `instantaneous_dam_breach_flood_wave` [S] [US] - How does a sudden (or progressive) dam/levee breach propagate a flood wave downstream, and how far/fast does the wave attenuate?
...
- [CAND-S] `upstream_sediment_supply_boundary` [S] [US] - How does a prescribed upstream sediment SUPPLY rate (rather than an initial bed stock) change downstream deposition patterns - the reservoir-inflow-sedimentation framing?
...
- [CAND-S] `sisyphe_gaia_migration_path` [S] [US] - For legacy TELEMAC setups authored against SISYPHE (the module GAIA replaced), can TRID3NT auto-translate steering-file sediment blocks to GAIA's newer keyword set?
...
- [CAND-S] `micropollutant_fate_validation_case` [S] [US] - Can TRID3NT stand up WAQTEL's MICROPOL sub-model (organic-load degradation, nitrification, algal uptake) using the shipped validation steering file as a golden regression case?
...
- [CAND-S] `overlap_wwm_vs_swan` [S] [US] - WWM (Wind Wave Model) vs SWAN - when does each win?
...
- [CAND-S] `overlap_gaia_vs_sed3d_vs_hecras_sediment` [S] [US] - GAIA vs SED3D vs HEC-RAS sediment - which for which scale/question?
...
- [CAND-S] `overlap_waqtel_vs_icm_vs_swmmwq` [S] [US] - WAQTEL vs ICM vs SWMM-WQ water quality - receiving water vs pipe network vs urban catchment?
...
- [CAND-S] `overlap_rain_on_grid_hecras_swmm_sfincs` [S] [US] - Rain-on-grid: HEC-RAS 2D vs SWMM vs SFINCS pluvial - which physics/scale for which pluvial question?

...
- [CAND-M] `hazus_eq_v5_vs_v6_dataset_comparison` [S->M, STOP ADR 0146: no v5.1 buildings alias exists; recipe = load v5.1 fragility/consequence csv by path vs the v6.1 alias at the Assessment API] [US] - How much do portfolio-level damage-state probabilities and repair-cost estimates shift between Hazus v5.1 and v6.1 building fragility/consequence datasets for the same building type?

...
Purpose: mesh is one of the big-3 inputs (data + MESH + engine); this cross-engine
row tracks the explicit, user-driven mesh surface, distinct from any single
solver's private meshing.
- [LANDED] `generate_mesh` + mesh precondition gate [M] [US] (2026-08-08, ADR 0200):
  the standalone mesh builder promotes the ADR 0193 watershed + ADR 0194 coastal
  water-edge sandbox meshers behind ONE registered tool (mode inferred from
  inputs; `min_edge_length_m`/`max_edge_length_m`/`grade` user levers). Emits an
  MDAL `.2dm` display layer (`layer_type="mesh"`, explicit UTM `crs_authid`) + a
  durable `MeshArtifact` (format URIs, CRS, node/element counts, has_bathymetry,
  open-boundary info, engine_compat) via a same-process case stash + a co-located
  `mesh_artifact.json` sidecar (no parallel store). Model templates gain
  PRECONDITION POLYMORPHISM: `telemac_rain_on_grid` (first consumer) discovers a
  case mesh at run start, validates engine-compat, and fires the yes/no gate on
  the existing input-review `tool-payload-warning` spine ("use this mesh?", labeled
  default USE per the user-mesh>drawn-box>geocoded-AOI ranking) -- accepted routes
  the supplied-mesh path, declined/incompatible falls back to fresh authoring with
  a loud one-line skip. Offline: 19 new tests (registration + retrieval top-8 +
  mode inference + 2dm round-trip + engine-compat gatekeeper + gate decision
  logic) + `use_supplied_mesh_2dm` node-population test. Engine-generic helper so
  SCHISM/SWAN adopt the same gate by passing their own `engine`.
  ADOPTION (2026-08-09, ADR 0208): the SCHISM/SWAN gate branches are now REAL. (1)
  `generate_mesh` gained an `open_boundary_side` lever -- a COASTAL mesh built with
  it also emits a SCHISM `hgrid.gr3` (via the worker's `tin_to_hgrid` bridge, depth
  negated to positive-down) + records `open_boundary_info` (side + open-node count)
  + adds `"schism"` to `engine_compat`. Compat facts tightened: SCHISM needs a
  designated OPEN boundary (bare bathymetry is not enough), so an inland WATERSHED
  mesh is honestly declined. (2) SWAN is `unstructured_unsupported` (regular-grid
  worker: CGRID REGULAR + bottom.bot, no fort.14 path) -> `mesh_compatible_with_engine`
  returns False unconditionally with that reason; NO gate wired. (3) Consumer
  `schism_baroclinic_circulation` wired (additive `supplied_mesh` on the deck
  author). Also FIXED a latent coastal-mode bug (the in-container mesher reads the
  water polygon from a FILE, not the inline dict generate_mesh passed). LIVE:
  coastal Galveston mesh 2743 nodes / 4763 elems EPSG:32615, engine_compat=[telemac,
  schism], open_node_count=93 (mesh-01KZKKQJK0BX2HFA3N7X0ZKYTP); gate accepted +
...
- [LANDED] `baroclinic_3d_circulation` [L] [US] (2026-08-08, ADR 0189: the NEW `schism_baroclinic_circulation` template, registry 232->233. The 3D density-driven pathway on the EXISTING hydro-core binary `pschism_TVD-VL`: an SZ vertical grid (`_author_sz_vgrid`, nvrt=10), ibc=0 baroclinic, an estuarine salinity-GRADIENT IC (fresh landward -> ocean-salty seaward), a sustained freshwater river point SOURCE (source_sink.in/vsource.th/msource.th S=0), TVD tracer transport (itr_met=3 + tvd.prop), a tidal ocean boundary, ics=2 (lat/lon). Deliverable = surface + bottom salinity COGs + top-minus-bottom stratification. Coarse georeferenced channel over a US estuary footprint (Delaware Bay default) - a screening geometry, NOT surveyed bathymetry. Live coarse smoke through run_solver+MinIO (sim_days=1.0, 240 nodes x 10 layers, wall 72s, run 01KZGMMBZ3C8FJ0XVGTDWPPG53): surface salinity 0.97-25.85 psu, bottom max 32.41 psu, stratification mean 5.19 / max 9.04 psu - a physically-sane salt wedge. Showcase case 01KZGNQ36SG737MEKVJZCAQMTA (3 layers, reconnect-durable). Proofs docs/proof/templates/schism_baroclinic_circulation_{surface_salinity,bottom_salinity,mesh,stratification_chart}.png. PENDING NATE-gated validation: the calibrated Test_CORIE Columbia River estuary 28-day 3D baroclinic hindcast vs ADCP/CTD stations (~600MB deck, hours-class, ADR 0126 sec 2c). 0191 REVISION (spot-check fix): the mesh is now SHORELINE-CLIPPED to the real NHDArea water polygons (largest connected component; land cells dropped) so the salinity raster never paints land; the default estuary moved from Delaware Bay (mostly LAND at that box) to Galveston Bay (79% water); the demo sim_days is 4.0 so river+tide restructure the field + a NEW surface-salinity-minus-IC change proof shows CIRCULATION (not the IC echoed back); fixed a latent schism_gr3.tin_to_hgrid bug (stale n_elem after pinch-point removal). Re-run 01KZH7B3DM2ADA4GXAETB4BSBN (668-node clipped mesh, strat mean 9.98 / max 18.05 psu).) - Given a shelf/estuary AOI, what is the density-driven 3D current + stratification structure?
  src: https://schism-dev.github.io/schism/master/schism/barotropic-solver.html (schism-docs-barotropic-solver)
  knobs: ibc=0 (baroclinic on), river_discharge_m3s (freshwater source), ocean_salinity_psu, vgrid SZ layer count, ocean_side, sim_days
  mesh-gate (2026-08-09, ADR 0208): FIRST SCHISM consumer of the mesh precondition gate. `author_baroclinic_estuary_deck` gained an additive `supplied_mesh=(points_lonlat, tris, depths_down)` param -- when the case holds a SCHISM-compatible `generate_mesh` mesh (coastal + open boundary) the gate offers it; accepted -> the user mesh (real shoreline + real bathymetry) REPLACES the idealized lattice, `ocean_side` taken from the mesh's designated open boundary, and the salinity IC gradient + river source + tidal boundary are re-authored on those nodes (forcing stays idealized, only geometry becomes real; surfaced in synthetic_inputs). Declined/absent/incompatible -> the idealized channel, unchanged. LIVE: consumed the Galveston coastal mesh (mesh-01KZKKQJK0BX2HFA3N7X0ZKYTP, 2743 nodes / 4763 elems, open_node_count 93) into a full deck AND the SCHISM baroclinic solver ran it to completion (run 01KZKN0GSA11T5YP4FV0A4C6VE, status=complete) -- consumed->solved closed. coupled_waves SKIPPED (its mesh is the bundled Test_WWM_Duck FRF validation deck); tidal_hydro coastal_tin queued as a same-helper candidate.
...
- [LANDED] `parametric_spectra_wave_forcing` [M] [US] (2026-08-08, ADR 0189: four composable KNOBS on the existing `schism_coupled_waves` template - `significant_wave_height_m` / `peak_period_s` / `mean_direction_deg` / `directional_spread` - registry UNCHANGED. Setting any switches the WWM open boundary from the bundled non-parametric Duck 8m-array spectrum to a PRESCRIBED parametric JONSWAP boundary (`_transform_wwm_input_parametric`: LBCWA=T/LBCSP=F/IBOUNDFORMAT=1/WBSS=2/WBHS/WBTP/WBDM/WBDS on the `pschism_WWM_GOTM_TVD-VL` binary built ADR 0131). The observed-gauge cross-shore V&V is honestly SKIPPED in parametric mode (a synthetic forcing does not reproduce the real 12 Oct 1994 event). Deliverable = nearshore Hs COG + wave SETUP (`read_wave_setup_from_out2d`, radiation-stress super-elevation). Two live runs through run_solver+MinIO on the Duck FRF (17054 nodes, sim_hours=1.0): Hs=4.0 m storm -> nearshore hs_max 4.0 / hs_mean 1.22 m (run 01KZGMX30R0YDQRJA8ZNFWAT15, wall 674s); Hs=1.5 m calm -> hs_max 1.66 / hs_mean 1.02 m (run 01KZGNJH7546Z71KZM2BACBFA7) - the knob is honored (the nearshore field scales with the offshore forcing). Showcase `!run schism_coupled_waves(significant_wave_height_m=4.0, peak_period_s=13.0, mean_direction_deg=70.0, directional_spread=25.0, sim_hours=0.5)`. Proofs docs/proof/templates/schism_coupled_waves_parametric_{hs,knob_chart,mesh}.png.) - Given a prescribed offshore JONSWAP spectrum, what nearshore wave transformation + setup result?
  src: https://schism-dev.github.io/schism/master/modules/wwm.html (schism-docs-wwm)
  knobs: significant_wave_height_m, peak_period_s, mean_direction_deg, directional_spread (WWM parametric JONSWAP &BOUC)
...
Aspects: multi-profile standard-step solve; mixed flow-regime (sub/super-critical) passes; Manning's n calibration to observed high-water marks; floodway/encroachment determination; 2D rain-on-grid (pluvial) + SCS-CN infiltration
- [LANDED-SUBSTRATE] `hecras_2d_rain_on_grid` [M] [US] - Given a design storm over a catchment (no inflow hydrograph), what 2D inundation + outlet discharge does HEC-RAS 2D produce with SCS-CN infiltration losses? [ADR 0199 2026-08-08: RoG authored as composer KNOBS (design_storm_mm_per_hr + storm_duration_hr + curve_number/amc_condition) on the fresh-AOI hecras_flood_2d chain. LANDED offline + live-ACCEPTED: the SCS-CN Infiltration geometry layer (decoded byte-exact from Bald Eagle g09) + the uniform-storm Meteorology Precipitation/Values + HEC-format Timestamp all pass the 6.6 engine's readers (RasGeomPreprocess exit 0; RasUnsteady reads past "Reading 2D Area(s)"). RESIDUAL: the SOLVE blocks at READ_UN_M2D_PRECIP_INTERP (MetInterp.f90) per-2D-area interpolation folder - the GUI-precomputed schema that segfaults authored blind (REFERENCE-FIXTURE gap, same wall as the 1D-steady row; needs a reference RoG deck). Registered-tool surface + worker parser FROZEN pending the link. HUNT 2026-08-08 (ADR 0199 appendix): the bounded reference hunt downloaded the official 6.6 Example Projects (sha256 ea239b50..., schema-matched to our solver) and found the only RoG-capable 2D example (BaldEagleCrkMulti2D, gridded NEXRAD/DSS precip + real SCS-CN Infiltration.hdf), but shipped decks are CONFIG-ONLY: an airtight scan shows the per-2D-area interpolation folder is present in ZERO shipped HDFs, and the whole 432MB set ships exactly ONE computed plan HDF (a WQ example, no precip). ras-commander RasPreprocess confirms verbatim the Linux binaries CANNOT produce the .tmp.hdf -- the folder is a Windows RAS-preprocessing artifact. STOPPED per the no-guess guardrail; unblock = a NATE Windows-GUI export of one COMPUTED plan HDF (decode harness scripts/sandbox/hecras/decode_precip_interp.py staged; reference deck staged at scripts/sandbox/hecras/_ref/, gitignored). UPDATE 2026-08-08 (ADR 0205, headless decode): the interp folder is now DECODED byte-exact and AUTHORED -- NOT from RAS Mapper (decompile showed the beta gutted the meteorology writer + the 2025 managed engine interpolates in-memory, never writing the folder) but from the un-stripped 6.6 RasUnsteady binary itself: READ_HDF_INTERP_COEFF names six CSR datasets (Cell/Face Info[start,count] int32 + Indexes int32 + Weights float32), nearest-neighbour single-pixel source (write_precipitation_interpolation in hecras_meteorology.py). LIVE: the engine reads PAST the MetInterp SIGSEGV that blocked all 12 prior attempts. RESIDUAL moved one layer deeper to READ_UN_HYDROLOGY2D: precip application (INIT_PRECIP2CELL->precip2fvcell) + SCS-CN both route through the 2D-hydrology module, which faults in output-id/region setup (H5Gcreate2 invalid location); crash-free without infiltration but applies ZERO precip (cells never linked). SCS-CN Infiltration + the newly-decoded sibling Percent Impervious groups authored + gated (apply_infiltration, default off). Narrower unblock: a NATE computed plan HDF to decode the hydrology output/region scaffold (the precip interp folder is SOLVED). Offline freshtopo 39 passed. UPDATE 2026-08-09 (ADR 0207, preproc-shim hunt): the SOLVE is UNBLOCKED on Linux and NATE's Windows export is NO LONGER REQUIRED -- not via the 6.6 Fortran path but via the 2025 MANAGED engine, which preprocesses (`ras prepare` -> ready-to-run `.r2r.h5`, the Windows `.tmp.hdf` equivalent) AND solves 2D unsteady SWE on Linux with the CPU solver (the only native P/Invoke in Ras.Engine is the CUDA/GPU path; the CPU stack is pure managed C#). RAIN delivery LIVE: a constant PrecipitationLayer applies rain to every cell in-memory (the ADR 0205 InitializeComputeDriver path), depth rises uniformly + linearly, mass-conservative (rate 100 -> +0.10 ft/hr, 300 -> +0.30 ft/hr) -- exactly what the 6.6 path could not do ("Cell are not linked" -> zero precip). Driver + repro: scripts/sandbox/hecras/managed_solve/. Remaining = authoring plumbing for a real-catchment (Muncie/Coweeta) 2025 RoG project + metric extraction + registration, all now on Linux; registered surface stays FROZEN pending that. UPDATE 2026-08-09 (ADR 0209, PRODUCTIONIZED): the real-catchment 2025 RoG path is LIVE end-to-end. `RealTerrainRoG` (Driver.cs) authors a structured 2D area + constant design storm + NormalDepth outlet over the fetched terrain (host reprojects to local SI + overwrites the terrain tif); four beta constraints cracked evidence-first (terrain tif must be TILED+NoData+OVERVIEWS or "Missing terrain data at Face"; terrain finer than the mesh cell; nodata nearest-filled; external BC line must PROTRUDE past the mesh corners or it is classed INTERNAL). Units DECODED + mass-checked: SI ConstantValue == mm/hr exactly (25 -> 25.0002 mm/hr uniform). INFILTRATION verdict: ABSENT in the 2025 managed engine (no Infiltration/CN symbol in Ras.Core/Ras.Engine; the 26 hits are the decoupled 6.6 Ras.Mapper) -> rain-only, stated. Metrics from `DEBUG/CellVolume` (true subgrid volume; depth x area overcounts ~6x) + mass-balance outlet Q. Muncie de-risk (8591 cells) mass-closes; Coweeta (28.87 km2, 8018 catchment cells, DWE) peak 195.3 m3/s @ 5.7 h, runoff coeff 0.77, max depth 8.98 m, wall 218 s. Registration UNFROZEN: `design_storm_mm_per_hr`/`storm_duration_hr` knobs on hecras_flood_2d dispatch the 2025 path (`model_hecras_flood_2d_rog` -> rog2025_pipeline -> depth COG); curve_number/amc inert (no infiltration). Mounted-driver = NO worker-image rebuild. Pipeline: rog2025_pipeline.py; driver: managed_solve/Driver.cs; proofs: hecras_flood_2d_rog_{depth,compare_chart}.png; offline: freshtopo 44 passed (39+5). ADR 0209. UPDATE 2026-08-09 (ADR 0210, PAPER-STYLE REFINED MESH): the uniform structured grid is now optionally a GRADED mesh (Godara et al. dynamic resolution) via a `channel_refinement` knob. Decompiled MeshFactory.TryCreateMesh(perimeter, cellCenters, IList<Polyline> breaklines, ...) -- NO refinement-region arg; the cell-center SPACING is the sizing, breaklines only magnetize facepoints. rog_refine.py authors variable-radius Poisson-disk seeds graded from the background cell size down to a channel size along the delineated channel network + main-stem breaklines; a degree-repair decimation keeps interior Delaunay degree <=8 (HEC hard-rejects >8-sided cells; blue-noise + repair converges where lattices/jitter/Lloyd failed). RealTerrainRoG.CreateMesh() override calls TryCreateMesh on the staged seeds; meshprobe mode verifies fast. Coweeta refined (22 m channel / 90 m background, 19462 cells): status=Complete, passes ras prepare, realized histogram ~7k fine (p5 25.7 m) + ~10k coarse (80-120 m). SOLVE vs uniform-60m: peak 195->200 m3/s but 5.7->4.9 h EARLIER, max vel 5.7->6.8 m/s, max depth 8.98->9.12 m, mass closure 99.6% both, 52% fewer total cells / 25% more channel cells, wall 218->415 s (dt 3->1.5 s). Refinement SHARPENS channel routing (paper rationale confirmed); peak-Q magnitude/extent/mass unchanged. Knob DEFAULT OFF (fidelity/cost trade, not strictly better) -- showcase stays uniform, no re-seed. Metrics mesh-agnostic (per-cell Voronoi areas) + nearest-cell depth COG for the graded mesh. Proofs: hecras_flood_2d_rog_mesh.png (graded wireframe) + hecras_flood_2d_rog_depth_refined.png + updated compare_chart (uniform vs refined vs TELEMAC). Offline: freshtopo 21-slice green (+6 test_rog_refine). ADR 0210.]
  src: Godara, Bruland and Alfredsen 2024, Front. Water 6:1384205 (NATE-provided)
  knobs: design_storm_mm_per_hr (25) x storm_duration_hr (6) = 150 mm; curve_number (80) + amc_condition (I/II/III, NRCS NEH-630); ia_ratio (0.2 std / 0.05 rev); Outlet normal-depth BC at the pour point (rain-fed interior)
  notes: Coweeta Creek NC (pour point -83.40402 35.05746), the TELEMAC RoG twin's catchment. TEMPLATE SMOKE, not the NATE-gated replication calibration.
- [LANDED-COMPARE] `rog_cross_engine_telemac_vs_hecras` [M] [US] - On the same steep catchment + design storm, how do TELEMAC-2D (triangular TIN) and HEC-RAS 2D (structured subgrid) compare on peak outlet Q, wet extent, wall time, and the paper's stability findings? [ADR 0199: harness scripts/sandbox/hecras/rog_compare_engines.py. TELEMAC side LIVE (ADR 0196: AMC II peak 45.5 m3/s, CORRECT END - the paper's "triangular mesh stable on steep terrain" CONFIRMED on our catchment); HR2D side authors the Coweeta RoG deck + records the MetInterp block; the HR2D peak-Q/wet/stability comparison pends the solve link. HUNT 2026-08-08: the HR2D solve link stays blocked -- the bounded reference hunt (ADR 0199 appendix) proved the per-2D-area precip interpolation folder is a Windows RAS-preprocessing artifact absent from all shipped USACE examples; unblock = a NATE GUI export. The comparison table therefore still reports the TELEMAC row live + the HR2D row as authored-and-blocked, honestly, not fabricated. UPDATE 2026-08-08 (ADR 0205): the HR2D precip interpolation folder is now DECODED byte-exact from the un-stripped 6.6 solver + authored (the engine reads past the MetInterp segfault); the HR2D solve residual moved down to the 2D-hydrology module (READ_UN_HYDROLOGY2D output/region setup) which gates BOTH rain application and SCS-CN loss. HR2D peak-Q/wet/stability comparison still pending a real solve (unblock = a NATE computed plan HDF for the hydrology scaffold); TELEMAC row unchanged. UPDATE 2026-08-09 (ADR 0207): the HR2D solve is UNBLOCKED on Linux via the 2025 MANAGED engine (prepare+solve, CPU, rain applied to cells + mass-conservative) -- no Windows/NATE export needed. The HR2D peak-Q/wet/stability row now pends only the real-catchment 2025-project authoring + metric extraction (Coweeta), a bounded follow-up; the TELEMAC row remains live. UPDATE 2026-08-09 (ADR 0209, TWO-SIDED): the comparison is finally two-sided. Coweeta Creek NC (28.87 km2, 25 mm/hr x 6 h), catchment-restricted like-for-like: HEC-RAS 2025 (DWE, rain-only) peak 195.3 m3/s / runoff coeff 0.77 / max depth 8.98 m / max vel 5.71 m/s / wall 218 s vs TELEMAC-2D (AMC II, SCS-CN loss) peak 45.5 m3/s / coeff ~0.28 / max depth 6.95 m / wall 64 s. HONEST reading: the ~4x peak-Q gap is DOMINATED by the infiltration difference (HR beta has no infiltration layer -> rain-only, reaches equilibrium Q ~= rainfall x area; TELEMAC removes ~65-70% via CN), NOT the hydraulics -- max depths agree to order. COST finding (direction-consistent with the paper's "HR2D ~5x slower"): DWE fast + stable; full-SWE on the steep grid is dramatically costlier (>1 h CPU). rog_compare_engines.py drives the 2025 path (--hecras/--full-swe). Proofs: hecras_flood_2d_rog_{depth,compare_chart}.png. ADR 0209.]
  src: Godara et al. 2024 (the paper's own TELEMAC-vs-HEC-RAS experiment, replicated on a US catchment)
  notes: reference comparison, honest - disagreement with the paper is a finding, not a failure.
...
- [LANDED] `2d_model_stability_diagnostic_sweep` [M] [US] - Given a levee-protected town downstream of a 2D mesh, does systematically tightening timestep (Courant~1.0), fixing downstream slope, raising culvert inverts to cell minimums, and re-aligning cells reduce volume error below a documented threshold? [LANDED 2026-08-08 (ADR 0188): a `computation_interval` timestep knob on `compose_pure2d_deck` + `hecras_flood_2d` (patches the .bNN Computation Interval, validated int+SEC/MIN/HOUR) makes the composable stability subset (timestep + `ds_slope`) sweepable; `stability_sweep.py` reference driver authors once and re-solves the ladder. Live (Blanco canyon, 8075 cells, 15000 cfs): peak depth 10MIN 487.49 (unstable spike) -> 5MIN 244.84 -> 2MIN 116.13 -> 1MIN 116.09 ft, converged at 2MIN (2MIN->1MIN change 0.04 ft, matching the published Bald Eagle ~0.05 ft max-WSE anchor); vol err sub-0.02% throughout. NON-composable subset recipe'd (culvert-invert + mesh re-alignment need Structure/topology authoring on fresh decks). Proof: docs/proof/templates/hecras_flood_2d_stability_sweep_chart.png.]
...
- [LANDED] `2d_diffusion_wave_vs_full_swe_regression` [M] [US] - For the same breach-driven 2D floodplain inundation, how much do peak discharge, arrival time, and max WSE differ between the Diffusion Wave and full Shallow Water Equations solvers at matched mesh/timestep? [LANDED 2026-08-08 (ADR 0188): the `equation_set` knob (ADR 0157) tested in TWO inertial regimes via the `compare_equation_sets.py` reference driver (author once, solve both, per-cell diff). FINDING: the schemes coincide on the peak FOOTPRINT and separate only LOCALLY. Sayers Dam dam-break (~300k cfs, ADR 0174 g09 deck): max WSE byte-identical over 19,597 cells, transient connection flow diverges up to ~14.7k cfs (IC-dominated pool). Blanco canyon (dry, steep, 15000 cfs): wet 6192=6192, max depth 116.13=116.13 ft identical, per-cell WSE separates up to 1.86 ft at ~0.35% of cells (canyon-head/constriction = the localized inertial signature). Diffusion Wave stays the cheaper default; full SWE for local momentum detail, not the envelope. Proofs: hecras_flood_2d_equation_diffmap.png + _equation_regression_chart.png.]
...
- [CAND-M] `pump_station_trigger_and_ramp_control` [S] [US] - For an interior-drainage pump station lifting flow from a 2D floodplain over a levee into a storage area, does the pump correctly stage-trigger on/off and ramp through its startup-transition time rather than stepping instantaneously? [STOP-RECIPE 2026-08-05 (ADR 0157): no pump-station machinery - `compose_pure2d_deck` strips all Structures and the Muncie deck has no pump. Recipe: author a pump-station structure (SA/2D connection endpoints, pump groups x pumps, on/off trigger elevations, startup ramp, efficiency curve) into the plan HDF + geometry, then solve + verify the pumped-outflow ramp vs step. NEW structure-authoring capability, not an S-tier knob.] [ADR 0171: no pump HDF reference exists anywhere in-repo (Muncie has none; x09 pump section empty) - fixture-gated]
...
- [PARTIALLY-SUBSUMED + STOP] `classical_psha_source_typology_sweep` (ADR 0182 triage: `openquake_psha` ALREADY renders TWO source typologies end-to-end - a synthetic `areaSource` over the AOI (job_ini.render_source_model_xml) AND a physics-based `simpleFaultSource` model when real GEM Global Active Faults intersect the AOI (render_fault_source_model_xml, moment-balanced GR MFD, hazard peaking on the trace), selected honestly by resolve_fault_sources + narrated via source_model_kind. So the two typologies a hazard workbench actually needs (distributed area seismicity vs mapped active faults) SHIP, and a run already answers "how do the curves differ" between them. The remaining exotic typologies - point / multipoint / complexFault / characteristicFault / nonparametric - are low near-term value for a US multi-hazard workbench (no strong published US question class demands them over area+simpleFault); STOP the full seven-typology sweep, reopen only if a concrete point/complex-fault question appears.) [M] [US] - Given a fault/area/point source geometry the user draws or fetches, which typology (area/point/multipoint/simple-fault/complex-fault/characteristic-fault/nonparametric) should the deck use and how do hazard curves differ?
...
- [LANDED] `seismic_hazard_disaggregation_by_scenario` (ADR 0182: openquake_disaggregation - local-subprocess disaggregation calculator, distinct from the classical map. Runs calculation_mode=disaggregation at the AOI centroid over a synthetic G-R area source (labelled demo), parses the Mag_Dist_Eps contribution matrix, surfaces the disaggregation site as a point marker, and emits the M-R contribution chart (grouped series: contribution % vs distance, one legended line per magnitude bin - the dock interpreter renders that with a real legend where a rect heatmap degrades to a legend-less scatter). Dominant = the modal magnitude-distance CELL summed over epsilon (the standard controlling earthquake, consistent with the chart). Live: SF Bay AOI, oq 3.25.1 local subprocess ~30s; iml@10%/50yr PGA 0.368 g, dominant M6.25 @ 10 km eps+2.0, contribution-weighted mean M6.11 @ 10 km, 13 non-zero bins, RP 475 yr. Proof docs/proof/templates/openquake_disaggregation_chart.png) [M] [US] - At a given site, what magnitude-distance-epsilon-TRT combination dominates the hazard at 10% probability of exceedance in 50 years?
...
- [LANDED] `stochastic_event_set_ground_motion_fields` (ADR 0182: openquake_event_based - local-subprocess event_based calculator, distinct from classical + scenario. Samples ses_per_logic_tree_path stochastic event sets over a synthetic G-R area source (labelled demo), computes per-rupture GMFs, extracts the event-based hazard MAP (COG primary via the shared classical postprocess) and back-derives the hazard curve from GMFs, then runs a classical single-site deck at the AOI centroid and overlays the two curves - the classic convergence check. Consistency verdict uses the MEDIAN relative PoE difference (robust; the max is dominated by the rare high-intensity tail the catalogue undersamples). Live: SF Bay AOI, oq 3.25.1 local subprocess, ses=300 -> 168 ruptures / 1527 events, 55-site event-based map max PGA 0.425 g, RP 475 yr; median rel PoE diff 18% over 19 shared IMLs (445% at the tail) = "consistent with classical". Proof docs/proof/templates/openquake_event_based_chart.png) [M] [US] - Generate N stochastic event sets and per-rupture ground motion fields for a site, and confirm the event-based hazard curve matches the classical PSHA curve within tolerance.
...
- [SUBSUMED] `scenario_liquefaction_probability_map` (ADR 0182 triage confirms ADR 0164: `openquake_secondary_perils` already maps scenario liquefaction PROBABILITY end-to-end - it rides the scenario GMF (run_scenario_gmf, one rupture, JB2009-correlated field) and applies the published Zhu et al. 2015 global geospatial logistic model per site (PGA from the GMF + magnitude + slope-derived Vs30 [Wald and Allen 2007] + compound-topographic-index wetness proxy from the DEM), publishing a per-site liquefaction-probability COG (primary) + a probability-distribution chart + a SecondaryPerilLayerURI, with honest site-data provenance. That IS this row. The only unbuilt sliver is the alternative model catalogue (HAZUS/Zhu2017/Bozzoni/Todorovic model SELECTION) - a small knob on the existing tool, deferred as low-value vs the shipped Zhu-2015 default. Strike as a standalone build.) [L] [US] - Given one rupture and a site model with Vs30/precipitation/water-table-depth/distance-to-water, map liquefaction occurrence class and probability across the domain.
...
- [FOLDED] `site_model_vs30_amplification_build` (ADR 0182: folded into `openquake_psha` knob `vs30_compare` -- a site-response A/B. The classical map already exposes the site-response LEVER via the `vs30` param (reference_vs30 rides to the deck); the fold adds `vs30_compare`, which runs two single-site classical hazard curves at the AOI centroid on the SAME synthetic demo source differing ONLY in reference Vs30 (the run's rock value vs the comparison soil) and overlays them in ONE legended figure with the amplitude ratio at the target PoE in the caption strip -- the "how does my soil change the hazard" A/B. Local oq subprocess, best-effort/non-fatal (never fails the primary map). NOTE: this is a REFERENCE-Vs30 A/B, not a per-site fetched-Vs30 site_model.csv build; the Wald-Allen slope-derived Vs30 in the 0164 secondary-perils machinery is the raster-Vs30 source to promote for a true spatially-varying site model (deferred). Live: SF Bay AOI, oq 3.25.1; rock 760 vs soft 260 m/s -- soft soil amplifies PoE at every PGA through the meaningful range. Proof docs/proof/templates/openquake_psha_vs30_site_response_chart.png) [M] [US] - Build a per-site Vs30 (and basin-depth where applicable) site model CSV from a fetched raster for an AOI, and confirm it changes hazard-curve amplitude vs the uniform reference_vs30 default.
...
- [CAND-M] `thacker_analytic_swe_validation` [M] [non-US] - How do I validate the SWE+AMR wetting-drying solver's mass/momentum conservation against Thacker's exact analytic solution (radially symmetric parabolic-bowl sloshing), checking gauges on the x-axis vs diagonal for symmetry? [SCOPED 2026-08-08 (ADR 0185): the first FULLY-SYNTHETIC GeoClaw fixture - a paraboloid-bowl deck path (no fetch_topobathy; whole pipeline otherwise assumes a real AOI DEM) + analytic tilted-surface qinit + frictionless closed domain + a closed-form period/amplitude/shoreline comparison chart (model_validation, chart-led). Bigger than the 45-min tag (new synthetic-domain deck branch in setrun_builder + image rebuild). Recipe in ADR 0185; not landed this session (no unverified engine code). Non-US idealized fixture kept as a V&V-doctrine cross-check only, never a hazard target.]
...
- [CAND-M] `okada_single_subfault_dtopo` [M] [US] - How do I generate a dtopo seafloor-deformation file from a single rectangular fault (strike/dip/rake/slip/depth) using the Okada model? [STOP 2026-08-08 (ADR 0185): the single-Okada MECHANISM already exists as the worker's SYNTHETIC default (setrun_builder.render_maketopo_dtopo: one dtopotools.SubFault, Wells-Coppersmith Mw-scaled length/width/slip, create_dtopo_xy -> dtopo.tt3), and the run-up CONSUMES a prescribed dtopo via geoclaw_inundation(tsunami_dtopo_uri=). What is NOT surfaced: a first-class dtopo-AUTHORING tool (user supplies one rectangular fault -> a reusable dtopo layer + Okada deformation chart). Lands as the S-tier sibling of the #18 authoring front. Recipe in ADR 0185.]
...
- [CAND-M] `multi_subfault_dtopo_from_finite_fault_model` [M] [US] - How do I build a dtopo file from a published multi-subfault finite-fault slip model (NOAA SIFT-format subfault CSV) instead of a single uniform-slip fault? [STOP 2026-08-08 (ADR 0185): needs the Okada-dtopo AUTHORING front - a paradigm-B fetch_finite_fault_model (USGS ComCat .fsp/SRCMOD or NOAA SIFT unit sources -> a normalized N-subfault table) + a build_okada_dtopo primitive (dtopotools.Fault of N subfaults -> dtopo.tt3 + a deformation product raster/chart). Real earthquake-source machinery beyond the demo single Okada (consistent with the 0143 okada-1d STOP: disproportionate to fold into a template without its own front). 1964 Alaska/Prince William Sound anchor (real US). Recipe in ADR 0185.]
...
- [CAND-M] `fgout_animation_frames` [M] [US] - How do I generate a uniform-grid animated depth/surface-elevation sequence (MP4/HTML) at fixed output times, decoupled from the AMR patch structure, for a smooth flood/tsunami animation? [SCOPED 2026-08-08 (ADR 0185): the scrubber animation ALREADY exists off the fort.q AMR frame cadence (postprocess rasterizes per-frame COGs); fgout is the SMOOTHER (uniform-grid, interpolated, fixed-interval) frame source. Recipe: an fgout_tools block in setrun_builder (mirrors the fgmax block) + entrypoint stage of the fgout binaries + a postprocess FGoutFrame reader (uniform grid, NO AMR flatten) reusing rasterize_frame_to_grid -> the scrubber group; demo on the Crescent City tsunami deck. Needs an image rebuild. Recipe in ADR 0185; not landed this session. The combined fgmax+fgout (#495) + netcdf-transect (below) fold onto this machinery afterward.]
...
- [LANDED] `best_track_to_geoclaw_storm_file` [M] [US] - How do I convert a real NHC/ATCF best-track advisory (or HURDAT2 archive record) for a named US hurricane into the GeoClaw storm-format file the solver reads?
  ...
  LANDED (ADR 0168): setrun_builder.render_storm_file writes the GeoClaw storm file DIRECTLY (byte-format-identical to Storm.write('geoclaw'), no pandas Storm import) from a StormTrackPoint list; the published Ike ATCF (bal092008) track is hardcoded in the smoke fixture (published-first). A fetch_storm_tracks (IBTrACS/ATCF) auto-source wiring is a follow-up fold.
- [LANDED] `parametric_holland_wind_surge_ike` [M] [US] - How do I run a parametric Holland-1980 wind/pressure storm-surge simulation for a real, published US hurricane (Ike, 2008) and check it against the example's committed regression output?
  ...
  LANDED (ADR 0168): surge scenario now emits wind_forcing+pressure_forcing+holland80+storm_file (num_aux=7 storm aux layout, pre-landfall run window). Ike-anchor smoke through the image: peak coastal surge 5.4 m right-of-track near Bolivar (observed Ike ~4.5-6 m, correct right-of-track side; idealized planar shelf -> qualitative anchor). geoclaw_storm_surge template.
...
- [LANDED] `wind_drag_law_selection` [M] [US] - How do I choose between no wind drag / Garratt / Powell(2006) drag laws for the wind-stress term, and how much does the choice change peak surge? [LANDED 2026-08-07 (ADR 0168, un-STOPs the ADR 0143/0155 verdict): the parametric-Holland wind module is now real, so drag_law is a LIVE knob (none|garratt|powell -> 0|1|2). Drag-law A/B smoke through the image (identical setup, only the law differs): Garratt vs Powell coastal surge 5.10 vs 4.95 m, gauge peak eta 1.99 vs 2.29 m (delta -0.30 m) - the knob MEASURABLY moves the surge (not a no-op). Superseded prior note:] [RETAG CAND-S -> CAND-M 2026-08-05 (ADR 0155, executing the ADR 0143 STOP verdict): the whole storm-surge/wind MODULE is absent from the surfaced deck, so drag_law is INERT and this is NOT an S-tier one-knob fold. rundata.surge_data EXISTS in the clawpack API (drag_law, wind_forcing, storm_specification_type, storm_file) and the surge Fortran is compiled in (geoclaw/src/2d/shallow/surge), BUT the TRID3NT "surge" scenario is a v0.1 sea-level-offset stub: NO surge_data setrun block, NO wind_forcing, NO storm file. drag_law scales the WIND-STRESS term, and with wind_forcing=False + no storm it changes NOTHING (you cannot demonstrate a peak-surge delta without wind). Landing drag_law is INSEPARABLE from landing the parametric-Holland wind module (this is best_track_to_geoclaw_storm_file + parametric_holland_wind_surge_ike territory). Recipe: (1) ATCF/HURDAT best-track fetcher -> geoclaw storm file (clawpack.geoclaw.surge.storm.Storm.write); (2) setrun_builder emit surge_data (storm_specification_type='holland80', storm_file, wind_forcing=True, pressure_forcing=True, drag_law knob); (3) entrypoint stage the storm file; (4) validate against the committed Ike regression_data. Then drag_law becomes a real 0/1/2 sweep. Deferred as its own storm-surge wave.]
...
- [LANDED] `initial_attack_containment_probability` [M] [US] - Given fire size and head-fire fireline intensity at the moment of first response, what is the Hirsch-model probability of containment, and how sensitive is it to INITIAL_ATTACK_TIME (detection+report+travel delay)? [LANDED 2026-08-08 (ADR 0190 row 2): NEW registered tool elmfire_initial_attack_containment_probability (closed form, NO engine run; registry +1 -> 235). Uses the EXACT published elmfire.io POC formula POC=E/(1+E), ln(E)=4.6835-0.7043*A-0.00041*I-0.000052*A*I (A ha, I kW/m) - byte-verified vs the formula in a test. Attack-delay sensitivity via Byram ROS -> elliptical fire growth during the get-away time -> POC(delay): critical delays (POC=0.5) 1000 kW/m never / 2500 53 min / 4000 29 min / 6000 15 min. Corpus + model-free retrieval top-8 (5/5); primary category model_validation. Proofs: _poc_delay_chart.png + _poc_surface.png. Showcase 01KZGYTV8WRG1J7BF1YG7RYR6Y (1 chart).]
...
- [LANDED] `nonstationary_time_marching_storm_evolution` [M] [US] - How does the nearshore wave field evolve minute-by-minute through a 24-48hr storm rather than a single snapshot? [LANDED 2026-08-08 (ADR 0190 row 3): swan_wave_field mode=nonstationary made REAL + a TIME-VARYING storm boundary added (storm_peak_hs_m + storm_peak_hour knobs -> a build-peak-decay offshore-Hs TPAR series; 0 new tools). Fixed THREE latent nonstationary bugs (the path had never run live): (a) COMPUTE tbegc/tendc were malformed bare-seconds -> now ISO YYYYMMDD.HHMMSS (multi-day valid); (b) the BLOCK had no OUTPUT clause -> no per-frame dumps; (c) the default higher-order propagation aborts on CFL for a time-march -> added PROP BSBT. Worker image rebuilt (bffb3ac6cc02, provenance clean). Live 36 h storm on the Apalachee Bay FL shelf: 19 time-stamped Hs frames build 1.0 -> 6.0 m (peak 18 h) -> 1.0 m, max_hs 6.02 m (matches the forcing), through the native SWAN solver. Proofs: _peak_hs.png + _frames.png filmstrip (pinned times + shared scale). Showcase 01KZGX66T41AGKKQPRF53D7VVR (20 layers = peak + 19 frames persisted, feeding the scrubber). 0191 REVISION (spot-check fix): the peak-Hs proof AOI moved off the Big Bend box (-84.30..-83.90) whose staged CUDEM tile carries a large elevation==0.0 interior void (read as dry, depth<DEPMIN -> ~half the AOI meshed dry, wave field only in the connected wet strip) to the void-free Mexico Beach/Tyndall shelf (-85.55..-85.40, ~86% wet, the showcase _APALACHEE box). The deck CGRID/INPGRID extent was already correct (refuted); this is a bathymetry-DATA fix. Re-run 01KZH582SZ8M1M9AM7PYPVQTES fills the AOI (max_hs 6.02 m).]
...
- [LANDED ADR 0167] `advanced_package_mover_routing_uzf_sfr_lak_wel` [M] [US] - Can MVR correctly transfer rejected UZF infiltration and LAK/WEL discharge into SFR reaches within one coupled timestep? -> `modflow_package_validation` case `mvr_routing`: UZF rejected infiltration (52482.5) + DRN discharge (3287.3) routed to SFR, received 55769.9 m3/d, conservation delta 0.0 (DRN stands in for the LAK/WEL discharge provider; mover invariant identical).
...
- [LANDED ADR 0167] SFR stream depletion by pumping (Glover V&V) -> `modflow_package_validation` case `sfr_stream_depletion`: an SFR-coupled well vs the Glover & Balmer (1954) transient erfc analytical (superposition; max rel error 4.3% over the resolved window). The georeferenced SFR stream-depletion PRODUCT (gaining/losing reaches, river-capture fraction) already ships on `modflow_sustainable_yield`'s stream_depletion branch; this case is the closed-form gate above it.
...
- [STOP ADR 0152] `weir_discharge_coefficient_tuning` [S] [US] - Should the weir discharge coefficient (par1/cd, default 0.6) be adjusted per structure type (sharp-crested vs broad-crested levee)? [ADR 0174 EPILOGUE: on the LIVE Sayers connection the coefficient IS monotonic (262/406/524 cfs for Cw 2.0/3.1/4.0) - the 0171 inert verdict was the uncoupled Muncie weir; capability proven at reference-builder level, template wiring = the flagged next job]
...
- [LANDED] `swmm_rdii_rtk_unit_hydrograph` [M] [US] - For a sewershed with known inflow/infiltration behavior, how much rainfall-derived flow enters the pipe network at a manhole, and how does it compare in magnitude and timing to direct subcatchment runoff at the same point? [LANDED 2026-08-08 (ADR 0190 row 4): NEW registered tool swmm_rdii_rtk_unit_hydrograph (registry +1 -> 235; host-side pyswmm, no worker image). RTK triangular unit-hydrograph RDII closed form (UH area = R*rain*area) convolved with the storm, split vs direct runoff. TWO acceptance checks: the RTK volume identity (ratio 1.00006) + a NATIVE SWMM 5 cross-check ([HYDROGRAPHS]/[RDII] deck solved through swmm5_run, closed-form peak matches to <1%). EPA Table 7-1 replication: the EPA SWMM5 Ch.7 worked example (10-ac sewershed, sum R=0.36, the published hourly rainfall) - closed-form peak 1.06 cfs vs the published Fig 7-10 peak 1.021 cfs (~4%; the exact per-UH R/T/K live only in Fig 7-8, not the text). Corpus + retrieval top-8 (5/5); primary category simulation_modeling. Proofs: _rdii_vs_runoff.png (closed form + native SWMM + published Fig 7-10 X-marks) + _unit_hydrographs.png. Showcase 01KZGYWHF98YZCV5KSG0YRQVPM (EPA example, 1 chart).]
...
- [LANDED] `rainfall_evaporation_forcing` [M] [US] - How does distributed rainfall (or evaporation) over the mesh change inundation depth and timing during a storm, independent of the upstream inflow hydrograph? [LANDED 2026-08-08 (ADR 0190 row 1): folded as distributed on-mesh RAIN knobs on telemac_river_dye (rainfall_mm_per_day + evaporation_mm_per_day + rainfall_gridmet_window; 0 new tools). author_deck emits RAIN OR EVAPORATION = YES + IN MM PER DAY (signed: + rain / - evap) + VALUES OF TRACERS IN THE RAIN (required with tracers), keywords vs telemac2d.dico v9.0; unset leaves the deck BYTE-IDENTICAL. gridMET pr fetcher reused for a REAL storm total (Hurricane Harvey Buffalo Bayou Houston 158.5 mm/day, live-verified). Worker image rebuilt (41c72ebeaa85, provenance clean). With-rain vs without live on the Snake River (1500 mm/day, 90 min): mean depth +8.6 mm, max +21 mm, monotone accumulation - the distributed source raises inundation independent of the identical inflow BC. Proofs: telemac_river_dye_rainfall_diffmap.png (depth-rise fingerprint over Esri + mesh) + _timing_chart.png. Showcase 01KZGT5MQZVB397XPJ8A42NQWV (Eel River nr Scotia, 150 mm/day, 2 layers + 1 chart). 0191 REVISION (spot-check fix): the diffmap's "messed up mesh" was a PROOF-RENDER artifact, NOT an offset-centerline domain (hypothesis REFUTED) -- the TELEMAC-2D mesh follows the REAL NHDPlus flowline, but the proof triangulated the scattered nodes with an unconstrained Delaunay that fanned across every river bend. read_selafin now returns the real element connectivity (IKLE) and the proof triangulates real elements, so the mesh traces the true channel. Re-run base 01KZH53WRW8KB4NYSAK7VDG4NT / rain 01KZH561BN64PFA5HWZ8EYEJPM (+8.6 mm mean / +21 mm max).]
- [LANDED] `telemac_rain_on_grid` [L] [US] - What discharge/inundation does THIS rainfall produce over THIS catchment (hydrodynamic rainfall-runoff, no inflow hydrograph)? [LANDED 2026-08-08 (ADR 0196 C1-C4): the registered `telemac_rain_on_grid` template (engine=telemac, tier=template) lands on the 0195 foundation. WORKER (C1): `services/workers/telemac/rog_build.py` + `mode=rain_on_grid` dispatch in entrypoint.py -- consumes the staged watershed SELAFIN (mesh_acquisition), rebuilds the boundary, marks the pour-point ring nodes a FREE exit (KSORT=4, no fixed stage) with walls elsewhere (KLOG=2), writes the per-node CN2 scatter (FORMATTED DATA FILE 2 read by HYDROMAP) + distributed Manning (FRICTION DATA FILE zone laws + ZONES FILE node->zone, fully native per-NLCD roughness) + the RoG deck (RAIN OR EVAPORATION + RAINFALL-RUNOFF MODEL=1 + ANTECEDENT MOISTURE CONDITIONS + OPTION FOR INITIAL ABSTRACTION RATIO), solves, and extracts the OUTLET HYDROGRAPH (unit discharge integrated across the outlet edges per frame), max-depth/velocity fields, and the WATER VOLUME continuity from the listing. PARSER (C2): ReachConfig RoG fields + bump telemac-reach-2 -> telemac-reach-3 (unknown-field rejection test); image REBUILT (id 4835445811db, `docker history` GRACE-2 refs = 0), parser v3 accept/reject + a solve verified THROUGH the baked image. Offline-first: synthetic tilted-plane catchment reaches CORRECT END OF RUN with continuity -4e-16 and a monotone AMC knob (dry 1.26 / normal 4.99 / wet 8.28 m3/s); 16 worker unit tests + 6 template registration tests green; retrieve_visible_tools surfaces it. LIVE COWEETA PROOF (C4): the delineated Coweeta Creek NC catchment (pour point -83.40402 35.05746, 28.72 km2, 4854 nodes / 9521 tris, 3DEP bed, NLCD-distributed CN 75-90 + Manning 0.05-0.20) solved THROUGH the rebuilt image, AMC II (peak 45.5 m3/s, runoff 162x10^3 m3, maxH 6.95 m, continuity 1.3e-15) vs AMC I dry (peak 6.1 m3/s, runoff 9.6x10^3 m3, maxH 1.21 m) -- the CN antecedent-moisture knob demo. Proofs (EPSG:3857): docs/proof/templates/telemac_rain_on_grid.png (peak-depth field self-organizing into the real dendritic drainage over ESRI + red catchment), _chart.png (dock hydrograph, AMC II vs AMC I overlay + NSE/R2 slot), _mesh.png (TIN wireframe). Constant design storm drives the native SCS-CN path (installed v9.0.0 hardcodes RAINDEF=1; a true time-varying MRMS hyetograph needs a recompiled user_rain.f -- documented in rog_build). Showcase: "Coweeta Creek watershed North Carolina" (seed_showcase_cases.py); verified !run line below. PRIOR FOUNDATION (ADR 0195, 2026-08-08): distinct question class from rainfall_evaporation_forcing (that is a knob ON an inflow-driven reach; this is a watershed-first RoG template with NO inflow BC). FOUNDATION LANDED THIS WAVE (offline, no image): (1) shared paper-exact validation primitives nash_sutcliffe_efficiency (eq 14) + pearson_r2 (eq 13) in compute_skill_metrics + build_hydrograph_overlay_chart in charts_common (12 offline tests); (2) SCS-CN infiltration preprocessing workflows/telemac/rain_on_grid/cn_infiltration.py - eq 7-8 rainfall-excess, Huang-2006 steep-slope (eq 9, the ACTUAL rational form TELEMAC uses), AMC I/II/III conversions byte-parity with runoff_scs_cn.f, NLCD-class -> CN/Manning Table-1 analog (15 offline tests). CN PATH VERDICT: TELEMAC v9.0.0 HAS the native SCS-CN runoff model (runoff_scs_cn.f, Ligier 2016; RAINFALL-RUNOFF MODEL=1 + ANTECEDENT MOISTURE CONDITIONS + OPTION FOR INITIAL ABSTRACTION RATIO are native keywords, CN2 read per-node from FORMATTED DATA FILE 2 via HYDROMAP) BUT two hardcoded limits in the installed build: steep-slope correction compiled OFF (STEEPSLOPECOR=.FALSE., not a keyword) and rain is RAINDEF=1 constant-intensity only -> native path for a design-storm block with distributed CN (steep-slope baked into the CN field in preprocessing), rainfall-excess preprocessing path (RUNOFF MODEL=0 + time-varying net rain) for a real MRMS hyetograph. PRECIP VERDICT: fetch_mrms_qpe hourly (1h, CONUS ~1km gauge-corrected) is the flash-flood product (gridMET daily too coarse; ERA5 27km global); MRMS S3 archive ~2020-10+. DEFERRED to the build wave: the registered template body + watershed-mesh promotion (0193 mesher) + worker deck/entrypoint changes + parser telemac-reach-3 + image rebuild + live Coweeta proof (hours-class run, not session-completable). Replication recon: docs/research/rog-replication-methodology.md.]
  replication: [EXECUTED 2026-08-09, ADR 0204] the Godara-2024 replication is RUN computed-vs-observed on the re-cut Ball Creek weir #9 catchment (7.24 km2 south fork; NHD/GNIS-confirmed pour point -83.43131 35.05701 -> fork outlet -83.42971 35.05921). Forcing = AORC hyetograph (ADR 0203 fetch_aorc_precip); observed = fetch_lter_records (Ball Creek weir #9). RESULT = honest PARTIAL replication: Dec 2015 calibration (CN2 55, AMC II, Manning x1.0) reproduces the flash-flood PEAK (8.46 vs 8.60 m3/s, -1.7%; peak-aligned R2 0.675) but raw hourly NSE -1.41 lands BELOW the paper's 0.70-0.90 (screening-grade). Gap fully attributable to the installed stack: constant rain (RAINDEF=1) -> ~11 h peak-timing lag (compounded by AORC precip-peak lagging the gauge peak ~4 h); no subsurface return flow -> missing recession tail (event volume -52%); 30-200 m mesh cannot convey thin low-intensity overland flow (Feb 2018 validation ponds, peak ~0 at the transferred CN; reproducible standalone only at a much higher CN -- static SCS-CN does NOT transfer one CN across antecedent-saturation regimes); the multi-peak control shows the expected structural failure (single hump, misses the 2nd peak, under-estimates inter-peak flow). Continuity O(1e-15) everywhere (numerics exact; the gap is forcing/mesh fidelity). WORKER (image rebuilt + smoke): author_rog_deck emits DURATION OF RAIN OR EVAPORATION IN HOURS (RAIN_HDUR -> rain stops -> recession); classify_outlet contiguous-arc fix (latent FRONT2 abort). Drivers scripts/sandbox/replication/rog_ballcreek_*.py; proofs docs/proof/templates/telemac_rain_on_grid_{replication,multipeak,calibration}_chart.png. Original STOP (ADR 0202, data block) + unblock (ADR 0203 AORC+LTER fetchers). RE-GRADED 2026-08-09 (ADR 0206): the RAINDEF=1 constant-rain limit REMOVED via a per-case FORTRAN FILE flipping the installed runoff_scs_cn.f to RAINDEF=3 (its own block-hyetograph branch) -- native SCS-CN now runs per-timestep on the REAL AORC hourly hyetograph, NO engine rebuild (worker image rebuilt ff38f5cfe73a for the authoring code; parser telemac-reach-4). Re-calibrated CN 53. Dec 2015 aligned NSE 0.04 -> 0.51 (SHAPE) and vol err -52% -> -19% (VOLUME) materially improve; raw NSE stays -1.41 (a ~11 h peak offset dominates a point-wise NSE) because the residual lag is now forcing/mesh-bound (AORC precip peaks h17 vs gauge h13 = ~4 h; coarse 30-200 m mesh routing ~6.5 h), not the rain representation. Multi-peak SECOND peak now REPRODUCED (real 2nd storm) -- reverses the ADR 0204 "2nd peak missing" -- but over-peaks +116% (static SCS-CN abstraction exhausts, no soil store). Discriminating physics: rog_twopulse_smoke.py through the image shows a two-pulse hyetograph = BIMODAL outlet (2 peaks) vs same-volume constant = UNIMODAL (1 peak), mass exact. Drivers scripts/sandbox/{telemac/rog_twopulse_smoke.py,replication/rog_ballcreek_hyeto.py}; proofs regenerated in place. Template surface: select_runoff_path routes time-varying -> native_hyetograph, composer mrms_window drives the AORC hyetograph.
  full-results publishing (2026-08-09, ADR 0208, NATE's scrubbing ask): alongside the peak-depth COG the template now publishes the full-results SELAFIN (`r2d_rog.slf` -- ALL frames, ALL variables) as a `layer_type="mesh"` Case layer ("Model results (time series): <reach>") via the ADR 0200 `publish_input_layer` mesh seam, stamped in the mesh's own UTM CRS, riding the runs-bucket object (no re-upload). QGIS/MDAL animates it natively with the temporal controller -- no per-frame COGs, no plugin change. Payload ~1.5 MB for a 2 h run, grows ~linearly with frames x nodes (docstring-stated). LIVE: run 01KZKMG2D69HM2NBD8XJ31DMNJ (Coweeta, fresh delineate) emitted BOTH the depth COG (telemac-wse-max) AND the mesh layer (rog-results-01KZKMG..., EPSG:32617, s3://trid3nt-runs/.../r2d_rog.slf, 1.47 MB).

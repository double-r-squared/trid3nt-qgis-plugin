...
**TOTALS: 126 modules mined, 334 candidate template rows, of which 98 are unsigned CAND-S (pre-authorized easy tier).** (ADR 0156 closed the 4 SCHISM easy-tier rows: transport_scheme_accuracy_comparison LANDED + generic_passive_tracer_mass_conservation FOLDED into the one `schism_transport_validation` template; tidal_constituent_extraction_inrun STOP-RECIPE (iharind needs a USE_HA build); harmonic_analysis_ccrm_wiki_method_notes DOC.)

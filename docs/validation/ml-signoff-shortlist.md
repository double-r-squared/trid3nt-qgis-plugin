...
| 9 | GEOCLAW `thacker_analytic_swe_validation` | Mass/momentum conservation of the SWE+AMR wet-dry solver vs Thacker's exact bowl solution? | FEATURE-M ~45 min (existing 2D SWE solver; idealized V&V) | Thacker analytic (non-US, V&V) [SCOPED ADR 0185: first fully-synthetic GeoClaw fixture (paraboloid-bowl deck + closed-form comparison); bigger than the 45-min tag - needs a synthetic-domain deck path (no DEM fetch) + image rebuild. Recipe in ADR 0185; not landed this session (no unverified engine code).] |
| 10 | GEOCLAW `fgout_animation_frames` | Uniform-grid animated depth/surface sequence at fixed times (smooth flood/tsunami animation)? | FEATURE-M ~60 min (fgout postprocess; 0155 already folded fgmax mask) - high UI value (time-series animation demo) | clawpack fgout examples [SCOPED ADR 0185: fgout_tools uniform-grid frame source feeding the existing scrubber (smoother than the fort.q AMR cadence). Recipe in ADR 0185; needs setrun fgout block + entrypoint stage + postprocess + image rebuild + Crescent City smoke; not landed this session.] |
...
|---|---|---|---|---|
| 18 | GEOCLAW `multi_subfault_dtopo_from_finite_fault_model` | dtopo from a published multi-subfault finite-fault model | front ~2-3 h + S-batch | Okada-dtopo authoring; **1964 Alaska/Prince William Sound, NOAA SIFT (real US)** [STOP ADR 0185: the CONSUMPTION path exists (`geoclaw_inundation(tsunami_dtopo_uri=)` -> `dtopo_file`), but there is NO multi-subfault AUTHORING from a published finite-fault model (USGS .fsp/SRCMOD, NOAA SIFT). Needs the Okada-dtopo authoring front (`fetch_finite_fault_model` + `build_okada_dtopo`). Recipe in ADR 0185.] |
| 19 | GEOCLAW `okada_single_subfault_dtopo` | dtopo from a single rectangular fault (Okada) | S-batch after #18 front | same Okada-dtopo front [STOP ADR 0185: the single-Okada MECHANISM is the worker's synthetic default (`render_maketopo_dtopo`, Wells-Coppersmith Mw-scaled), but is not surfaced as a first-class dtopo-authoring tool. Lands as the S-tier sibling of the #18 front. Recipe in ADR 0185.] |
| 20 | GEOCLAW `parametric_holland_wind_surge_ike` | Parametric-Holland wind storm-surge vs committed regression output | front ~3-4 h + S-batch | GeoClaw storm-surge front; **Hurricane Ike 2008, committed regression_data** [LANDED ADR 0168 (reconciled ADR 0185): `geoclaw_storm_surge` on the published NHC ATCF bal092008 Ike track - live smoke peak surge 5.4 m right-of-track at Bolivar, in the observed 4.5-6 m class; drag-law A/B met the 0143 non-noop gate. Board line 530 already [LANDED]; this shortlist row was stale.] |

...
| `code-graph/` | `scripts/instruments/code_graph.py` | the code atlas: `SUMMARY.md`, `orphans.md` (modules unreachable from every declared root) and `dead_symbols.md` (vulture at min-confidence 80), over the `graph.json` the same run writes for machines to read. |

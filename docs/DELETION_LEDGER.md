...
- MEASURED NET for this slice, base `0a31eb42` -> HEAD over the 20 files it touched or replaced: 9,235 -> 8,101 = **-1,134** server Python, while the tool surface went 5 templates -> 8. The spec's -7,000 to -8,500 is the WAVE's number and Stage 3 carries the rest of it (the two worker deck builders 2,178, `authoring/{agitation,stratified,open_water}` 1,363, the plan language ~2,600).

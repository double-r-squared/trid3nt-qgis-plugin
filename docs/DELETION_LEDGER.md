...
| HOOK-RATCHET: chained id/detail resolution mode (name->id or list->per-item-detail) | _router modes | MANDATORY-REVIEW (rule 4, 4x: gbif, inaturalist, nws_alerts_conus, nws_river_forecast) - promote a declarative resolve-then-fetch / bounded-enrichment mode; on promotion its per-source hooks would DELETE | QUEUED | ADR 0061 |
| HOOK-RATCHET: offset paging ($skip/$top, stop-on-short-page) sibling of totalPages paging | _router http_source | 2x flag (openfema; likely OData feeds) - add as a paging mode variant | QUEUED | ADR 0061 |
| HOOK-RATCHET: attribute-feed <- boundary-service FIPS join (openfema<-TIGERweb) | _router transforms/join.py | reuse existing join transform for the attribute->geometry FIPS join | QUEUED | ADR 0061 |
| HOOK-RATCHET: bulk-file-behind-an-index (regex-directory->newest->GET + CSV->point) | _router modes | distinct bulk-file mode (storm_events; likely NCEI archives) | QUEUED | ADR 0061 |

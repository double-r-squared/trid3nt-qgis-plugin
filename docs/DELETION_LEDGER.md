...
|---|---|---|---|---|
| `contracts/trid3nt_contracts/telemac_contracts.py` whole - the seven style rows `TELEMAC_DYE_STYLE`, `TELEMAC_SEDIMENT_CONCENTRATION_STYLE`, `TELEMAC_BED_EVOLUTION_STYLE`, `TELEMAC_DO_STYLE`, `TELEMAC_AGITATION_STYLE`, `TELEMAC3D_STRATIFICATION_STYLE`, `TELEMAC_MAX_DEPTH_STYLE` | `contracts/trid3nt_contracts/`, `trid3nt_server/workflows/telemac/templates/` | MOVED, not lost: a style row is a VALUE of the template that draws it, like its captions and its answer, so each row is declared beside the `OUTPUTS` entry that references it - `DYE_STYLE` in `river_dye`, `OIL_STYLE` in `river_oil_spill`, `MARKER_STYLE` + `BED_EVOLUTION_STYLE` in `river_scour`, `SEDIMENT_STYLE` + `BED_EVOLUTION_STYLE` in `river_sediment_plume`, `AGITATION_STYLE` in `agitation`, `STRATIFICATION_STYLE` in `stratified_flow`, `OXYGEN_STYLE` in `do_sag`, `MAX_DEPTH_STYLE` in `rain_on_grid`. The three templates that were reading the dye row now name their own quantity (`Dissolved oil concentration`, `Marker concentration`) instead of borrowing a label that was never theirs. The module held nothing else - its `LayerURI` subclasses died with the products - so the file goes and `contracts` carries no engine's display values. No schema named it: `export_schemas` never exported it. | DELETED(residue wave, slice 3) | `docs/specs/render-and-engine.html` section 1 |
| `docs/specs/processing-redundancy-cull-proposal.md` | `docs/specs/` | A FROZEN proposal: it argues a cull that has already happened and names the tools as if they were still in the tree, when they are in the scope attic. A proposal that has been acted on is history, and git is the archive. | DELETED(residue wave, slice 3) | `docs/IDEAS.md` RESIDUE WAVE GO (7) |
| field raster painting whole - `workflows/publishing/raster.py` (`rasterize_elements`, the element fill and the node halo, `grid_shape`, `TARGET_GROUND_RES_M`), `workflows/publishing/cog.py` (`write_cog_4326_from_grid`, `reproject_cog_file_to_4326`, `upload_cog`, `cog_bbox_4326`, `safe_unlink`, `CogIoError`, `_read_crs_from_dataset`, `NODATA_DEPTH_M`), and `publish.py`'s `_layer` / `_value_range` | `trid3nt_server/workflows/publishing/`, `tests/publishing/test_raster.py`, `tests/publishing/test_cog.py` | REPLACED: render accepts a FORMAT SET and a TELEMAC field is the mesh the run solved on with a dataset group selected, so no COG of a field is painted. The whole path had one product consumer - the field painter this wave deletes - and `_read_crs_from_dataset`, `reproject_cog_file_to_4326`, `cog_bbox_4326` and `NODATA_DEPTH_M` had none at all. The grid-to-COG writer is a CAPABILITY an engine outside the format set would need, so it leaves INTACT. ATTIC: `~/Documents/trid3nt-attic/field-raster-painting/` with a MANIFEST naming the re-mount condition (an engine whose transform module produces a nodal field on an unstructured mesh). The COG writer that stays is render's one writer: `render/publish.py` (`publish_layer` - write, register, notify) over `render/cog.py` (`translate_to_cog`), which every published raster already crossed. Audit row 11's EPSG:3857 CRS guess left with it, emptying the parked-substitution register in `tests/fetchers/test_fallback_sweep_guard.py`. | ATTIC(render wave, leg B) | `docs/specs/render-and-engine.html` section 1 |
| `outputs.json` whole - `workflows/publishing/manifest.py` (`write_outputs_manifest`), `workflows/publishing/animation.py` (`publish_results_mesh_via_seam`, `_build_entries`, `_write_and_read_mesh_layers`, `RESULTS_MESH_QUANTITY`), `contracts/trid3nt_contracts/outputs_manifest.py` with `OUTPUTS_MANIFEST_BASENAME` / `OutputEntry` / `OutputsManifest` / `append_entries` / `build_entry` / `parse_outputs_manifest` and their `trid3nt_contracts.__init__` exports, `outputs_seam.build_layers_from_outputs` / `read_outputs_manifest` / `entry_style` / `PublishedFrame` / `SeamPublishResult`, `docs/design/outputs-manifest-schema.md` | `trid3nt_server/workflows/publishing/`, `contracts/`, `trid3nt_server/render/outputs_seam.py`, `tests/render/test_outputs_manifest_schema.py` | REPLACED: a SECOND registry for what a run produced. Its only writer was the animation publication, which wrote the manifest and immediately read it back to build one mesh layer; the results-mesh publication is render's mesh kind now and needs no round trip. Its two readers read the RUN RECORD instead - the journal line's `outputs` field, written by `render.formats.publish` as the layers are emitted through `journal.journal_outputs`. `outputs_seam.run_outputs(run_id)` and `list_run_frames` read that field and nothing else, pinned by `tests/render/test_outputs_seam.py` and `tests/solver/test_list_run_frames.py` (both fail if the reader touches the object store). The schema and the writer are a CAPABILITY a worker-side producer would re-enter through, so they leave INTACT beside the painter. `list_run_frames` no longer lists per-frame COGs (none exist): it lists the run's published layers by uri, with its docstring, corpus and result shape rewritten to say so. | ATTIC(render wave, leg B) | `docs/specs/render-and-engine.html` section 1 |
| the thirteen derive reimplementations of QGIS and GDAL algorithms - `clip_raster_to_polygon`, `compute_aspect`, `compute_blended_composite`, `compute_building_density`, `compute_change_detection`, `compute_colored_relief`, `compute_contours`, `compute_hillshade`, `compute_impervious_surface`, `compute_ndvi`, `compute_slope`, `enhance_satellite_image`, `extract_landcover_class` - with their corpus and tests | `trid3nt_server/tools/derive/<tool>/`, `tests/derive/test_<tool>.py`, `tests/derive/test_job0305_memoryfile_lifetime.py` | A CAPABILITY set QGIS ships: every one is a Processing algorithm the user's own session runs through `run_qgis_algorithm`, and the ad hoc rest is a `run_pyqgis` snippet behind the approval card. They leave INTACT as ONE set. ATTIC: `~/Documents/trid3nt-scope-attic/extensions/derive_reimplementations/` with a MANIFEST naming the thirteen registration lines, the three offload rows, the uri-registry rows, the prose that named them, the re-pointed tests and the expected registry size after re-mount (+13; the registry went 162 -> 150 with this wave's two session tools in). | SCOPE-ATTIC(render wave, slice A1) | `docs/specs/render-and-engine.html` sections 3, 4 |
| the container sandbox and its tool - `trid3nt_server/sandbox/` (`box.py`, `driver.py`), `tools/meta/code_exec_tool/` (`code_exec_request`), `contracts/trid3nt_contracts/sandbox_contracts.py` with `CodeExecResultPayload` and the `code-exec-result` envelope, `tests/sandbox/`, the result emission in `server/dispatch/results.py` and `server/turn/stream.py`, the compact-summary strip in `adapters/adapter.py`, the plugin's result handling | `trid3nt_server/sandbox/`, `trid3nt_server/tools/meta/code_exec_tool/`, `contracts/`, `tests/sandbox/`, `plugin/` | REPLACED: the session is the one code path. `run_pyqgis` executes the approved snippet in the user's QGIS session through the plugin wire behind the SAME code-exec approval card (`CodeExecRequestPayload` stays, without `layer_refs`); the gate, its timeout and its typed errors stay and front the new tool. ATTIC: `~/Documents/trid3nt-attic/code_exec_sandbox/` with a MANIFEST naming the registration line, the gate wiring the tree keeps and the re-mount cost. | ATTIC(render wave, slice A1) | `docs/specs/render-and-engine.html` sections 3, 4 |
| `telemac/solving/` whole: `solve.solve_reach`, `class Solve` (`Solve.telemac`), `read_run_metrics`, `_MIN_WAIT_S` / `_WAIT_HEADROOM` / `_CASE_TIMEOUT_S`, `_ALLOWED_COMPUTE`, `_run_start_iso`, `solving/run_telemac.py`, `solving/__init__.py`; and out of `solver.py` the `COMPUTE_CLASS_ALIAS` map, `set_s3_client` / `set_runs_bucket` / `_get_s3_client` / `_get_runs_bucket` / `_get_local_runs_bucket` / `_split_object_uri`, plus `download_result`'s `error_code` parameter | `trid3nt_server/workflows/telemac/solving/`, `trid3nt_server/workflows/solver/solver.py` | one executor over one file per engine. `solve_reach` and `Solve` are the plan era's twin of `solve_case`: both dispatched the same solver with the same cards, differing only in where the UTM zone was read (the worker's metrics, which carry `server_facts` VERBATIM, versus the assembler's own record - the same number) and in the wait. `solve_case`'s body is the ONE run path, in `telemac/engine.py`, and all eight templates dispatch through it; the wait is `engine._wait_seconds`, the estimate off the sheet's own DURATION and TIME STEP that `solve_reach` already used, with the day-long ceiling kept for a sheet that states no horizon. `dispatch_and_wait`, `download_result` and the compute-class coercion know no engine and are the executor's (`solver.py`, `solver/compute_class.py`); `_ALLOWED_COMPUTE` was a second copy of `COMPUTE_CLASS_ALIAS`'s key set. The object store is `trid3nt_server/storage.py`, reached module-qualified (`storage.client()`, `storage.runs_bucket()`) by its thirty importers rather than by importing a private name out of the executor. WIRE: `TELEMAC_COMPUTE_CLASS_UNKNOWN` -> `COMPUTE_CLASS_UNKNOWN`, `TELEMAC_OUTPUT_MISSING` (download miss) -> `RUN_OUTPUT_MISSING`, and a missing `TRID3NT_RUNS_BUCKET` raises `StorageError` (`STORAGE_UNAVAILABLE`) rather than `SolverDispatchError`, which now derives from it - no matcher on any of the three in the plugin, the contracts or the product (grep 0). | DELETED(A3) | `docs/specs/render-and-engine.html` section 2 |
...
| `search_spatial_functions` (the tool, its module, its 8-phrasing corpus, its registration in `tools/__init__.py`, the vendored `tools/duckdb_spatial_functions.json` it answers from, that file's `pyproject.toml` package-data row, its `tools/README.md` file row, the `search/` folder row and `tools/search/__init__.py` docstring that named it, `search_tools`' DuckDB routing clause, and `tool_retrieval.py`'s enumeration-versus-ranking comment naming the spatial functions) | `trid3nt_server/tools/search/search_spatial_functions/`, `trid3nt_server/tools/duckdb_spatial_functions.json`, `tools/__init__.py`, `tools/README.md`, `tools/search/__init__.py`, `tools/search/search_tools/search_tools.py`, `tools/search/tool_retrieval.py`, `pyproject.toml`, `tests/search/test_search_spatial_functions.py` | A CAPABILITY, and it leaves INTACT - following `spatial_query`, which the row above left standing pending NATE's call. Every phrasing in its corpus and every line of its docstring routes a caller toward composing `spatial_query` SQL, so with no SQL surface in the tree the lookup answers a question the product no longer takes. ATTIC: `~/Documents/trid3nt-scope-attic/extensions/spatial_query/`, the same package, its MANIFEST amended with the second registration line, the package-data row, the vendored catalog's `parents[3]` resolution depth, the shared `_tokenize` and optional `rank_bm25` it requires, and the expected registry size after re-mount (148 -> 150 for both tools). Registered count 149 -> 148. | SCOPE-ATTIC(render wave A4) | `docs/specs/render-and-engine.html` sections 3, 4 |
| `fetch_surface_water_ndwi` (the tool, its module, its own `_pc_search.py`, its 8-phrasing corpus, its registration in `tools/__init__.py`, its `_ALWAYS_OFFLOAD_SYNC_TOOLS` row, and the `fetch_nwi_wetlands` / `fetch_jrc_global_surface_water` cross-reference rows that named it) | `trid3nt_server/tools/fetchers/hydrology/fetch_surface_water_ndwi/`, `tools/__init__.py`, `server/dispatch/emitter.py`, `tools/fetchers/hydrology/fetch_nwi_wetlands/source.yaml`, `tools/fetchers/hydrology/fetch_jrc_global_surface_water/source.yaml`, `tests/fetchers/test_fetch_surface_water_ndwi.py` | A CAPABILITY, and it leaves INTACT. It is registered `fetch`, but it is a COMPUTE over a fetch: two Sentinel-2 bands read through the STAC catalog, differenced into NDWI, thresholded and polygonized - and the compute half is the half QGIS ships. The re-mount path without it is a generic Sentinel-2 band fetch plus NDWI and `gdal:polygonize` run in the user's own session. ATTIC: `~/Documents/trid3nt-scope-attic/extensions/derive_reimplementations/` as the FOURTEENTH member of that package, its MANIFEST amended with the registration line, the offload row, the shared `FetchError` / `bbox_pixel_dims` / `read_through` it requires, the rewritten prose rows and the expected registry size after re-mount (147 -> 148). Registered count 148 -> 147. | SCOPE-ATTIC(render wave B2) | `docs/specs/render-and-engine.html` sections 3, 4; `docs/IDEAS.md` ruling 38 |
...
| `tools/processing/_gdal_runner.translate_to_cog` (the COG encoder living inside the terrain-tool runner) | trid3nt_server/tools/processing | emission needs it to publish a renderable raster, and reaching backwards from `emission/publish.py` into a terrain TOOL to get it is the wrong-direction import. | DELETED (wave A, placement debts). CONDITION MET: moved verbatim to `trid3nt_server/render/cog.py`; the six processing tools plus `emission/publish.py` import it from there. `_gdal_runner` keeps what it is actually for - gdaldem/gdal_contour binary resolution, the PROJ env wiring, the subprocess call and the raster-bytes reader. | placement debts |
...
| PipelineEmitter public properties inline_geojson_by_layer_id + density_meta_by_layer_id | server/src/trid3nt_server/render/pipeline_emitter.py | CONDITION MET (ADR 0266): defensive-copy accessors whose SOLE consumer was _persist_case_view_snapshot (grep of .inline_geojson_by_layer_id / .density_meta_by_layer_id reads = ZERO after chop). The private _inline_geojson_by_layer_id / _density_meta_by_layer_id attrs KEPT (live emit_session_state wire path reads them directly). -29 LOC. | DELETED (commit-pending) | ADR 0266 |
...
| TiTiler tile-template unwrap (`_titiler_cog_uri` + Branch-3 in `_normalize_layer_uri`) | server/src/trid3nt_server/render/uri_registry.py ~314,~843 | FLAGGED (ADR 0269, persisted-shape): unwraps OLD PERSISTED case docs / `layer_handles` maps still carrying legacy TiTiler tile-template URLs; publish_layer no longer mints them. CONDITION-to-delete: a persisted-data audit/migration proves no live Case carries a TiTiler template URL (then the read-side guard is unreachable). Same class as ADR 0262's ~88 residual-TiTiler-prose flag. reopen: never (TiTiler is decommissioned). | FLAGGED (KEEP; wire/persisted) | ADR 0269 |
...
- What moved, and why it is the mechanism that moved rather than the tool that
  ...
  is now `trid3nt_server/render/publish.py` and every one of those edges is
  intra-package.
...
| `emission/quantity_styles.py`'s hand-mirrored copy of `_QGIS_STYLE_REGISTRY` | trid3nt_server/render/quantity_styles.py | the file kept "only the idea" of the registry rather than importing it; two drift tests existed to catch divergence between the two copies. | DELETED (ledger hygiene, condition MET, verified independently): `grep -rln quantity_styles --include=*.py .` = 0; `ls trid3nt_server/render/` returns cog, layer_uri_emit, mesh_display, outputs_seam, pipeline_emitter, presets, publish, restyle, uri_registry - no quantity_styles.py. Folded into `contracts/.../styles.yaml` + `emission/styles.py`. | cleanup wave phase 2
| The two lazy `emission.publish` -> `tools.processing` imports (`compute_sediment_yield.SEDIMENT_YIELD_LOG_CLASSES` + `hex_to_rgba` at `:540`, `compute_hillshade._get_gdaldem_bin` + `._translate_to_cog` at `:1459`) | trid3nt_server/render/publish.py | wrong-direction edges: emission/ imported style/raster helpers from a compute tool. | DELETED (ledger hygiene, condition MET, verified independently): both spellings grep to 0 inside `emission/*.py`; the only `compute_hillshade` reference left there is a name literal at `uri_registry.py:173`. | cleanup wave phase 2
...
## Layer reuse dissolves into the seams that already held it

| what | where it was | condition | status |
| --- | --- | --- | --- |
| `server/dispatch/layer_reuse.py` (whole module): `_FETCH_TOOL_KIND`, `_FETCHED_KIND_MARKERS`, `_RESULT_LAYER_MARKERS`, `_BBOX_QUANT_DEG`, `fetched_kind_for_tool`, `fetched_layer_kind`, `_is_result_layer`, `_layer_haystack`, `_coerce_bbox`, `_quantize_bbox`, `bbox_equivalent`, `bbox_encloses`, `_layer_to_dict`, `FetchedLayerMatch`, `find_reusable_fetched_layer`, `_ReuseEntry` | `trid3nt_server/server/dispatch/layer_reuse.py`, its `server/__init__.py` facade binding, `tests/server/test_layer_reuse.py` | The three behaviors it held each have a home: the fetch short-circuit is a LAYER-REGISTRY lookup by the cache key a landed fetch's uri already addresses (`tools/cache.parse_cache_path` -> `UriRecord.cache_key` -> `SessionUriRegistry.handle_for_cache_key`, matched against `router.prospective_cache_key`); the RESULT/INPUT labels read that same address for the dataset; `_ReuseEntry` is the dispatch's own shim and moved into `server/dispatch/emitter.py`. | **DELETED.** The eight-tool kind table, the substring markers and the 0.02-degree bbox quantization go with it: an exact content-addressed key needs no taxonomy and no tolerance. 148 pure product lines and 118 pure test lines left the tree. |
| `_pin_case_aoi_from_tool_bbox`, `_maybe_default_fetch_bbox_to_pinned_aoi`, `_bbox_round6`, `_AOI_DEFAULT_EQ_TOL_DEG` | `trid3nt_server/server/dispatch/aoi.py`, `tests/fetchers/test_aoi_pin_lane_c.py`, `tests/runtime/test_aoi_pin_fetch_bbox_durability.py` | The Case AOI is the extent a RUN solved over, pinned by `pin_case_aoi_from_solve` on a completed workflow; the fill rule is `tool_arg_normalizer.autofill_missing_bbox`, which already reads supplied > canvas AOI > Case AOI and otherwise leaves the slot for the tool's own typed error. A supplied bbox WINS, so the snap that overrode a drifted one has no rule left to enforce. | **DELETED.** A fetch states its own area; it does not decide the Case's. |
| `_bbox_overlaps` | `trid3nt_server/server/dispatch/aoi.py` | Its one live reader is the turn engine's AOI-drift warning, and what a box means against another box belongs to `Extent`. | **MOVED** to `trid3nt_server/inputs/extent.py` as `bbox_overlaps`, beside `bbox_equivalent`. |
| `bbox_encloses` | `trid3nt_server/server/dispatch/layer_reuse.py` | Named for `inputs/extent.py` by the ruling, but its two readers were the enclosure match and the bbox snap, and the ruling deletes both. No reader, no function. | **DELETED**, not moved. It returns the moment something asks whether one extent covers another. |
| The hand-written tool lists in the routing prompt: the "New Wave 4.10 endpoints" alias table, the HRRR dispatch paragraph, the "OVERLAP routing" block, the "Satellite fire-animation routing" block, the `fetch_administrative_boundaries` signature note, the three per-tool "Key behaviors" bullets, the modelling-surface capability paragraph | `trid3nt_server/adapters/adapter.py` | The prompt's tool section is GENERATED at daemon start from the registry - each model-facing tool's name beside the first line of its routing docstring (`build_tool_section`, substituted by `system_prompt()`). A second list of the same tools rots the moment one changes. | **DELETED.** The generated section is one line for each of the registry's 146 model-facing tools; the prompt went 31,560 -> 39,624 chars, and every name in it is now a tool that exists by construction. |

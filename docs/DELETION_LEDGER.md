...
| `LegendKey` as a separate wire shape from the shipped `.qml`, and the plugin's ramp translation (`render/ramps.py`, `_color_ramp_items`, `_build_pseudocolor_renderer`, `_categorical_fallback_stops`, `_embedded_palette_classes`, `_apply_raster_renderer`) | contracts/trid3nt_contracts/execution.py, plugin/render/ | The server now ships the `.qml` the map should load on `LegendKey.qml`; the plugin still rebuilds an equivalent renderer from the key's ramp + range, which is the same decision made twice. Condition: the plugin leg swaps `_apply_raster_renderer` for `loadNamedStyle(legend.qml)` (spec section 6's plugin row) - then the key collapses to the legend TEXT and ~350 LOC of translation goes with the ramp table. | QUEUED | ADR 0326 / emission-fold spec section 6 |

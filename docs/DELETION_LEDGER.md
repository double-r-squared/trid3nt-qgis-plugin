...
| HOOK-RATCHET: offset paging ($skip/$top, stop-on-short-page) sibling of totalPages paging | _router http_source | 2x flag (openfema; likely OData feeds). The chained mode's ``next_page`` hook now provides a hook-driven offset-paging primitive an openfema fold can reuse; the declarative $skip/$top variant still QUEUED (openfema not folded this wave) | QUEUED (primitive available, ADR 0063) | ADR 0061 |

...
| 23b | the solved deck's node-inflow SERIES, reduced to its peak and discarded | NEW: `curves.swmm_times_hr` + `curves.swmm_rdii_cfs`, and the declared chart OVERLAYS them. Same computed-and-discarded class as row 23, and worse in consequence: the template's claim IS the cross-check, so a product that showed only a peak ratio left the reader taking its word for it. The series rides on the ENGINE's own clock (SWMM advances on a variable step), read off the run rather than assumed to be the closed form's grid - which HEAD's proof renderer did assume |
...
- All THREE series crossed the wire on the run's own chart - the closed form
  (244 points), the native engine (287 points, its own variable-step clock) and
  direct runoff (244) - so the cross-check is IN the product, not only in a
  scalar beside it.
...
- `scripts/proof_swmm_rdii.py` - repointed onto ONE declared invocation of the
  ...
  committed panel's own caption. The panel's native-SWMM overlay is now the
  run's OWN series rather than one the renderer re-solved, and it is plotted on
  the engine's reported clock rather than an assumed uniform grid.

# RenderSeam - derived view

GENERATED from `docs/model/render-seam.sysml` by `scripts/model_check.py --view`. Never hand-edited: regenerate it, and `tests/model/test_model_conformance.py` fails while it is stale.
...
```mermaid
flowchart LR
    emissionSeamPoint["EmissionSeamPoint<br/>trid3nt_server/render/layer_uri_emit.py"]
    ...
    layerRecord["LayerRecord<br/>trid3nt_server/render/uri_registry.py"]
    outputsSeamPoint["OutputsSeamPoint<br/>trid3nt_server/render/formats.py"]
    pipelineEmitter["PipelineEmitter<br/>trid3nt_server/render/pipeline_emitter.py"]
    presetFamily["PresetFamily<br/>trid3nt_server/render/presets.py"]
    publishPath["PublishPath<br/>trid3nt_server/render/publish.py"]
    restyleSurface["RestyleSurface<br/>trid3nt_server/render/restyle.py"]
    restyleTool["RestyleTool<br/>trid3nt_server/tools/derive/restyle_layer/restyle_layer.py"]
...
| --- | --- | --- |
| **AFailedStepNamesWhyItFailed** | `pipelineEmitter` | `tests/server/test_nameless_step_failure.py::test_a_failure_with_no_name_is_refused_rather_than_recorded`<br/>`tests/server/test_nameless_step_failure.py::test_a_named_failure_is_recorded_with_its_code_and_sentence`<br/>`tests/server/test_nameless_step_failure.py::test_the_verdict_refuses_to_read_failed_off_a_step_that_names_nothing`<br/>`tests/runtime/test_nameless_failure.py::test_a_step_failure_raised_with_no_sentence_is_itself_refused`<br/>`tests/runtime/test_nameless_failure.py::test_a_runner_that_raises_silently_reaches_the_caller_named` |
| **EmissionIsAutomatic** | `emissionSeamPoint`, `outputsSeamPoint`, `pipelineEmitter`, `restyleSurface` | `tests/render/test_auto_publish_droppable_raster.py::test_intermediate_publishes_too_there_is_no_opt_out`<br/>`tests/render/test_auto_publish_droppable_raster.py::test_publish_for_emission_is_the_only_seam`<br/>`tests/render/test_restyle_surface.py::test_hiding_a_layer_is_the_un_emit_and_unhiding_puts_it_back`<br/>`tests/render/test_restyle_surface.py::test_hiding_a_layer_nobody_published_refuses_rather_than_creating_one`<br/>`plugin/tests/test_raster_render.py::test_a_visibility_flip_on_a_row_already_seen_reaches_the_layer_tree` |
| **OneScalePerQuantity** | `presetFamily`, `publishPath` | `tests/render/test_presets.py::test_one_range_spans_a_compared_set`<br/>`tests/render/test_presets.py::test_a_shared_range_beats_a_read_and_the_legend_stops_naming_percentiles`<br/>`tests/render/test_presets.py::test_a_data_policy_reads_the_range_off_the_layer_and_says_so`<br/>`tests/scripts/test_animation_legend_stability.py::test_the_scale_is_resolved_over_the_whole_run_never_one_frame`<br/>`tests/scripts/test_animation_legend_stability.py::test_the_animation_paints_what_the_published_raster_of_that_quantity_paints`<br/>`tests/render/test_publish_layer_legend.py::test_the_layer_ships_the_qml_the_map_loads_over_that_same_range`<br/>`tests/render/test_formats.py::test_a_mesh_product_paints_the_group_it_declares_on_the_shared_range`<br/>`tests/scripts/test_animation_legend_stability.py::test_the_published_range_is_found_by_quantity_not_by_the_title_it_was_painted_under`<br/>`tests/scripts/test_animation_legend_stability.py::test_a_log_ramp_takes_the_published_top_and_its_own_declared_floor` |
| **PresentationNeverInDeclarations** | `styleDeclaration`, `fetcherRouter`, `restyleSurface`, `restyleTool` | `tests/runtime/test_declarative_library.py::test_a_declaration_carries_no_presentation_vocabulary`<br/>`tests/fetchers/test_router_hrrr.py::test_units_and_style_by_param`<br/>`tests/model/test_model_conformance.py::test_the_model_conforms_to_the_tree` |
| **StyleIsDeclaredOrKindDefault** | `styleDeclaration`, `styleRowSchema`, `presetFamily`, `publishPath`, `layerMaterializer` | `tests/render/test_presets.py::test_the_family_is_four_kinds_and_every_one_of_them_writes_a_document`<br/>`tests/render/test_presets.py::test_a_declaration_that_names_no_parameters_gets_its_kinds_bare_default`<br/>`tests/render/test_presets.py::test_a_quantity_parameterises_the_preset_it_never_mints_one`<br/>`tests/fetchers/test_router_spec_loader.py::test_a_style_row_naming_a_kind_outside_the_family_is_refused_at_registration`<br/>`tests/fetchers/test_router_spec_loader.py::test_a_style_row_naming_a_geometry_outside_the_family_is_refused`<br/>`plugin/tests/test_raster_render.py::test_the_legends_qml_reaches_load_named_style`<br/>`plugin/tests/test_raster_render.py::test_a_vectors_declared_symbol_is_what_the_read_back_reports`<br/>`tests/render/test_publish_layer_legend.py::test_a_vector_row_resolves_through_the_same_seam_without_reading_the_object`<br/>`tests/render/test_publish_layer_legend.py::test_a_mesh_row_resolves_bound_to_the_group_it_declares`<br/>`tests/render/test_publish_layer_legend.py::test_the_emission_seam_stamps_the_resolved_row_on_a_vector_it_passes`<br/>`tests/model/test_model_conformance.py::test_the_model_conforms_to_the_tree` |
| **TheModelIsHandedAHandleNotAUri** | `layerRecord`, `pipelineEmitter` | `tests/render/test_layer_handles.py::test_mint_is_idempotent_per_uri`<br/>`tests/render/test_layer_handles.py::test_mint_is_monotonic_per_registration_order`<br/>`tests/render/test_layer_handles.py::test_exact_uri_values_become_short_handles`<br/>`tests/render/test_layer_handles.py::test_plugin_envelope_keeps_the_real_uri`<br/>`tests/render/test_layer_handles.py::test_verbatim_registered_uri_dual_accepts`<br/>`tests/render/test_layer_handles.py::test_short_handle_case_and_padding_tolerant`<br/>`tests/render/test_layer_handles.py::test_unknown_short_handle_rejects_typed_with_inventory`<br/>`tests/render/test_layer_handles.py::test_unregistered_object_store_uri_rejects_typed`<br/>`tests/render/test_layer_handles.py::test_emit_seam_llm_sees_handle_not_uri` |
| **UserChoiceBeatsPreset** | `presetFamily`, `restyleSurface`, `restyleTool` | `tests/render/test_presets.py::test_the_override_wins_field_by_field_over_the_declaration`<br/>`tests/render/test_restyle_surface.py::test_the_ask_beats_the_declared_row_field_by_field`<br/>`tests/render/test_restyle_surface.py::test_an_ask_nobody_made_leaves_the_declaration_alone`<br/>`tests/render/test_restyle_surface.py::test_a_comparison_paints_every_layer_on_the_one_range`<br/>`plugin/tests/test_raster_render.py::test_a_case_reopen_adopts_its_own_layers_and_loads_no_style_over_them`<br/>`plugin/tests/test_raster_render.py::test_a_different_case_sweeps_the_group_it_did_not_open` |

## What each requirement says

- **AFailedStepNamesWhyItFailed** - A step that ends red states a typed code and a sentence. A run that stops in silence leaves the reader a failed card and nothing to read off it, which is the same fault as a bed that paints in silence: the machinery cannot say what stopped the run, so nobody can act on it. Both seams refuse rather than record: the transition that marks a step failed, and the verdict stated over the steps afterwards. Where the cause is an upstream service the name says so, and where the raised exception says nothing at all its type is the sentence.
- **EmissionIsAutomatic** - A produced layer appears. There is no verb for showing one, on either arm - a fetch emits what it fetched, a solve emits every product its outputs list names - and there is no opt-out, so an intermediate is a layer too and the reader hides what they did not want to see. This is why there is no way to produce a layer and forget to display it: the display is not a second step somebody has to remember. The un-emit is the same surface running the other way: a visibility flip on a row the canvas already holds, not a second emission verb. A layer nobody published refuses rather than being quietly created.
...
- **TheModelIsHandedAHandleNotAUri** - A layer reaches the model as a short minted HANDLE, never as a store path. A path is a long string the model has to echo back exactly, and a plausible-but-wrong echo - a rewritten prefix, an invented hash tail, an id substituted for a basename - is indistinguishable from a correct one until the read fails. A handle carries no structure to get wrong. The mint is idempotent per uri and monotonic per case, so the same layer is the same handle for as long as the case is open, and the record binds ONE id to ONE uri: with a single scheme there is nothing to translate between, so a display face cannot drift from the data face. Resolution is TOLERANT INWARD and STRICT OUTWARD. A handle is accepted case-insensitively and zero-padded, and a verbatim uri the record already holds is accepted too; anything the record cannot resolve is a TYPED refusal naming what it does hold - never a guessed rewrite, and never a pass-through that reaches a reader as a path to nothing. The rewrite is the LLM-facing surface only: the envelope the canvas reads keeps the real uri, because the plugin opens the store and the model does not.

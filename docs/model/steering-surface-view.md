...
GENERATED from `docs/model/steering-surface.sysml` by `scripts/instruments/model_check.py --view`. Never hand-edited: regenerate it, and `tests/test_model_conformance.py` fails while it is stale.
...
| --- | --- | --- |
| **ANoDefaultKeywordIsTheTemplatesToState** | `slot`, `sheet` | `tests/test_telemac_module_surface.py::test_the_basin_states_how_its_tracer_is_carried_and_under_what_ceiling`<br/>`tests/test_telemac_module_surface.py::test_the_engine_default_is_on_the_slot_or_the_slot_is_a_question` |
| **ATemplateNumberIsMetToTheMeshsPrecision** | `door`, `sheet` | `tests/test_open_water_domains.py::test_the_footprint_and_the_solid_faces_are_cut_at_the_same_width`<br/>`tests/test_open_water_domains.py::test_a_boundary_node_is_on_the_structure_when_it_stands_on_the_punched_outline`<br/>`tests/test_open_water_domains.py::test_a_lone_node_between_two_of_another_kind_is_not_a_face` |
...
| **CompositesSetOnlyPresence** | `composite`, `wrapper` | `tests/test_telemac_module_surface.py::test_a_composite_sets_only_what_its_value_s_presence_defines`<br/>`tests/test_telemac_module_surface.py::test_a_wind_from_the_north_drives_the_water_south`<br/>`tests/test_telemac_module_surface.py::test_a_continuation_names_the_file_and_leaves_its_format_to_the_template`<br/>`tests/test_telemac_module_surface.py::test_a_curve_number_field_names_no_model_and_the_template_does` |
...
| **EngineDefaultSurfaced** | `slot`, `sheet`, `door` | `tests/test_telemac_module_surface.py::test_an_open_slot_under_the_fold_says_the_dictionary_answers_it_for_nobody`<br/>`tests/test_telemac_module_surface.py::test_the_engine_default_is_on_the_slot_or_the_slot_is_a_question`<br/>`tests/test_telemac_module_surface.py::test_an_engine_default_is_never_written_into_the_deck`<br/>`tests/test_telemac_module_surface.py::test_the_bare_sheet_opens_every_keyword_the_dictionary_answers_for_nobody` |
...
| **EverythingOverridable** | `wrapper`, `sheet`, `canvasGate`, `door` | `tests/test_telemac_module_surface.py::test_a_raw_keyword_on_the_wire_fills_the_slot_it_names`<br/>`tests/test_telemac_module_surface.py::test_a_raw_keyword_the_module_does_not_have_refuses_naming_the_nearest`<br/>`tests/test_telemac_module_surface.py::test_a_raw_keyword_value_outside_the_choices_refuses_naming_the_choices`<br/>`tests/test_telemac_module_surface.py::test_two_releases_on_the_floor_are_two_sources_in_the_deck`<br/>`tests/test_telemac_module_surface.py::test_every_template_wire_carries_the_raw_keyword_floor`<br/>`tests/test_telemac_module_surface.py::test_a_floor_that_is_not_a_mapping_refuses_by_name`<br/>`tests/test_telemac_module_surface.py::test_resolution_order_is_engine_then_the_parts_then_template_then_fill`<br/>`tests/test_telemac_module_surface.py::test_a_composed_slot_says_which_part_asserted_it`<br/>`tests/test_telemac_module_surface.py::test_a_fill_is_repeatable_and_the_later_one_stands`<br/>`tests/test_telemac_module_surface.py::test_a_drawn_point_is_a_fill`<br/>`tests/test_telemac_module_surface.py::test_a_canvas_that_answers_nothing_refuses_and_invents_nothing` |
...
| **OneTemplatePerQuestion** | `door` | `tests/test_telemac_module_surface.py::test_a_structural_fork_is_a_template_and_never_a_switch`<br/>`tests/test_telemac_module_surface.py::test_no_flipped_body_branches_on_anything`<br/>`tests/test_door_dissolution.py::test_all_templates_registered_and_callable`<br/>`tests/test_telemac_module_surface.py::test_every_template_names_the_reader_its_own_question_needs`<br/>`tests/test_postprocess_telemac.py::test_the_peak_layer_handle_carries_the_product_that_wrote_it` |
| **ProvenanceNamesTheLayerThatAnswered** | `sheet`, `door` | `tests/test_telemac_module_surface.py::test_a_value_the_run_measured_reads_as_derived_not_as_the_body_that_named_it`<br/>`tests/test_telemac_module_surface.py::test_a_measured_value_a_fill_overrides_still_reads_as_the_users`<br/>`tests/test_telemac_module_surface.py::test_a_composed_slot_says_which_part_asserted_it` |
| **ReviewIsTheDoorsView** | `door` | `tests/test_telemac_module_surface.py::test_the_card_shows_what_is_set_and_open_and_folds_the_rest_by_rubrique`<br/>`tests/test_telemac_module_surface.py::test_the_fill_docstring_names_the_module_its_rubriques_and_its_open_slots`<br/>`tests/test_telemac_module_surface.py::test_the_review_is_the_doors_view_and_never_a_step_of_its_own`<br/>`tests/test_declarative_library.py::test_do_sag_declares_no_gate_in_front_of_its_self_gating_review` |
| **RunIsHeld** | `sheet`, `serializer`, `stager` | `tests/test_telemac_module_surface.py::test_run_refuses_an_incomplete_sheet_naming_the_required_file`<br/>`tests/test_telemac_module_surface.py::test_run_serializes_then_stages_then_dispatches`<br/>`tests/test_model_conformance.py::test_the_model_conforms_to_the_tree` |
| **SharedBodyHasTwoUsers** | `sharedBody`, `sheet` | `tests/test_telemac_module_surface.py::test_every_shared_body_has_at_least_two_users` |
| **TheOpenSetIsCompleteAndRequiredIsTheObligFiles** | `door`, `slot`, `sheet` | `tests/test_telemac_module_surface.py::test_every_required_file_is_the_template_s_own_statement`<br/>`tests/test_telemac_module_surface.py::test_the_artemis_forcing_composite_carries_the_file_and_not_its_name`<br/>`tests/test_telemac_module_surface.py::test_the_bare_sheet_opens_every_keyword_the_dictionary_answers_for_nobody`<br/>`tests/test_telemac_module_surface.py::test_a_defaulted_oblig_file_is_not_a_question_the_sheet_asks`<br/>`tests/test_telemac_module_surface.py::test_the_open_row_says_whether_the_run_cannot_begin_without_it`<br/>`tests/test_telemac_module_surface.py::test_run_does_not_refuse_an_open_keyword_the_engine_may_yet_default`<br/>`tests/test_telemac_run_reads.py::test_the_engine_s_own_demand_is_read_by_name_out_of_the_listing`<br/>`tests/test_run_telemac_chain.py::test_classify_exit_names_the_keyword_lecdon_asked_for`<br/>`tests/test_run_telemac_chain.py::test_classify_exit_invents_no_demand_where_the_engine_made_none` |
| **TheSteeringFormatHasOneWriter** | `serializer`, `steeringDriver` | `tests/test_telemac_module_surface.py::test_the_serializer_is_the_only_module_that_writes_a_keyword_into_a_deck` |
| **WrapperHasNoOpinion** | `wrapper` | `tests/test_telemac_module_surface.py::test_a_wrapper_asserts_nothing_and_has_no_hook_to`<br/>`tests/test_telemac_module_surface.py::test_a_wrapper_is_a_declaration_and_refuses_to_be_a_value`<br/>`tests/test_telemac_module_surface.py::test_a_coupled_body_states_only_what_its_caller_handed_it` |

## What each requirement says

- **ANoDefaultKeywordIsTheTemplatesToState** - RULING 2026-09-06 (docs/IDEAS.md, "STAGE 3 RULINGS, LIVE FINDINGS", (a)): a keyword the dictionary gives NO default for is a choice somebody makes. Left unstated it is not silence - the engine substitutes something of its own (SCHEME FOR ADVECTION OF TRACERS falls back to whatever the VELOCITIES are advected by) and the deck records no choice at all, so nobody can read what the run was solved under. Such a keyword is the TEMPLATE's to state, never the wrapper's, and it states three things with it: the value, the BASIS for the value where the LLM and the human both read it, and a way to override it on the sheet. A value the template derives rather than asserts names the producer it came from, and reuses the one that already exists.
- **ATemplateNumberIsMetToTheMeshsPrecision** - RATIFIED 2026-09-06 (docs/IDEAS.md, "REMEDY RULINGS" (a)): a template declares ONE number for a physical thing - the obstacle half-width says where the cut is - and the deck's own statement about that thing is read off the same number. Two numbers would let the cut and the deck disagree about where a structure is. What a node is MEASURED AGAINST is that number's outline plus the ACCEPTED MESH'S OWN EDGE. The edge is a property of the mesh - what a relaxation places a node on a locked outline to - and not a second tolerance the template gets to pick. Measured both ways on the same harbour: a band sized off the element edge ALONE sat entirely inside the cut, matched zero boundary nodes, and the run solved with its breakwater as absorbing shore; an equality at the half-width exactly halved a population sitting 9-11 m off a 20 m structure. A value derived that way is DERIVED on the sheet, not template: the template says which measurement the slot takes, and the number is the accepted artifact's.

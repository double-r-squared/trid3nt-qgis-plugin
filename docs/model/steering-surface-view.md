...
- **OneTemplatePerQuestion** - A structural fork of the deck is a TEMPLATE, never a switch on a param. A tracer, an oil slick, a moving bed and a settling class fill DIFFERENT slots, and the arity of the carrier's own tracer surface moves with the fork - so each body states that surface itself rather than letting a composite own a carrier slot out of sight. What varies WITHIN one question is a composite that states nothing when it is given nothing. The rule holds on the READING side too. Each template NAMES the reader its question needs, out of the module's own OUTPUTS bindings; a single publisher branching on a substance-class string put the fork back in the products, one level down from where a person reads it.

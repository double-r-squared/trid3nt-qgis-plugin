...
| **OneTemplatePerQuestion** | `door` | `tests/test_telemac_module_surface.py::test_a_structural_fork_is_a_template_and_never_a_switch`<br/>`tests/test_telemac_module_surface.py::test_no_flipped_body_branches_on_anything`<br/>`tests/test_door_dissolution.py::test_all_templates_registered_and_callable` |
...
- **OneTemplatePerQuestion** - A structural fork of the deck is a TEMPLATE, never a switch on a param. A tracer, an oil slick, a moving bed and a settling class fill DIFFERENT slots, and the arity of the carrier's own tracer surface moves with the fork - so each body states that surface itself rather than letting a composite own a carrier slot out of sight. What varies WITHIN one question is a composite that states nothing when it is given nothing.

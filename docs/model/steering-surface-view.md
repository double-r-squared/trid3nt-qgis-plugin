...
- **InputsAreTypedPrimitives** - Every point in the system is a Point. A slot that asks for one fills from each of its sources - the pick, a pair, a point layer, a geocoded name - under the same ingestion, keeps the name the source carried, refuses by name a point outside the domain, and moves a dry landing to the nearest wet node. Neither the user nor the model names it twice.

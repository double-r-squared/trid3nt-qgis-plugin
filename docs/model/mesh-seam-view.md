...
GENERATED from `docs/model/mesh-seam.sysml` by `scripts/model_check.py --view`. Never hand-edited: regenerate it, and `tests/model/test_model_conformance.py` fails while it is stale.
...
```mermaid
flowchart LR
    meshArtifactStore["MeshArtifactStore<br/>trid3nt_server/mesh/artifact.py"]
    meshGate["MeshGate<br/>trid3nt_server/mesh/gate.py"]
    meshRouter["MeshRouter<br/>trid3nt_server/mesh/tool.py"]
    meshSession["MeshSession<br/>trid3nt_server/mesh/session.py"]
    mesherRegistry["MesherRegistry<br/>trid3nt_server/mesh/meshers/__init__.py"]
    om2dAdapter["MesherAdapter<br/>trid3nt_server/mesh/meshers/om2d.py"]
    om2dBox["MesherBox<br/>workers/mesh/scripts/om2d.py"]
    opTool["OpTool<br/>trid3nt_server/mesh/op_tool.py"]
    recipeObject["RecipeObject<br/>trid3nt_server/mesh/recipe.py"]
    regGridAdapter["MesherAdapter<br/>trid3nt_server/mesh/meshers/reg_grid.py"]
    selafinIo["SelafinIo<br/>trid3nt_server/workflows/telemac/authoring/selafin_io.py"]
    sharedPrimitives["SharedPrimitives<br/>trid3nt_server/mesh/shared/primitives.py"]
...
| --- | --- | --- |
| **APinchedBoundaryIsRefusedByName** | `selafinIo` | `tests/mesh/test_mesh_topology_and_bed.py::test_a_boundary_walk_whose_loops_share_a_node_refuses_naming_it`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_the_boundary_numbering_is_one_count_across_every_loop`<br/>`tests/telemac/test_telemac_boundary_contract.py::test_a_pinched_boundary_refuses_by_name` |
| **AnUnreadableFileRefusesRatherThanFallBack** | `selafinIo` | `tests/telemac/test_telemac_result_reader.py::test_a_refusal_names_the_file`<br/>`tests/telemac/test_telemac_result_reader.py::test_a_result_whose_frames_are_cut_short_refuses_by_name`<br/>`tests/telemac/test_telemac_result_reader.py::test_a_pair_that_cannot_be_written_refuses_by_name`<br/>`tests/telemac/test_telemac_result_reader.py::test_a_frame_that_will_not_read_refuses_by_the_same_name` |
| **BoundaryRolesAreContiguousRuns** | `sharedPrimitives`, `om2dAdapter` | `tests/mesh/test_mesh_topology_and_bed.py::test_a_scattered_candidate_boundary_resolves_into_two_contiguous_runs`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_a_run_that_wraps_the_contours_origin_stays_one_run`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_one_role_declared_across_two_faces_lands_as_two_sections`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_boundary_runs_prescribe_the_roles_their_types_name`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_runs_and_named_faces_are_the_same_statement`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_a_node_two_declared_faces_both_claim_refuses`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_a_face_the_mesh_never_reaches_refuses_rather_than_going_unprescribed` |
| **CorrectDataClassAtSetBed** | `sharedPrimitives` | `tests/mesh/test_mesh_topology_and_bed.py::test_the_bed_op_permits_no_ladder_rung_on_the_authors_behalf`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_a_source_naming_nothing_refuses_rather_than_leaving_a_bedless_mesh`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_a_conditioning_this_primitive_does_not_perform_refuses`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_the_interpolation_is_a_visible_default_off_a_declared_roster`<br/>`tests/mesh/test_mesh_topology_and_bed.py::test_the_journal_names_the_rung_that_ACTUALLY_painted_the_bed`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_the_bed_is_one_source_and_two_are_merged_before_the_op`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_a_domain_no_source_covers_refuses_by_name`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_a_stated_depth_is_a_flat_bed_below_the_free_surface` |
| **ModelShrinksWithTheTree** | `om2dAdapter`, `regGridAdapter`, `om2dBox` | `tests/mesh/test_mesh_meshers.py::test_the_roster_is_the_meshers_the_tree_carries`<br/>`tests/mesh/test_mesh_meshers.py::test_the_roster_is_the_meshers_the_tree_carries`<br/>`tests/model/test_model_conformance.py::test_the_model_conforms_to_the_tree` |
| **NothingPaintsSilently** | `sharedPrimitives` | `tests/mesh/test_bed_sources_and_runs.py::test_a_node_no_row_reached_refuses_with_the_feedback_and_never_a_zero`<br/>`tests/mesh/test_bed_sources_and_runs.py::test_the_journal_states_what_every_row_of_a_composed_bed_reached`<br/>`tests/inputs/test_bed_merge.py::test_the_feedback_states_the_water_and_the_land_separately`<br/>`tests/inputs/test_bed_merge.py::test_a_hole_too_small_to_round_is_stated_and_still_filled`<br/>`tests/runtime/test_matched_slots.py::test_the_bed_lays_the_one_row_the_match_ranked_first` |
...
| **TheGateIsNeverSpecialised** | `meshGate` | `tests/model/test_model_conformance.py::test_the_model_conforms_to_the_tree`<br/>`tests/mesh/test_mesh_gate_loop.py::test_the_accept_is_the_card_submitted_rather_than_a_tool`<br/>`tests/mesh/test_mesh_gate_loop.py::test_adopting_a_hand_edited_layer_is_a_row_on_the_card`<br/>`tests/telemac/test_telemac_module_surface.py::test_the_mesh_gate_receives_the_runs_own_mode` |
...
- **APinchedBoundaryIsRefusedByName** - A mesh becomes a geometry pair only if the walk that NUMBERS its boundary visits every node once. IPOBO is a permutation of 1..NPTFR, so a node two rings both pass through would need two positions and gets the second, and the successor array built from the contour lengths then outruns the boundary itself. The walk that decides this is OURS and runs in this process: over the same cells it returns the engine's own rings - the same boundary node set and the same count - differing only in which node each ring starts at. The refusal names the shared nodes and their count, typed, never the index error that named nothing.
- **AnUnreadableFileRefusesRatherThanFallBack** - Both directions of the pair are this process's own, so neither has an in-image half to retry against. A result whose header will not open, and one whose header opens but whose frames are cut short, refuse by the same name and name the FILE; a geometry or .cli that cannot be written refuses by name and names both halves. The pair is one artifact - half of it on disk is a numbering nothing agrees with - so a partial write is a refusal and never a return value.
- **BoundaryRolesAreContiguousRuns** - A role is a RUN, not a node set: a liquid boundary is numbered by walking the boundary, so a scatter of nodes near a face is not a boundary. One role may land several runs - a two-mouth estuary has one open boundary in two sections - and a node two faces both claim refuses, because it carries one boundary condition and picking silently would put a flowrate on a stretch meant to hold at a level. The law is the MESH's, so it holds wherever roles settle - a recipe prescribing them across a declared face, and equally a steering author settling them on a mesh it was handed. The harbour settle was deciding node by node and produced exactly what front2.f refuses by name, "a solid point between two liquid points"; a lone node whose two walk neighbours agree with each other takes their role, and the two roles come out as runs. And a node is measured against a locked outline TO THE MESH'S OWN EDGE. The edge length is a property of the accepted mesh - what a relaxation places a node to - not a second tolerance an author may pick: Point Judith holds 176 boundary nodes at 9-11 m off a 20 m structure with nothing at all between 11 and 12 m, so an equality at 10.000 m halves one population and alternates along the walk.
- **CorrectDataClassAtSetBed** - A bed is TOPOBATHY, and the primitive takes the data class it is defined over. Acquiring that class is the DATA row's problem: no ladder rung is permitted here on the author's behalf, because a rung this op allowed would be a cross-dataset bed nobody wrote down. A domain the primary does not cover refuses in the fetcher's own words, and what ACTUALLY painted the bed rides back rather than what was asked for. The bed is ONE source here. A bed composed of several rows is a spatial MERGE of available layers, which is a derive - useful on its own, outside any slot - and it is composed in the DATA body before this op runs, because the RUN stated the op that lays them: the first row paints every cell it measured, each named row paints what the ones before it left, and a sidecar records which won at each cell. A fetcher's own fallback chain is a different thing entirely, and one is never spelled as the other.
...
- **NothingPaintsSilently** - FEEDBACK OVER BUILT-IN BEHAVIOUR. A node standing on a cell no row measured has no elevation, and a number put there on its behalf - a mean, a zero, a plane - is a bed nobody surveyed and nothing downstream can tell from one somebody sounded. So the op REFUSES by name: how many nodes, the box they stand in, and the coverage feedback the surface states about itself, which is the thing the person acts on. The feedback itself is stated over the two grounds a bed answers for separately - the water the domain was cut with, and the land - because a share over the whole grid mixes the land a terrain surface painted with the channel the run is about. Every row that painted is named with the share it reached, never the first two of five.

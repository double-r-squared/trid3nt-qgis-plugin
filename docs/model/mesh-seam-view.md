...
| --- | --- | --- |
| **AcceptanceRefusesAPinchedBoundary** | `meshSession` | `tests/test_mesh_gate_loop.py::test_accept_refuses_a_boundary_walk_whose_rings_share_a_node`<br/>`tests/test_mesh_gate_loop.py::test_a_boundary_that_closes_once_is_accepted` |
...
- **AcceptanceRefusesAPinchedBoundary** - A mesh is FROZEN only if its boundary walk closes once through every node. A node two rings both pass through has no boundary numbering: the walk visits it twice while the geometry holds it once, so the successor array a writer builds outruns the node count and the write either crashes or classifies the wrong nodes. The acceptance names the shared nodes and their count, because a crash carries none of that and a caller can act on a named node.

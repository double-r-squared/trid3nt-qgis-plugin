...
| --- | --- | --- |
| **APinchedBoundaryIsRefusedByName** | `selafinCliDriver` | `tests/telemac/test_telemac_boundary_contract.py::test_a_walk_whose_rings_share_a_node_refuses_naming_them`<br/>`tests/telemac/test_telemac_boundary_contract.py::test_a_walk_that_visits_every_node_once_states_nothing`<br/>`tests/telemac/test_telemac_boundary_contract.py::test_the_host_re_raises_the_walks_own_refusal_typed` |
...
- **APinchedBoundaryIsRefusedByName** - A mesh becomes a geometry pair only if the walk that NUMBERS its boundary visits every node once. IPOBO is a permutation of 1..NPTFR, so a node two rings both pass through would need two positions and gets the second, and the successor array built from the contour lengths then outruns the boundary itself. The walk that decides this is the engine's own, inside the image: a host-side walk of the same cells returns different rings, so nothing outside that process can measure it. The refusal is therefore written WHERE the walk runs, naming the shared nodes and their count, and travels back as a document the host re-raises typed - never as the index error that named nothing.

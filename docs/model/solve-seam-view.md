...
flowchart LR
    ...
    basinDomainClip["WaterDomainClip<br/>trid3nt_server/workflows/telemac/templates/stratified_flow/measured_bed.py"]
...
| --- | --- | --- |
| **AWaterDomainIsClippedToItsMeasuredBed** | `basinDomainClip` | `tests/test_open_water_domains.py::test_the_basin_is_the_water_the_survey_actually_sounded`<br/>`tests/test_open_water_domains.py::test_water_the_survey_never_reached_refuses_rather_than_meshing`<br/>`tests/test_open_water_domains.py::test_the_basin_reads_the_same_survey_it_was_clipped_to` |
...
| **CatchmentOutletHoldsADerivedRatingCurve** | `assembler`, `uniformFlow` | `tests/test_telemac_outflow_stage.py::test_the_rating_curve_starts_dry_and_ends_at_the_stated_ceiling`<br/>`tests/test_telemac_outflow_stage.py::test_the_curve_rises_monotonically_so_the_engine_can_interpolate_it`<br/>`tests/test_telemac_outflow_stage.py::test_every_point_is_the_normal_depth_the_reachs_own_stage_would_be`<br/>`tests/test_telemac_outflow_stage.py::test_the_curve_file_is_written_in_the_engines_own_block_format`<br/>`tests/test_telemac_outflow_stage.py::test_the_catchments_outlet_slope_is_the_bed_over_the_elements_it_touches`<br/>`tests/test_telemac_outflow_stage.py::test_a_flat_outlet_refuses_rather_than_holding_a_level_nobody_measured`<br/>`tests/test_telemac_outflow_stage.py::test_the_flow_range_is_the_gross_rain_rate_on_the_meshed_area` |
...
| **OneNumberDefinesTheStructure** | `assembler` | `tests/test_open_water_domains.py::test_the_footprint_and_the_solid_faces_are_cut_at_the_same_width`<br/>`tests/test_open_water_domains.py::test_a_boundary_node_is_on_the_structure_when_it_stands_on_the_punched_outline` |
| **OutflowStageIsNormalDepth** | `uniformFlow`, `assembler` | `tests/test_telemac_outflow_stage.py::test_the_stage_is_the_depth_at_which_the_section_conveys_the_discharge`<br/>`tests/test_telemac_outflow_stage.py::test_the_friction_slope_is_the_measured_fall_over_the_measured_length`<br/>`tests/test_telemac_outflow_stage.py::test_the_outflow_face_is_measured_as_a_transect_of_the_painted_bed`<br/>`tests/test_telemac_outflow_stage.py::test_the_reach_length_is_walked_along_the_line_the_mesh_was_built_over`<br/>`tests/test_telemac_outflow_stage.py::test_a_bigger_discharge_stands_higher_in_the_same_channel`<br/>`tests/test_telemac_outflow_stage.py::test_strickler_and_its_reciprocal_manning_derive_the_same_stage`<br/>`tests/test_telemac_outflow_stage.py::test_an_input_the_stage_cannot_be_derived_from_refuses_by_name`<br/>`tests/test_telemac_module_surface.py::test_the_reach_body_is_written_at_the_derivation_it_was_solved_for`<br/>`tests/test_telemac_outflow_stage.py::test_a_flatter_reach_stands_higher_for_the_same_flow` |
...
- **AWaterDomainIsClippedToItsMeasuredBed** - RULING 2026-09-06 (docs/IDEAS.md, "CONFORMANCE RULINGS": D8b): a water domain is the mapped water polygon INTERSECT the bed raster's valid-data footprint. A mapped polygon says where the water is; a survey says where its floor was sounded, and the two disagree over an arm or a slip - so meshing the difference builds elements the bed painter has nothing to give. A water domain has no bed where nobody measured. The footprint is measured from the raster's own MASK, never from a threshold on its values: a nodata cell and a cell sounded at zero depth are different statements and only the mask tells them apart. The clip is journaled with the area it kept against the area the polygon mapped, beside the node count of the mesh built on it. A clip that meets nothing REFUSES by name - the answer to "the survey does not reach this water" is a bed that does, never a domain nobody sounded. ONE fetch serves both readers: the clip and the paint read the same survey, or a node lands where the clip said nothing was measured.
...
- **OneNumberDefinesTheStructure** - RULING 2026-09-06 (docs/IDEAS.md, "CONFORMANCE RULINGS": D8a): the declared structure's HALF-WIDTH is what the mesher removes water inside and what the deck calls solid. A survey maps a breakwater as a centreline, the recipe punches the centreline buffered by half the declared width, and the water inside that footprint is gone - so a boundary node stands on the punched outline exactly when it lies within the half-width of the centreline, and there is no other way for a boundary node to be there. Two numbers let the two disagree. A band sized off the element edge reaches past the cut, stamps solid on open water and leaves one solid node between two liquid ones, which ARTEMIS refuses in FRONT2 - measured on the supplied-mesh acceptance arm, which would not solve while the same structure on the same AOI solved through the recipe.

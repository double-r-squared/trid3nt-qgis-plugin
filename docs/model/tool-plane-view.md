...
| **TheLibraryStatesOnlyWhatTheRegistriesCarry** | `libraryListing`, `libraryPanel` | `tests/server/test_library_http_route.py::test_every_registered_tool_is_listed_once`<br/>`tests/server/test_library_http_route.py::test_every_coverage_row_is_listed_under_its_own_class_and_kind`<br/>`tests/server/test_library_http_route.py::test_a_data_row_carries_its_cell_window_and_datum`<br/>`tests/server/test_library_http_route.py::test_search_route_answers_both_facets_from_one_ask`<br/>`tests/server/test_library_http_route.py::test_a_class_the_ask_never_names_brings_no_rows`<br/>`plugin/tests/test_library_panel.py::test_tools_keep_their_subsystem_and_wire_order`<br/>`plugin/tests/test_library_panel.py::test_rows_sit_under_class_then_kind_with_their_fetcher_first` |
...
- **TheLibraryStatesOnlyWhatTheRegistriesCarry** - The library is a READ over what is registered and what is declared. Every registered tool is listed once under one subsystem - a tool under no known module prefix is listed as other rather than dropped, so the listing can never be quietly shorter than the registry - and every source row is listed under the class and kind the row itself declares. Every fact beside a name is metadata the tool or the row already carries; the listing measures nothing and concludes nothing. One ask reaches both facets. The tools are ranked by the same corpus the model routes on, so the person and the model are answered off one shortlist. A source row carries no corpus phrasing and is reached only through the data classes the ask names, and it carries no score, because a number beside a row would read as a measurement of the row. The order is the server's. The panel that reads the listing re-sorts nothing, so what a person sees and what the listing states are one order.

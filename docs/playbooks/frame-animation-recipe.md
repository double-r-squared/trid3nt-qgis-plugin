...
- `fetch_goes_animation` / `fetch_goes_blend_animation` / `fetch_viirs_day_fire`
  ...
  (`"GOES <Product> step <N> <ISO> (<SAT>)"`). These three are spec-driven
  (`shape: animation_frames` + the SLIDER-stitch `frames_plan` / `frame_bytes`
  hooks) rather than coded twins; the names, the list return and the scrubber
  token are the same either way, so this recipe is unaffected. (`band="blend"`

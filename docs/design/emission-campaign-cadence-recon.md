...
| HEC-RAS flood_2d + culvert_embankment_flow | Plan HDF stores 2D results at its own default Mapping Output Interval (never touched by our deck-edit code) | **At-exit** -- `subprocess.run` blocks; postprocess agent-side (`postprocess_hecras.py`) | Not found -- `deck_edit.py` only edits "Computation Interval" (the solver TIME STEP, not the output/mapping interval); no mapping-interval keyword is ever written by us | **No** | **Peak-only**: `postprocess_hecras` reads `depth_ft` as "the peak WATER DEPTH (max WSE minus bed elevation)" into ONE COG + a mesh preview; zero frame emission | **L** |

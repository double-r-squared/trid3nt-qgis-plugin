...
| SWMM urban_flood / dual-drainage | SWMM `.out` binary -- EVERY `REPORT_STEP` reporting timestep, natively | **At-exit** for the dev-primary path: `pyswmm.Simulation` runs **IN-PROCESS on the agent host** (`run_swmm_local`), not in a container; postprocess reads the finished `.out` after the `with Simulation(...)` block exits | `REPORT_STEP` (HH:MM:SS), hardcoded per-template (e.g. `01:00:00`, or a `dt_min`-derived string in some templates); no `output_interval_min` name anywhere | **No** -- no template wires `REPORT_STEP` to a shared cadence arg | **All** reporting steps, subsampled to `MAX_FLOOD_FRAMES` (shared `frames.py` selector) -- peak + `swmm_depth_frame_NN.tif` | **M** |

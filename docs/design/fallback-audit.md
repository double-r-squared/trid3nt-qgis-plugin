...
| 17 | `mesh/raster_cell_mesh.py:1203-1211` roughness/imperviousness demo defaults | user did not set the constitutive lever | Manning n / imperviousness fall to historical literals (drive routing) | **SILENT** / physics | **WORSE THAN AUDITED** (re-verified 2026-08-21): the `run_swmm.py:204` log (cited as `:184` until the F2b review -- that line is a COMMENT) fires only when `resolved_physics` is truthy -- i.e. only when the user OVERRIDES. On the actual default path nothing logs, no field, no `SyntheticInput`. PARKED (ADR 0299) |
...
| 19 | `workers/modflow/gwt_adapter.py:181` `DEFAULT_SFR_STREAMBED_GRADIENT` linear profile | no DEM rbot supplied for the SFR reach | streambed slope = flat demo gradient (drives Manning flow) | **SILENT** / physics | **WRONG AS AUDITED**, and the 2026-08-21 re-verification was wrong too (corrected by the F2b review, same day): `river_dem_uri` IS a field on `MODFLOWRunArgs` (`modflow_contracts.py:427`, documented `:283-285` as the DEM-sampled rbot path) -- but `run_modflow.py:1052-1064` builds `river_kwargs` WITHOUT it, so the contract advertises a path the composer silently drops (the row-8 false-promise class). The branch at `gwt_adapter.py:1241` is real but unreachable, so the demo gradient is unconditional IN EFFECT, and the constant is ALSO the per-cell default inside the DEM-supplied branch (`:1243-1245`) and the last reach's gradient (`:1268`). No `SyntheticInput` exists anywhere on this path. PARKED as a law-9 + wire-or-delete fork (ADR 0299 fork 3, as corrected) |

...
The SWAN analogue of ``workers/geoclaw/test_setrun_builder.py``. These
...
def test_entrypoint_swanrun_invocation_finds_authored_swn(tmp_path: Path):
    ...
    from workers.swan.entrypoint import _run_swan
...
def test_all_dry_guard_fires_and_passes():
    ...
    """
    from workers.swan.entrypoint import (
        SwanAllDryGridError,
...
def test_all_dry_guard_excludes_exception_sentinel():
    ...
    """
    from workers.swan.deck_builder import SWAN_EXCEPTION_VALUE
    from workers.swan.entrypoint import _assert_bottom_has_wet_cells
...
def test_storm_boundary_timeseries_writes_tpar_and_uses_file_boundspec(tmp_path):
    from workers.swan.deck_builder import TPAR_FILENAME

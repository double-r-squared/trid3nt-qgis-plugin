...
the agent box). The SWAN analogue of ``workers/geoclaw/entrypoint.py``.
...
SCRATCH = Path(os.environ.get("TRID3NT_SWAN_SCRATCH", "/opt/trid3nt/work"))
...
def _author_deck(build_spec: dict, cwd: Path, bathy_path: Path) -> Any:
    ...
    ``SwanDeckManifest``.
    """
    from workers.swan.deck_builder import build_swan_deck
...
def _assert_bottom_has_wet_cells(cwd: Path) -> tuple[float, float, int, int]:
    ...
    """
    from workers.swan.deck_builder import (
        SWAN_DEPMIN_M,
...
def _run_swan(cwd: Path) -> tuple[int, Path, Path]:
    ...
    """
    from workers.swan.deck_builder import SWN_CASENAME
...
def _write_publish_manifest(run_id: str, pp_manifest: dict) -> str:
    ...
    from workers._raster_postprocess import manifest as _manifest_mod
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOG.info(
        "trid3nt-swan-solver starting - project=%s run_id=%s manifest=%s "
        "object_store=%s",
        GCP_PROJECT,
        run_id,
        manifest_uri,
    ...
    try:
        ...
        if status == "ok":
            try:
                from workers._swan_postprocess import run_swan_postprocess

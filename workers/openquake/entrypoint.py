...
try:
    # Production package context (python -m workers.openquake.entrypoint).
...
LOG = logging.getLogger("trid3nt.worker.openquake")
logging.basicConfig(
    level=logging.INFO,
...
def _build_argv_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="s3:// / gs:// URI of the build_spec (also $TRID3NT_MANIFEST_URI).",
    )
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOG.info(
        "trid3nt-openquake-solver starting — project=%s run_id=%s manifest=%s "
        "object_store=%s",
        GCP_PROJECT,
        run_id,
        manifest_uri,
    ...
    try:
        ...
        if rc == 0:
            try:
                from workers._openquake_postprocess import run_openquake_postprocess

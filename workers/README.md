...
- **Docker image (`local-docker`)** -- SFINCS, GeoClaw, SWAN, TELEMAC, ELMFIRE.
  ...
  ```sh
  docker build -t trid3nt-local/geoclaw:latest -f workers/geoclaw/Dockerfile .
  docker build -t trid3nt-local/swan:latest -f workers/swan/Dockerfile .
  ...
  independent of `workers/elmfire/Dockerfile` (that Dockerfile targets

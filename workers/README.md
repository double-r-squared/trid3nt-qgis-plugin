...
| --- | --- |
| `telemac/` | The one solver worker plus `scripts/` - TELEMAC's own in-image scripts (`cas.py`, `dico.py`, `result.py`), bind-mounted as `/drivers` for the short round trips. `entrypoint.py` reads the `manifest.json` the agent staged into the bind-mounted rundir, runs the deck and writes results back into it. No boto3 in the image - the agent-side supervisor uploads the mounted outputs and writes `completion.json` afterward. |
| `mesh/` | The GPL-isolated OceanMesh2D ENVIRONMENT plus `scripts/` - the mesh's own in-image scripts (`om2d.py`, `selafin_cli.py`), bind-mounted as `/drivers` beside a rundir, the entrypoint overridden. |

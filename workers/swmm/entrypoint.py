...
Near-verbatim copy of ``workers/sfincs/entrypoint.py`` (the worker
...
SCRATCH = Path(os.environ.get("TRID3NT_SWMM_SCRATCH", "/opt/trid3nt/work"))
...
# worker (workers/sfincs/entrypoint.py): the worker contract is
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if rc == 0 and postprocess_spec:
            try:
                from workers._swmm_postprocess import run_swmm_postprocess

...
uses (schema documented in workers/landlab/entrypoint.py), but with
...
def run_chain(manifest_path: str) -> int:
    ...
    # same workers.landlab package -- the repo root must be on PYTHONPATH,
    # which the LocalSolverSpec's env_overrides handles).
    try:
        from workers.landlab.entrypoint import (  # type: ignore[import]
            _read_dem_for_grid,
            _write_field_cog,
        )
        from workers.landlab.component_chain import run_component_chain  # type: ignore[import]

...
The Landlab analogue of ``workers/modflow/test_gwt_adapter.py``: pins
...
Run from the repo root so ``workers.landlab`` imports resolve, e.g.
``PYTHONPATH=. pytest workers/landlab/test_component_chain.py``.
"""
...
# Make ``workers.landlab.component_chain`` importable when this file is
# run directly from the worker dir (mirrors test_gwt_adapter's path bootstrap).
_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from workers.landlab.component_chain import (  # noqa: E402
...
# (``workers/landlab``), and pytest's default prepend import mode puts
# ``workers`` on sys.path — so a bare ``import landlab`` can resolve to
...
# (``workers`` is NOT on it), so ``import landlab`` resolves to the real
# library and ``import workers.landlab.component_chain`` resolves here —

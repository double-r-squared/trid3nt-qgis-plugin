...
```sh
docker build -t trid3nt-local/hecras:latest workers/hecras/

# Muncie gate in-container (bind-mount a rundir seeded from the fixture):
rundir=$(mktemp -d)
cp workers/hecras/fixtures/muncie_smoke/wrk_source/*.* "$rundir/"
cp workers/hecras/fixtures/muncie_smoke/manifest.json  "$rundir/"

...
SCRATCH = Path(os.environ.get("TRID3NT_GEOCLAW_SCRATCH", "/opt/trid3nt/work"))
...
def _write_outputs_manifest(run_id: str, entries: list[dict]) -> str:
    ...
    body = _om.append_entries(None, engine="geoclaw", run_id=run_id, new=list(entries))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # workers/sfincs/entrypoint.py's _solve_postprocess_sweep):
        ...
        if rc == 0 and pp is not None and pp.status == "ok" and pp.manifest is not None:
            try:
                from workers._geoclaw_postprocess import (
                    GEOCLAW_SCRATCH_KEEP_PATTERNS,
                    GEOCLAW_SCRATCH_PATTERNS,
                )
                from workers._raster_postprocess.retention import (
                    reap_run_scratch,
                )

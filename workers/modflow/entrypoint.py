...
# Job AND AWS Batch). MIRRORS workers/sfincs/entrypoint.py verbatim:
...
# Mirrors workers/sfincs/entrypoint.py's --build-spec-uri branch.
# --------------------------------------------------------------------------- #
...
from workers._modflow_postprocess.postprocess import (  # noqa: E402
...
def _dispatch_archetype_postprocess(
    ...
    """
    from workers import _modflow_postprocess as _pp_mod
...
def _run_build_mode(run_id: str, build_spec_uri: str) -> dict:
    ...
    from workers.modflow import gwt_adapter

...
The rule: never hand a computed precision, or an unvalidated ``(vmin, vmax)``
range, to a native Qt/QGIS formatting API. Clamp the precision into a sane band
and replace a non-finite or degenerate range BEFORE the renderer sees it."""
...
def clamp_decimals(n, default: int = DEFAULT_DECIMALS) -> int:
    ...
    a non-finite or non-numeric ``n`` collapses to ``default``. EVERY site that
    feeds a native ``decimals`` argument routes through here."""
...
def sane_range(
    ...
    """A finite, strictly-increasing ``(vmin, vmax)``, or ``default`` when
    either bound is non-finite or the span is not positive. NaN defeats a plain
    ``vmax <= vmin`` guard, since every comparison with NaN is False."""
...
def is_sane_range(vmin, vmax) -> bool:
    """Whether ``(vmin, vmax)`` is already finite and strictly increasing, so
    ``sane_range`` would pass it through untouched. A caller reads it to know
    whether it had to substitute a default."""
    return (
...
def decimals_for_range(vmin, vmax, default: int = DEFAULT_DECIMALS) -> int:
    ...
    the endpoints, always inside ``[MIN_DECIMALS, MAX_DECIMALS]``. A degenerate
    or non-finite range yields ``default``, never a non-finite precision."""

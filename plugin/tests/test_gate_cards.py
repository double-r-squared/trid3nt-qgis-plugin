...
class TestGateParsing(unittest.TestCase):
    ...
    def test_summary_lines_state_the_cores(self):
        # The card states the cores the run is partitioned across, and no size
        # name: there is no tier vocabulary left to render.
        import copy

        row = copy.deepcopy(PAYLOAD_WARNING_ROW)
        row["granularity"]["vcpus"] = 8
        joined = "\n".join(gate.summary_lines(gate.parse_payload_warning(row)))
        self.assertIn("8 cores", joined)
        self.assertNotIn("vCPU", joined)
        row["granularity"]["vcpus"] = 1
        joined = "\n".join(gate.summary_lines(gate.parse_payload_warning(row)))
        self.assertIn("1 core", joined)
        self.assertNotIn("1 cores", joined)

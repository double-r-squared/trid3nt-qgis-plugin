...
from plugin.ui.cards import ToolCandidatesCard  # noqa: E402
...
assert tool_choice_sends == [
    (STUB_TOOL_CANDIDATES_REQUEST_ID, "probe_point", None)
...
assert card.summary_lbl.text() == "Step 1: picked probe_point", card.summary_lbl.text()
...
dock._on_event("chunk", {"delta": "Running probe_point."})
pump()
assert card.summary_lbl.text() == "Step 1: picked probe_point", (
    "answered chip was clobbered by the supersede sweep"
)
assert dock._open_tool_pickers == [], "answered card must leave the open list"
print("[pick] one send + lock + chip 'picked probe_point' ok")
...
dock._on_event("chunk", {"delta": "No answer -- proceeding with probe_point."})
pump()

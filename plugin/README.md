...
QGIS bundles its own Python and it carries almost everything this plugin
needs; the one gap is **matplotlib**, which QGIS 3 bundled and QGIS 4 dropped,
and which only the Charts dock uses. `install_dependencies.py` ships inside the
plugin and is the fix on Linux and Windows; on macOS QGIS's bundled Python has
no pip at all, so nothing can be installed into it and the wheels are unzipped
into the profile instead. The per-OS commands are in
[docs/site/install.md](../docs/site/install.md#plugin-dependencies-matplotlib-per-os),
and the Charts dock's "Missing matplotlib" panel shows the same block with
every value filled in for the QGIS you are running.

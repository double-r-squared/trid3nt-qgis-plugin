...
FlatGeobuf (emitted by ``pelicun_damage_assessment``) into portfolio-level
damage, loss, and population-impact statistics.

**Why a separate envelope type?**

``pelicun_damage_assessment`` returns a ``LayerURI`` pointing at a
...
- *Provenance*: every numeric claim is traceable to ``pelicun_run_id`` (the
  ULID of the ``pelicun_damage_assessment`` call), ``damage_layer_uri``
...
# labels is co-owned with ``pelicun_damage_assessment``).
...
class OccupancyClassImpact(GraceModel):
    ...
    ``component_types`` in the upstream ``pelicun_damage_assessment`` call).
...
class ImpactEnvelope(GraceModel):
    ...
    FlatGeobuf returned by ``pelicun_damage_assessment`` over all asset
    ...
    **Provenance fields:**
    ...
    - ``damage_layer_uri``: the gs:// URI of the FlatGeobuf aggregated; this
      is the ``LayerURI.uri`` returned by ``pelicun_damage_assessment``.
    - ``flood_layer_uri``: the gs:// URI of the source hazard raster (the
      ``hazard_raster_uri`` passed to ``pelicun_damage_assessment``).
    ...
    damage_layer_uri: str = Field(
        ...
        description=(
            ...
            "This is the LayerURI.uri returned by pelicun_damage_assessment."
        ),
    )
    ...
    realization_count: int = Field(
        ...
        description=(
            ...
            "pelicun_damage_assessment call."
        ),
    )

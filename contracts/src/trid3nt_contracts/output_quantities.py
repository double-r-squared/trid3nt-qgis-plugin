...
resolves this with TWO definitions gated on ONE ``schema_version``. We follow it:
  ...
  * THIS module (agent-side) holds the SPEC as PLAIN FROZEN DATACLASSES + an
    ``OUTPUT_REGISTRY_SCHEMA_VERSION``. It lives in ``trid3nt_contracts`` because
...
worker module that cannot import pydantic-heavy ``trid3nt_contracts``. A frozen
...
executor (``trid3nt_server.agent.workflows.shared.publish_quantities``) is importable + typed +

"""SFINCS regular grid and mask utilities.

Provides the SfincsGrid class (legacy regular-grid version) and helper functions
for building, reading, and visualising the SFINCS computational mesh and mask.
"""
...
class SfincsGrid:
    """SFINCS regular grid (legacy version) container.

    Stores the regular-grid mesh data and provides methods for building,
    reading, writing, and visualising the SFINCS computational grid.
    ...
    def build(
        ...
        """Build the regular/quadtree grid from scratch (legacy version).

        Constructs the computational mesh starting from a uniform base grid
        and refines it inside any supplied polygons.
        ...
        refinement_polygons : geopandas.GeoDataFrame, optional
            Polygons defining refinement zones; must have a
            ``refinement_level`` column.
        refinement_levels : ignored
        ...
        if refinement_polygons is not None:
            for irow, row in refinement_polygons.iterrows():
                        ...
                        if not isrefined[ilev][n, m]:
                            ...
                            if iok:
                                ...
                                for j in range(4):
                                    if (
                                        nnbr[j] > -1
                                        and nnbr[j] <= nmx[ilev] - 1
                                        and mnbr[j] > -1
                                        and mnbr[j] <= mmx[ilev] - 1
                                    ):
        ...
        self.data["n"] = xu.UgridDataArray(
            xr.DataArray(data=n, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["m"] = xu.UgridDataArray(
            xr.DataArray(data=m, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["level"] = xu.UgridDataArray(
            xr.DataArray(data=level, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["z"] = xu.UgridDataArray(
            xr.DataArray(data=z, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        ...
        self.data["snapwave_mask"] = xu.UgridDataArray(
            xr.DataArray(data=swmask, dims=[ugrid2d.face_dimension]), ugrid2d
        )

        print(f"Number of cells : {nb}")
    ...
    def find_neighbors(self) -> None:
        """Compute all cell-neighbour connectivity arrays from ``self.data``.

        Reads the ``n``, ``m``, and ``level`` arrays from ``self.data`` and
        populates ``mu``, ``mu1``, ``mu2``, ``md``, ``md1``, ``md2``, ``nu``,
        ``nu1``, ``nu2``, ``nd``, ``nd1``, ``nd2`` for every cell.
        ...
        self.data["nu"] = xu.UgridDataArray(
            xr.DataArray(data=nu, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["nu1"] = xu.UgridDataArray(
            xr.DataArray(data=nu1, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["nu2"] = xu.UgridDataArray(
            xr.DataArray(data=nu2, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["nd"] = xu.UgridDataArray(
            xr.DataArray(data=nd, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["nd1"] = xu.UgridDataArray(
            xr.DataArray(data=nd1, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["nd2"] = xu.UgridDataArray(
            xr.DataArray(data=nd2, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["mu"] = xu.UgridDataArray(
            xr.DataArray(data=mu, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["mu1"] = xu.UgridDataArray(
            xr.DataArray(data=mu1, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["mu2"] = xu.UgridDataArray(
            xr.DataArray(data=mu2, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["md"] = xu.UgridDataArray(
            xr.DataArray(data=md, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["md1"] = xu.UgridDataArray(
            xr.DataArray(data=md1, dims=[ugrid2d.face_dimension]), ugrid2d
        )
        self.data["md2"] = xu.UgridDataArray(
            xr.DataArray(data=md2, dims=[ugrid2d.face_dimension]), ugrid2d
        )

        print(f"Time elapsed : {time.time() - start} s")

    def set_bathymetry(self, bathymetry_sets: list, quiet: bool = True) -> None:
        ...
        for ilev in range(nlev):
            if not quiet:
                print(f"Processing bathymetry level {ilev + 1} of {nlev} ...")
    ...
    def cut_inactive_cells(self) -> None:
        ...
        Reads mask arrays from ``self.data``, discards inactive cells, then
        rebuilds the xugrid dataset for the remaining cells.
    ...
    def get_ugrid2d(
        ...
        """Build an xugrid ``Ugrid2d`` from cell (n, m, level) index arrays.

        Parameters
        ----------
        n : numpy.ndarray
            Row indices of each cell.
        m : numpy.ndarray
            Column indices of each cell.
        level : numpy.ndarray
        ...
        node_x[0 : 4 * self.nr_cells : 4] = (
            self.x0 + cosrot * (mf * dxf) - sinrot * (nf * dyf)
        )
        node_y[0 : 4 * self.nr_cells : 4] = (
            self.y0 + sinrot * (mf * dxf) + cosrot * (nf * dyf)
        )
        ## Lower right
        nf = (n) * fac
        mf = (m + 1) * fac
        nm_nodes[1 : 4 * self.nr_cells : 4] = ifac * (mf * (nmax + 1) + nf)
        node_x[1 : 4 * self.nr_cells : 4] = (
            self.x0 + cosrot * (mf * dxf) - sinrot * (nf * dyf)
        )
        node_y[1 : 4 * self.nr_cells : 4] = (
            self.y0 + sinrot * (mf * dxf) + cosrot * (nf * dyf)
        )
        ## Upper right
        nf = (n + 1) * fac
        ...
        node_x[2 : 4 * self.nr_cells : 4] = (
            self.x0 + cosrot * (mf * dxf) - sinrot * (nf * dyf)
        )
        node_y[2 : 4 * self.nr_cells : 4] = (
            self.y0 + sinrot * (mf * dxf) + cosrot * (nf * dyf)
        )
        ## Upper left
        nf = (n + 1) * fac
        ...
        node_x[3 : 4 * self.nr_cells : 4] = (
            self.x0 + cosrot * (mf * dxf) - sinrot * (nf * dyf)
        )
        node_y[3 : 4 * self.nr_cells : 4] = (
            self.y0 + sinrot * (mf * dxf) + cosrot * (nf * dyf)
        )
    ...
    def get_datashader_dataframe(self) -> None:
        ...
        3857) and stores the result in ``self.df``.
    ...
    def map_overlay(
        ...
        -------
        bool
            ``True`` on success, ``False`` if no grid data is present.
        """
    ...
    def read(self, file_name: str | None = None) -> None:
        ...
        # Should not really do this here, as all this is also stored in the attributes of the xu grid
    ...
    def make_index_tiles(
        ...
        """Write map-tile PNG files mapping pixel positions to cell indices.

        Parameters
        ----------
        path : str
            Output directory; tiles are written to ``<path>/<zoom>/<x>/<y>.png``.
        zoom_range : list[int], optional
            ``[min_zoom, max_zoom]`` tile zoom levels to generate.
            Defaults to ``[0, 13]``.
        format : int, optional
            Tile format flag (currently unused).  Defaults to ``0``.
                    ...
                    for ilev in range(self.nr_refinement_levels):
                        ...
                        if incell[0].size > 0:
                            ...
                            try:
                                cell_indices = (
                                    np.searchsorted(
                                        nm_lev[ilev], ind[incell[0], incell[1]]
                                    )
                                    + i0_lev[ilev]
                                )
    ...
    def get_uv_points(self) -> "np.ndarray":
        """Build the UV-point index array from the neighbour connectivity data.

        Returns
        -------
        numpy.ndarray
            2-D integer array of shape ``(2, nuv)`` where each column holds
            the cell index pair ``[cell_z, cell_nmu]`` for one velocity point.
        """
...
def get_neighbors_in_larger_cell(n: int, m: int) -> tuple:
    """Return the (n, m) indices of a cell's three sibling cells in the parent.

    Given the fine-level indices *(n, m)*, this function returns the three
    other fine cells that share the same coarse parent cell.

    Parameters
    ----------
    n : int
        Row index of the cell at the fine level.
    m : int
        Column index of the cell at the fine level.

    Returns
    -------
    tuple[list[int], list[int]]
        ``(nnbr, mnbr)`` – lists of length 4 with the row and column
        indices of the sibling cells; unused slots are ``-1``.
    """
    nnbr = [-1, -1, -1, -1]
    mnbr = [-1, -1, -1, -1]

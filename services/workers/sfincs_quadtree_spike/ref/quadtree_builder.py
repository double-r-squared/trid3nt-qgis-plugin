"""SFINCS quadtree mesh builder.

Provides the build_quadtree_xugrid function and helpers for constructing
an xugrid-based quadtree mesh from a base regular grid with optional
patch refinements.
"""
...
def build_quadtree_xugrid(
    ...
    """Build a quadtree mesh and return it as an xugrid ``UgridDataset``.

    Convenience wrapper around :class:`QuadtreeGrid` that constructs the mesh,
    optionally refines it inside *refinement_polygons*, and returns the
    resulting dataset.
    ...
    crs : pyproj.CRS
        Coordinate reference system to attach to the grid.
    ...
    bathymetry_sets : list, optional
        Bathymetry dataset identifiers passed to the database.
    ...
    -------
    xugrid.UgridDataset
        Fully assembled quadtree mesh dataset.
    """
...
def cut_inactive_cells(data: "xu.UgridDataset") -> "xu.UgridDataset":
    """Remove inactive cells (mask == 0) from a quadtree xugrid dataset.

    Parameters
    ----------
    data : xugrid.UgridDataset
        Quadtree dataset as produced by :func:`build_quadtree_xugrid`.

    Returns
    -------
    xugrid.UgridDataset
        Dataset with inactive cells removed.
    """
...
class QuadtreeGrid:
    """Internal helper that builds and stores a SFINCS quadtree mesh.

    Parameters
    ----------
    data : xugrid.UgridDataset, optional
        Pre-existing dataset to initialise from.  When ``None`` the grid must
        be built via :meth:`build` before use.
    """

    def __init__(self, data: "xu.UgridDataset | None" = None) -> None:
        self.data = data
    ...
    def build(
        ...
        refinement_polygons : geopandas.GeoDataFrame or None
            Refinement polygon GeoDataFrame with a ``refinement_level`` column.
        bathymetry_sets : list or None
            Bathymetry dataset identifiers.
        bathymetry_database : optional
    ...
    def refine_in_polygon(self, polygon: dict) -> None:
        ...
        ----------
        polygon : dict
            Dictionary with keys ``"geometry"``, ``"refinement_level"``, and
            optional ``"zmin"``/``"zmax"``.
    ...
    def refine_cells(self, ind_ref: "np.ndarray", ilev: int) -> None:
        """Split cells at level *ilev* into four child cells.
    ...
    def find_first_cells_in_level(self) -> None:
        """Populate ``ifirst`` and ``ilast`` arrays for each refinement level.
    ...
    def find_lower_level_neighbors(
        ...
        """Find coarser-level neighbours of cells to be refined.
        ...
        -------
        numpy.ndarray
            Global indices of coarser-level neighbours that also need
            refinement.
        """
    ...
    def get_bathymetry_min_max(
        ...
        """Compute the min and max bathymetry across the four corners of each cell.

        Used during mesh refinement to apply depth-based refinement criteria.
        ...
        tuple[numpy.ndarray, numpy.ndarray]
            ``(zmin, zmax)`` arrays of minimum and maximum depth for each cell.
        """
...
def interp2(
    ...
    """Interpolate a regular grid onto scattered target points.

    Parameters
    ----------
    x0 : numpy.ndarray
        1-D x-axis coordinates of the source grid (must be monotonically
        increasing).
    y0 : numpy.ndarray
        1-D y-axis coordinates of the source grid.
    z0 : numpy.ndarray
    ...
    -------
    numpy.ndarray
        Interpolated values at *(x1, y1)*; ``NaN`` for out-of-bounds points.
    """

...
uploaded. ``services/workers/sfincs/entrypoint.py`` has the correct order
...
from services.workers.geoclaw import entrypoint as ep
...
def test_postprocess_runs_before_sweep_and_reap_runs_after(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setitem(
        __import__("sys").modules, "services.workers._geoclaw_postprocess", postprocess_mod,
    )
    monkeypatch.setitem(
        __import__("sys").modules, "services.workers._raster_postprocess.retention", retention_mod,
    )

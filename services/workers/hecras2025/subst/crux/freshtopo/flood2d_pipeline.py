...
def author_and_compose(
    ...
    *,
    ...
    ``computation_interval`` overrides the 2MIN time step (the stability knob)."""
...
def main() -> int:
    ...
    ap.add_argument("--computation-interval", default=None,
                    help="time step override, e.g. 30SEC / 1MIN / 5MIN (default 2MIN)")
    args = ap.parse_args()
    result, metrics = run_flood2d(
        ...
        equation_set=args.equation_set, computation_interval=args.computation_interval,
    )

...
def test_upload_s3_uses_content_type_when_set(tmp_path: Path) -> None:
    ...
    with (
        patch("trid3nt_server.data.cache.storage_scheme", return_value="s3"),
        patch("trid3nt_server.data.simulation.solver.solver._get_s3_client", return_value=fake_client),
    ):
...
def test_upload_gs_no_bucket_with_fallback_returns_file_uri(tmp_path: Path) -> None:
    ...
    cog.write_bytes(b"tiff")
    with (
        patch("trid3nt_server.data.cache.storage_scheme", return_value="gs"),
...
def test_upload_non_s3_scheme_no_fallback_raises_typed(tmp_path: Path) -> None:
    ...
    cog = tmp_path / "x.tif"
    cog.write_bytes(b"tiff")
    with patch("trid3nt_server.data.cache.storage_scheme", return_value="gs"):
        with pytest.raises(CogIoError) as ei:
            cog_io.upload_cog(

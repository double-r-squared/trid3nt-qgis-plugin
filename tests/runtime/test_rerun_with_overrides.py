...
def test_the_cut_is_the_first_node_the_override_reaches():
    """A threshold the sheet reads leaves everything before the fill inheritable."""
    from trid3nt_server.tools import TOOL_REGISTRY

    wf = TOOL_REGISTRY["telemac_do_sag"].fn.workflow
    labels = [n.label for n in _nodes(wf)]

    cut, keep = reuse_plan(wf.plan, wf.data, ("do_standard_mgl",))
...
def test_a_mesh_override_cuts_earlier_than_a_physics_one():
    ...
    cuts further back than re-asking a threshold the sheet reads."""
    from trid3nt_server.tools import TOOL_REGISTRY

    wf = TOOL_REGISTRY["telemac_do_sag"].fn.workflow
    physics, _ = reuse_plan(wf.plan, wf.data, ("do_standard_mgl",))
...
def _probe_workflow(*, extra: tuple = (), validity: tuple = ()) -> Workflow:
    ...
    return Probe(
        ...
        template=SimpleNamespace(STEPS=plan),
        validity=validity)

...
No docker / no TELEMAC / no S3: local-frame fields from the ``telemac_result``
fixture + a monkeypatched COG write/upload seam. The georef math (offset
reconstruction + reproject) runs for real; only the raster bytes + upload are
stubbed.
"""
...
def _local_frame_field(telemac_result, x_local, y_local, hs, *, ikle):
    """A single-frame WAVE HEIGHT field in LOCAL (origin-shifted) metres."""
    return telemac_result(varnames=["WAVE HEIGHT"], x=x_local, y=y_local,
                          ikle=ikle, times=[0.0], data={"WAVE HEIGHT": [hs]})
...
def _marquette_local_mesh():
    ...
    # a trivial triangulation just so the raster has real elements
    ikle = [[0, 1, 2]]
...
def test_published_cog_bounds_fall_inside_request_bbox(
        tmp_path, monkeypatch, telemac_result):
    x_local, y_local, hs, ikle, bbox, utm_epsg = _marquette_local_mesh()
    slf = tmp_path / "agit_field.slf"
    _local_frame_field(telemac_result, x_local, y_local, hs, ikle=ikle)
...
def test_georef_needs_request_bbox_when_utm_set(tmp_path, monkeypatch, telemac_result):
    x_local, y_local, hs, ikle, bbox, utm_epsg = _marquette_local_mesh()
    slf = tmp_path / "agit_field.slf"
    _local_frame_field(telemac_result, x_local, y_local, hs, ikle=ikle)
    _stub_cog_seam(monkeypatch, tmp_path)
...
def test_idealized_local_frame_unaffected(tmp_path, monkeypatch, telemac_result):

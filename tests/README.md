...
| `runtime/` | the declarative runtime, its engine-neutral slots and levers, the run journal | 9 | 297 |
| `scripts/` | the dev instruments, the live-run harness and the proof renderers, skipped when `dev/` is absent | 9 | 90 |
...
| `telemac/` | the TELEMAC templates, the module surface, what each module writes and the primitives that read it, the listing reads, authoring | 40 | 730 |
...
Six slices by subsystem, each its own foreground invocation, from the repo root:
    ...
    make test-engines         # tests/telemac tests/runtime tests/solver tests/search                               1289

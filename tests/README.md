...
| `telemac/` | the TELEMAC templates, the module surface, what each module writes and the primitives that read it, the listing reads, authoring | 27 | 617 |
...
Six slices by subsystem, each its own foreground invocation, from the repo root:
    ...
    make test-engines         # tests/telemac tests/runtime tests/solver tests/search                               1143

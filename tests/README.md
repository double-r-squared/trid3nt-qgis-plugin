...
| `inputs/` | the typed inputs: a Point, an Extent, a Shape, each from every form it arrives in, the AOI acquired from any of them, and a user's own file adopted as a layer | 5 | 51 |
...
Six slices by subsystem, each its own foreground invocation, from the repo root:
    ...
    make test-engines         # tests/telemac tests/runtime tests/solver tests/search   1100
    make test-server          # tests/server tests/inputs tests/gates tests/credentials tests/sandbox tests/model tests/scripts   1117

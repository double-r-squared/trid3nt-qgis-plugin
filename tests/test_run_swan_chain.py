...
def test_fetch_bathy_accepts_real_bathymetry():
    from trid3nt_server.workflows.swan.wave_field.wave_field import _fetch_bathy_for_swan
...
def test_fetch_bathy_typed_error_when_topobathy_fails():
    ...
    with patch(
        "trid3nt_server.workflows.swan.wave_field.wave_field.fetch_topobathy", _boom
    ):
        with pytest.raises(SwanComposerError) as ei:
            _fetch_bathy_for_swan(bbox)
...
def test_swan_wave_height_preset_resolves_to_titiler_rescale_colormap():
    ...
    params = resolve_style("continuous_wave_height").style_params()
...
def test_composer_arg_assembly_and_dispatch(tmp_path: Path):
    ...
    from trid3nt_server.workflows.swan.wave_field import wave_field as comp

...
Five network fetches used to run inside the solver container - an NLDI position
snap, two NHDPlus_HR flowline re-seeds, the NLDI navigate that IS the model
centerline and an NHDArea bank query. They are server tier now, and these pin
what that bought. The BED is not among them any more either: the mesh ask
declares its own, so the bed a run solves on is the one its geometry file
carries.
...
class _World:
    ...
    def registry_fn(self, name):
        ...
        return {"fetch_nhdplus_hr_flowlines": _flowlines,
                ...
                "fetch_nhdplus_nldi_navigate": _navigate}[name]
...
# What gets staged
# --------------------------------------------------------------------------- #
def test_the_two_inputs_land_under_the_names_the_worker_reads():
    out = _World().run()
    assert [row["dest"] for row in out["inputs"]] == [
        R.CENTERLINE_DEST, R.BANKS_DEST]

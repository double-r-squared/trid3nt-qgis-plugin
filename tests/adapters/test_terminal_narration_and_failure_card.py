"""Terminal closing narration, and the terminal-failure replay card.

A turn that accumulated narration but never streamed a segment opens ONE final
agent-message, streams it and closes ``done=True`` - and only when there IS
narration, so an empty turn still emits no bubble. A terminal generation failure
persists a FAILED tool-card row, so a replay never shows one still running."""
...
async def test_accumulated_narration_no_open_segment_emits_terminal_done(
    ...
    """Accumulated narration with no open wire segment still reaches the user.

    A terminal ``done=True`` agent-message carries it and it persists as the closing
    agent row; the fake stream fills the accumulator and yields no events."""
    ws = FakeWS()
...
async def test_empty_narration_turn_emits_no_bubble(file_persistence) -> None:
    """A turn with NO accumulated narration and no open segment emits no closing chunk.

    The narration-less marker row is allowed but carries empty content - never a
    fabricated summary."""
    ws = FakeWS()
    state = server.SessionState(session_id=new_ulid())
    case_id = await _create_case(ws, state)
...
async def test_segments_already_streamed_no_double_narration(
    ...
    """Narration already streamed this turn is never re-streamed at the end.

    The recovery branch would otherwise double the closing text on the wire and
    duplicate the chat rows; exactly one narration bubble survives."""
...
async def test_terminal_model_failure_persists_failed_tool_card(
    ...
    """A model-generation failure persists a tool row whose card state is ``failed``.

    A later reopen replays the FAILED card instead of one stuck running; the error
    code and message ride in the row content, the card record having no error fields."""
    ws = FakeWS()
    state = server.SessionState(session_id=new_ulid())
    case_id = await _create_case(ws, state)
    state.current_turn_case_id = case_id

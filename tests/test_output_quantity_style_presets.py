...
registered in ``publish_layer._QGIS_STYLE_REGISTRY`` silently falls through to
...
The check is the EXACT-KEY leg of the resolver (``_QGIS_STYLE_REGISTRY``):
...
def test_registry_entries_are_well_formed() -> None:
    ...
    for preset in referenced:
        rescale, cmap = _QGIS_STYLE_REGISTRY[preset]
        lo, hi = rescale.split(",")

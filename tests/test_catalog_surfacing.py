...
def test_default_config_identity():
    ...
    assert r["registry_size"] == 254  # ADR 0276 -2 list_categories+list_tools_in_category chop; ADR 0259 +coastal_tidal_surge (TELEMAC-2D coastal tidal/surge inundation); ADR 0251 +culvert_embankment_flow (HEC-RAS 2D culvert-through-embankment); ADR 0247 +pelicun_hazus_lifeline_seismic_dl_run
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 254  # ADR 0276 -2 list_categories+list_tools_in_category chop; ADR 0259 +coastal_tidal_surge; ADR 0251 +culvert_embankment_flow; ADR 0247 +lifeline template; registry does NOT shrink, only the pool does

...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 101  # TELEMAC wave B +fetch_osm_breakwaters leaves the arm-ON pool too; ADR 0298 +fetch_water_table_depth +fetch_aquifer_thickness +fetch_aquifer_transmissivity; ADR 0297 +fetch_groundwater_recharge; ADR 0112 +nwm_streamflow; ADR 0203 +aorc_precip +lter_records leave the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 101  # 102 specs minus the internal copernicus seam (TELEMAC wave B +fetch_osm_breakwaters) (ADR 0298 +fetch_water_table_depth +fetch_aquifer_thickness +fetch_aquifer_transmissivity) (ADR 0297 +fetch_groundwater_recharge) (ADR 0112 +nwm_streamflow; ADR 0203 +aorc_precip +lter_records)

...
def test_default_config_identity():
    ...
    assert r["registry_size"] == 255  # ADR 0297 +fetch_groundwater_recharge (staged CONUS recharge spec); ADR 0276 -2 list_categories+list_tools_in_category chop; ADR 0259 +coastal_tidal_surge (TELEMAC-2D coastal tidal/surge inundation); ADR 0251 +culvert_embankment_flow (HEC-RAS 2D culvert-through-embankment); ADR 0247 +pelicun_hazus_lifeline_seismic_dl_run
    assert r["n_specs"] == 98  # ADR 0297 +fetch_groundwater_recharge (staged-dataset spec); ADR 0112 +nwm_streamflow (fetcher finale); ADR 0203 +fetch_aorc_precip +fetch_lter_records
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 255  # ADR 0297 +fetch_groundwater_recharge (staged CONUS recharge spec); ADR 0276 -2 list_categories+list_tools_in_category chop; ADR 0259 +coastal_tidal_surge; ADR 0251 +culvert_embankment_flow; ADR 0247 +lifeline template; registry does NOT shrink, only the pool does
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 97  # ADR 0297 +fetch_groundwater_recharge; ADR 0112 +nwm_streamflow; ADR 0203 +aorc_precip +lter_records leave the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 97  # 98 specs minus the internal copernicus seam (ADR 0297 +fetch_groundwater_recharge) (ADR 0112 +nwm_streamflow; ADR 0203 +aorc_precip +lter_records)

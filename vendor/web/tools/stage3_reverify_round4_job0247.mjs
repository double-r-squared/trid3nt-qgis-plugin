#!/usr/bin/env node
// GRACE-2 — job-0247 Stage 3 re-verify ROUND 4 — LIVE, ONE browser session.
//
// Runs after the context-carryover fix (commit 74fc0d6): Case create/select now
// CLEARS the per-connection LLM conversation (state.chat_history + turn_count).
// Round-3 proved every post-switch prompt mis-routed to the prior Case's Twin-Falls
// groundwater composer; this round re-verifies B + C now start genuinely clean.
//
// Scenario A (Case-2 render) is OUT OF SCOPE this round (user-gated QGIS cache env).
//
// Scenario B — flood + P5 + analysis + charts (FRESH Case):
//   B1: "Model flood damage for Fort Myers using the existing flood layer."
//       PRIMARY ASSERT (context_isolation): NO Twin-Falls/groundwater gate. A
//       groundwater/MODFLOW/TCE route = context-carryover REGRESSION -> FAIL.
//       EXPECT: flood/Pelicun chain -> ImpactPanel with headline numbers [P5].
//   B2: "How many structures are impacted above damage state 2?" -> narrated count.
//   B3: "Show me the damage distribution as a chart." -> chart-emission -> ChartStack -> gallery.
//   B4: reload + reselect Case -> charts replay (Gemini-free).
// Scenario C — sandbox live gate (FRESH Case):
//   "Run a quick Python computation: compute the mean and max of the numpy array
//    [1, 5, 9, 12] and print both." (self-contained, no layer dependency)
//   EXPECT: code-exec-request -> SandboxCard (Proceed/Cancel) -> Proceed -> LOCAL
//           exec -> status=ok -> narration with real numbers (mean 6.75, max 12).
//   WS ordering assert: code-exec-request BEFORE any sandbox spawn.
//
// LIVE-DRIVEN ONLY: NO __grace2Inject* seams. Read-only __grace2GetMap permitted.
// <=9 Gemini turns total. On ANY 429 -> stop, mark remaining BLOCKED.
...
const OUT_DIR =
  "/home/nate/Documents/GRACE-2/reports/inflight/job-0247-testing-20260610/evidence";
...
// Context-carryover regression sentinel: any sign the agent routed to the prior
// Case's Twin-Falls groundwater composer / TCE / MODFLOW.
const GW_RE = /twin.?falls|groundwater|trichloro|\btce\b|modflow|contaminat|gwt|snake river|idaho/i;
    ...
    ws.on("framesent", (data) => {
      try {
        ...
        if (type === "user-message" || type === "tool-payload-confirmation" || type === "chat-message" ||
            type === "create-case" || type === "open-case")
...
// Scan inbound WS frames (within a window) for any groundwater/Twin-Falls route signal.
function scanGroundwaterRoute(startIdx = 0) {
  const hits = [];
  for (let i = startIdx; i < wsFrames.length; i++) {
    const f = wsFrames[i];
    if (typeof f.type === "string" && f.type.startsWith("SENT:")) continue; // ignore our own sends
    const hay = JSON.stringify(f.tool_name ?? "") + " " + (f.preview ?? "");
    if (GW_RE.test(hay)) hits.push({ idx: i, type: f.type, tool_name: f.tool_name, snippet: (f.preview ?? "").slice(0, 240) });
  }
  return hits;
}

// Scan inbound frames for a flood / pelicun / damage route signal.
const FLOOD_RE = /flood|sfincs|pelicun|damage|inundat|fort myers|depth/i;
function scanFloodRoute(startIdx = 0) {
  const hits = [];
  for (let i = startIdx; i < wsFrames.length; i++) {
    const f = wsFrames[i];
    if (typeof f.type === "string" && f.type.startsWith("SENT:")) continue;
    const hay = JSON.stringify(f.tool_name ?? "") + " " + (f.preview ?? "");
    if (FLOOD_RE.test(hay)) hits.push({ idx: i, type: f.type, tool_name: f.tool_name, snippet: (f.preview ?? "").slice(0, 200) });
  }
  return hits;
}
...
async function main() {
  ...
  console.log("=== job-0247 Stage 3 re-verify ROUND 4 — LIVE ONE-SESSION (B + C) ===");
  let turns = 0;
  ...
  try {
    ...
    // ============================================================= SCENARIO B
    console.log("\n##### SCENARIO B — flood + P5 + analysis + charts #####");
    ...
    const sendB1 = await sendPrompt(page,
      "Model flood damage for Fort Myers using the existing flood layer.");
    turns++;
    findings.scenarios.B.b_frames_start = bFramesStart;
    await flush();

    // STALL-GUARD + route watch.
    let b1Stalled = false;
    {
      const b1T0 = Date.now();
      const B1_MAX = 900000;
    ...
    // ----- PRIMARY ASSERT: context_isolation -----
    const gwHits = scanGroundwaterRoute(bFramesStart);
    const floodHits = scanFloodRoute(bFramesStart);
    findings.scenarios.B.context_isolation = {
      groundwater_route_hits: gwHits.slice(0, 12),
      groundwater_hit_count: gwHits.length,
      flood_route_hit_count: floodHits.length,
      flood_route_sample: floodHits.slice(0, 6),
      // A groundwater gate appearing in chat would also surface as a payload-warning;
      // capture that explicitly too.
      payload_warning_inline_count: await page.locator('[data-testid="payload-warning-inline"]').count(),
    };
    // capture any gate text for diagnosis
    if (findings.scenarios.B.context_isolation.payload_warning_inline_count > 0) {
      findings.scenarios.B.context_isolation.gate_text = await page.evaluate(() => {
        const c = document.querySelector('[data-testid="payload-warning-inline"]');
        return { tool: document.querySelector('[data-testid="payload-warning-tool"]')?.textContent ?? null, full: (c?.textContent ?? "").slice(0, 600) };
      });
    }
    await page.screenshot({ path: `${OUT_DIR}/B02_route_decision.png` });
    console.log(`[B1] context_isolation: gw_hits=${gwHits.length} flood_hits=${floodHits.length}`);
    await flush();

    // If a flood-side payload warning (large response) gate is up, Proceed it.
    {
      ...
      if ((await gate.count()) > 0) {
        ...
        // give the chain time to continue after proceed
        await waitForTurnSettle(page, { sendT: sendB1, maxMs: 600000, label: "B1-postgate" });
        await page.waitForTimeout(3000);
      }
    }
    ...
    if (impactCount > 0) {
      ...
      await page.screenshot({ path: `${OUT_DIR}/B03_impact_panel_P5.png` });
      console.log("[B1] IMPACT PANEL:", JSON.stringify(impact));
    } else {
      await page.screenshot({ path: `${OUT_DIR}/B03_no_impact_panel.png` });
      findings.scenarios.B.b1_scroll_tail = ((await chatText(page)).scroll_text ?? "").slice(-2800);
    }
    ...
    findings.scenarios.B.b1_narration_tail = ((await chatText(page)).scroll_text ?? "").slice(-2500);
    await flush();
    ...
    if (doB23) {
      ...
      console.log("[B] B1 produced no impact (stalled=" + b1Stalled + "); SKIPPING B2/B3, proceeding to C.");
    ...
    await flush();
    let sandboxReqT = null, execReqFrameIdx = null, sandboxSpawnFrameIdx = null;
    ...
    while (Date.now() - cWatch0 < 300000) {
      ...
      // ordering: find first code-exec-request and first code-exec-result indices
      ...
      if (sandboxSpawnFrameIdx === null) {
        const idx = wsFrames.findIndex((f, i) => i >= cFramesStart && f.type === "code-exec-result");
        if (idx >= 0) sandboxSpawnFrameIdx = idx;
      }
    ...
    // context-isolation sentinel for C as well
    const cGwHits = scanGroundwaterRoute(cFramesStart);
    findings.scenarios.C.groundwater_route_hits = cGwHits.slice(0, 8);
    ...
    if (sandboxReqT) {
      ...
      // ordering assert: code-exec-request must precede code-exec-result
      ...
      const execResultFrame = resIdx >= 0 ? wsFrames[resIdx] : null;
      findings.scenarios.C.code_exec_result_frame_preview = execResultFrame?.preview ?? null;
      // narration sanity: mean 6.75 and max 12 expected
      const ct = (findings.scenarios.C.c_scroll_tail ?? "") + " " + (findings.scenarios.C.result_payload ?? "");

#!/usr/bin/env node
// GRACE-2 — job-0252 Stage 3 ROUND 7 (P5 FINAL) — LIVE, ONE browser session.
//
// Runs after the postprocess_flood fix (commit 0b35791, job-0251-agent):
// OQ-0250 (gcsfs 0.8.0 NoOpCallback crash) is FIXED — postprocess_flood now
// downloads run outputs via google-cloud-storage; fsspec/gcsfs/storage family
// harmonized (2026.1.0 / 2026.1.0 / 3.11). Proven Gemini-free against the REAL
// round-6 solve (run 01KTRZSPY7 -> flood_depth_peak.tif).
//
// Everything upstream is proven across rounds 1-6 (routing, gates, registry,
// context isolation, MODFLOW chain, plume render, sandbox, SFINCS build+solve).
// THIS round verifies ONLY the tail:
//   postprocess -> publish -> flood layer renders -> Pelicun -> ImpactPanel ->
//   analysis count -> chart emission -> chart replay.
//
// THE scenario (FRESH Case):
//   P5  — "Run a flood damage assessment for Fort Myers with Pelicun using the
//          NSI building inventory and the existing Fort Myers flood depth layer."
//          Clarify once -> "use the NSI inventory" if asked.
//          EXPECT: chain completes; postprocess succeeds; flood layer publishes +
//          renders; Pelicun runs against REAL layer URI; ImpactPanel slides out
//          with headline numbers. [P5 EVIDENCE]  Screenshot flood layer + panel.
...
// GetMap until the periodic project cache refresh. If the layer publishes but the
// map overlay 404s, that is the KNOWN user-gated item (job-0245 USER_UNBLOCK.md):
// record layer-panel entry + GetCapabilities, DO NOT fail the scenario on the
// overlay alone. The ImpactPanel/Pelicun legs are the gates.

import { chromium } from "@playwright/test";
import { mkdir, writeFile } from "fs/promises";

const OUT_DIR =
  "/home/nate/Documents/GRACE-2/reports/inflight/job-0252-testing-20260610/evidence";
    ...
    ws.on("framereceived", (data) => {
      try {
        ...
        if (type && (KEEP.has(type) || isChunk)) {
          ...
          wsFrames.push({ t_rel_ms: rel(), type, tool_name: toolName, preview: t.slice(0, isChunk ? 240 : 2200) });
        }
      } catch {}
    });
...
// publish/postprocess evidence: a successful publish_layer for a flood-depth layer
...
// Inspect the LayerPanel DOM for a flood-depth layer entry (independent of the
// MapLibre overlay, which can lag on QGIS cache).
...
async function main() {
  ...
  console.log("=== job-0252 Stage 3 ROUND 7 (P5 FINAL) LIVE — postprocess fix verify ===");
  let turns = 0;
  ...
  try {
    ...
    {
      const b1T0 = Date.now();
      const B1_MAX = 1500000; // 25 min ceiling (SFINCS re-solve + postprocess + publish + Pelicun + narration)
      let bhb = -1;
      // ROUND-7 FIX: the settle gate must be relative to the MOST-RECENT user
      // send (lastSendT) AND require new frames after a clarification. Round-6's
      // bug: it gated on the first prompt's send time + stale quiesce, so the
      // loop "settled" 1s after sending the clarification, never awaiting turn 2.
      let lastSendT = sendB1;
      let framesAtSend = wsFrames.length;
      const SETTLE_AFTER_SEND_MS = 25000;
      while (Date.now() - b1T0 < B1_MAX) {
        if (rateLimited) break;
        const st = await turnState(page);
        // require: quiet, idle, not generating, >25s since the latest send, AND
        // at least one new inbound frame produced since that send (so a stale
        // quiesce from the prior turn cannot satisfy the gate).
        const sawNewFrames = wsFrames.length > framesAtSend;
        if (st.sawSignal && !st.generating && st.running === 0 && st.disabled === false &&
            st.msSinceFrame > QUIESCE_MS && Date.now() - lastSendT > SETTLE_AFTER_SEND_MS && sawNewFrames) {
          await page.waitForTimeout(4000);
          const st2 = await turnState(page);
          if (st2.running === 0 && st2.disabled === false && !st2.generating && st2.msSinceFrame > QUIESCE_MS) {
            ...
            if (impactNow === 0 && !answeredClarify && asksClarify) {
              ...
              lastSendT = csend;                 // gate now relative to clarification
              framesAtSend = wsFrames.length;     // require fresh frames for turn 2
              await flush();
    ...
    // (a) map-command frame referencing a flood-depth WMS layer
    ...
    if (floodMapCmd) findings.scenarios.B.flood_map_command_preview = floodMapCmd.preview;
    // (b) layer-published frame
    const layerPub = wsFrames.slice(bFramesStart).find((f) => f.type === "layer-published" && /flood/i.test(f.preview ?? ""));
    if (layerPub) findings.scenarios.B.layer_published_preview = layerPub.preview;
    // (c) LayerPanel DOM has a flood entry
    ...
    // (d) MapLibre overlay actually carries a flood source/layer
    ...
    // flood_layer_published = published (map-command OR layer-published OR layer-panel entry)
    ...
    // dedicated flood-layer-on-map screenshot

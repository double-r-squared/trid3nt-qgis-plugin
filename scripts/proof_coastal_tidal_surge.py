#!/usr/bin/env python
"""QGIS-true proof render for the coastal tidal/surge inundation worker.

Renders the LIVE A/B pair (Apalachicola Bay, CO-OPS 8728690):
  A = Hurricane Michael OBSERVED surge series, B = calm astronomical PREDICTION,
...
the mesh wireframe over ESRI World Imagery. Persisted to docs/proof/templates/.

Env: SP = the coastal_ab dir holding A/ and B/ (each with res_coastal.slf +
telemac_metrics.json). Run from repo root with venvs/agent active.
"""
...
def _load_case(run_dir, bbox, utm_epsg, init_wl):
    """Peak inundation depth over LAND + mesh, georeferenced to 3857.

    Peak stage per node = max FREE SURFACE across frames; depth = stage - bottom;
    land cells only (bottom > init_wl); dry cells masked out of the fill."""
    ...
    bot = _var("BOTTOM")[-1]
    peak = np.asarray(sfc).max(axis=0)                 # per-node peak stage
    depth = peak - bot
    ...
    # keep only NEWLY-flooded land cells: all 3 nodes dry at t0 (bottom>init_wl)
    # AND all 3 wet at peak -- exactly the flooded_land_km2 discriminator.
...
def main():
    ...
    fig.suptitle(
        "Apalachicola Bay, FL -- coastal tidal/surge inundation (TELEMAC-2D, real NOAA DEM_all "
        "topobathy, ocean boundary forced by the LIQUID BOUNDARIES FILE SL(1))\n"
        f"REAL NOAA CO-OPS 8728690 water-level series: Hurricane Michael surge floods "
        f"{ratio:.0f}x more land than the calm tide "
    ...
    out = os.path.join(os.path.dirname(__file__), "..", "docs", "proof", "templates",
                       "coastal_tidal_surge_boundary_forcing.png")

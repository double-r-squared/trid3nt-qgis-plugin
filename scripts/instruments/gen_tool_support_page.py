...
JSONL_PATH = REPO_ROOT / "run" / "sweep" / "tool-sweep-results.jsonl"
CHECKLIST_PATH = REPO_ROOT / "run" / "sweep" / "tool-sweep-checklist.md"
...
def render(latest: dict[str, dict]) -> str:
    ...
    lines.append(
        "    Generated by `scripts/instruments/gen_tool_support_page.py` in the `trid3nt-local` repo from"
    )
    lines.append(
        "    `scripts/instruments/tool_sweep.py`'s own record (the 3-pass direct-execution tool sweep,"
    )
    lines.append(
...
def main() -> int:
    ...
    # The sweep record is transient: a clone has the page but not the record it
    # was cut from. Without a record there is nothing to regenerate against, so
    # the committed page stands rather than being rewritten from nothing.
    if not JSONL_PATH.exists():
        print(f"[gen_tool_support_page] no sweep record at {JSONL_PATH}; "
              f"{OUT_PATH} left as committed (re-run scripts/instruments/tool_sweep.py)")
        return 0

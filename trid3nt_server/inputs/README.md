...
value, a selected layer - and is read in one. Each kind here has
...
| `point.py` | `Point` - one location with an optional name - and `point`, its ingestion from a pick, a pair, a `"lat,lon"` string, or a point layer (a place name refuses, naming `geocode_location`); the pick-or-wire coercion; containment in a domain, the move onto a wet node, the UTM projection and the context layer. |
| `extent.py` | `Extent` - one lon/lat box with an optional name - and `extent`, its ingestion from a bbox pick, the canvas AOI, four numbers or a layer's bounds (a place name refuses, naming `geocode_location`); and what a box means against another - the same extent, or overlapping ground. |
...
| `series.py` | `align` - one measured record onto the clock a reader reads it on: the quantity class picks the move (a rate conserved, a state interpolated, a class label to its nearest neighbour), the unit is converted, a hole wider than the bound refuses, and the stamp says what was done; `quantity_class` reads that class off a source's own coverage row. |

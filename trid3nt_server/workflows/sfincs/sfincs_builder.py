"""SFINCS model setup via HydroMT (§4 - Invariant 7 mitigation).
...
contract in ``docs/decisions/oq-4-hydromt-depth.md`` §4 "Immediate ".

The headline of this module - and the safety-critical invariant -
is the **NLCD vintage validation gate**:
    ...
    incorrect roughness in cells where the mapping failed. - §2
...
Decision §3 "Selected: Option A - Full HydroMT" (approved):
...
logger = logging.getLogger("trid3nt_server.workflows.sfincs.sfincs_builder")


# --------------------------------------------------------------------------- #
# GDAL/rasterio env: stabilize remote-raster reads + kill gcsfs
# --------------------------------------------------------------------------- #
#
# headline: the agent process was crashing mid-run with
#
...
class InfiltrationForcing:
    ...
    the missing forcing archetype. Infiltration is a loss, not
...
class ForcingSpec:
    ...
    # ``breach`` is an INTERIOR levee-breach point source (reuses
    ...
    # infiltration is a LOSS term (scsfile/qinffile/qinf), not a
    ...
    def has_surge_forcing(self) -> bool:
        ...
        ``breach`` is a DRIVER (an interior discharge jet) so it
...
class BuildOptions:
    ...
    # advanced-physics overrides (advection/theta/alpha/huthresh
...
def validate_nlcd_vintage_against_mapping(
    ...
    """The ** §4 Invariant-7 mitigation gate**.
...
def _stage_gcs_local(uri: str) -> str:
    ...
    HydroMT's data adapter stats every
    ...
    GCP is decommissioned (GCP-teardown): ``s3://`` URIs stage via
    **boto3** through the solver module's shared S3 client seam
    (``tools.simulation.solver._get_s3_client`` - boto3 NOT s3fs, the
    ...
    cache_dir = _Path(tempfile.gettempdir()) / "trid3nt-hydromt-stage"
...
def _write_hydromt_reclass_table_csv(
    ...
    (load-bearing for ``load_manning_mapping`` + the §4 validation gate);
...
def _default_setup_uri(bbox: tuple[float, float, float, float]) -> str:
    ...
    contract in ``workers/sfincs/entrypoint.py``:
    ...
    (``TRID3NT_STORAGE_BACKEND=s3`` → ``s3://``, default ``gs://`` unchanged)
...
# --------------------------------------------------------------------------- #
# Domain-adaptive active-cell mask bounds
# --------------------------------------------------------------------------- #
...
# ``setup_mask_active`` elevation window - NOT the raw bbox area).
# We:
#
...
def _emit_surge_forcing_blocks(
    ...
    # --- 2b. Levee-breach INTERIOR point source ---
...
def _emit_physics_config(
    ...
    *,
    ...
    ``setup_config`` is a HydroMT passthrough (any key -> a
...
def _generate_hydromt_yaml_config(
    *,
    ...
    # advanced-physics overrides + a bare-constant infiltration
    ...
    # (supersedes the /vsigs/ rewrite FOR THE CATALOG PATH
    ...
    # - which also keeps gcsfs out of the read path (segfault
    ...
    # 2. ``pluvial_observed`` (v2, real precip raster): the
    ...
    if (
        ...
        # v2 - area-mean netamt path. The magnitude was computed
...
def build_sfincs_model(
    ...
    # --- Step 2.5: adaptive grid autoscale (SFINCS per-job autoscale)
    # Snap grid_resolution_m UP the ladder so the estimated ACTIVE-cell count
    # (cells inside the setup_mask_active window - NOT raw bbox area)
    ...
    # The worker contract (workers/sfincs/entrypoint.py:9-23) requires
    ...
    with tempfile.TemporaryDirectory(prefix="sfincs-build-") as tmpdir:
        tmp = Path(tmpdir)
        # hotfix: the authored ``manning_mapping.csv`` uses
        # our column names (``nlcd_class,manning_n,description``) that are
        # load-bearing for ``load_manning_mapping`` + the §4 validation
        ...
        yaml_text = _generate_hydromt_yaml_config(
            bbox=bbox,
            options=opts,
            # ``_generate_hydromt_yaml_config`` rewrites
        ...
        try:
            # hotfix: hydromt-sfincs 1.2.x expects ``opt`` as a
            ...
            # component-based, hence the §4 pin to < 2.0.
        ...
        # Worker reads this file first (workers/sfincs/entrypoint.py

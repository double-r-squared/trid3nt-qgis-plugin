...
The gr3 bridge lives in the WORKER tree (``workers/schism/schism_gr3.py``,
...
logger = logging.getLogger("trid3nt_server.workflows.schism.deck_authoring")
...
def quarterannulus_fixture_dir() -> Path:
    ...
    ``services/``) and returns ``workers/schism/fixtures/quarterannulus``.
    """
    here = Path(__file__).resolve()
    for parent in here.parents:
        cand = parent / "workers" / "schism" / "fixtures" / "quarterannulus"
        if cand.is_dir():
            return cand
    raise SchismDeckError(
        "SCHISM_INPUT_INVALID",
        "could not locate the bundled QuarterAnnulus fixture deck under workers/schism/fixtures",
    )
...
def wwm_duck_fixture_dir() -> Path:
    ...
    here = Path(__file__).resolve()
    for parent in here.parents:
        cand = parent / "workers" / "schism" / "fixtures" / "wwm_duck"
        if cand.is_dir():
            return cand
    raise SchismDeckError(
        "SCHISM_INPUT_INVALID",
        "could not locate the bundled Test_WWM_Duck fixture under workers/schism/fixtures",
    )
...
def load_gr3_bridge() -> Any:
    ...
    here = Path(__file__).resolve()
    for parent in here.parents:
        cand = parent / "workers" / "schism" / "schism_gr3.py"
        if cand.exists():
    ...
    raise SchismDeckError(
        "SCHISM_MESH_INVALID",
        "could not load the schism_gr3 TIN->hgrid bridge from workers/schism",
    )

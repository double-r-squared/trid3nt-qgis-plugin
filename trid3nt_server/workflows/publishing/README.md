...
and every layer after it is surfaced beside it; every field layer of one
quantity is ranged over all of them and a plane of a 3D field is named in its
layer, so the surface and the bottom of one variable read on one ramp; an
animation adopts the range of the published layer of the same quantity, so the
frames and the still read on the same one; a chart draws the reference lines its caller computed as named series
...
| `reads.py` | What a read IS - a `Field` (with the plane it is, on a 3D result), a `Series` (with the station it was read at), a `Profile` (with what it runs along), a `Frames`, a `Track`, a bare `Read` of measures, the `Line` a chart draws beside one - and the `Deliverable` that pairs a read with how it is published. |

...
The Landlab worker (``workers/landlab/entrypoint.py``) reads the
...
logger = logging.getLogger("trid3nt_server.workflows.landlab.run_landlab")
...
def stage_landlab_manifest(
    ...
    *,
    ...
    # flow_accumulation / susceptibility templates all funnel their DEM+grid
    # through this stage. A default/in-range value returns silently.
    enforce_resolution(LANDLAB_RES_SPEC, run_args.target_resolution_m)
...
# entrypoint (workers/landlab/entrypoint.py) runs as a subprocess
# in the current venv's Python via ``sys.executable -m
# workers.landlab.entrypoint``, with the repo root on
# PYTHONPATH so the worker's ``from workers.*`` imports resolve.
#
...
#: Repo root so the subprocess can resolve ``workers.*`` imports.
_TRID3NT_REPO_ROOT = str(Path(__file__).resolve().parents[3])
...
def landlab_local_spec() -> Any:
    ...
    ``from workers.*`` imports resolve in the current venv (no
    ...
    # ``workers.landlab.component_chain`` etc. directly.
...
def register_landlab_solver() -> None:
    ...
    """
    from trid3nt_server.data.simulation.solver.solver import LOCAL_SOLVER_SPEC_REGISTRY, register_local_solver_spec

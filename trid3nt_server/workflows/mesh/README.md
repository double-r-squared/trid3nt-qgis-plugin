...
| folder | what it is |
| --- | --- |
| `meshers/` | What a mesher IS - namespaces, a role adapter, a default recipe - plus one file per mesh library: `om2d.py` (OceanMesh2D) and `reg_grid.py` (the regular lattice). Its in-container scripts live beside their Dockerfile at `workers/mesh/scripts/`, mounted read-only at `/drivers` by `run_image_script` (`workflows/solver/image_script.py`): `om2d.py` there INTERPRETS the recipe's ops list, calling each name verbatim on the library. The TELEMAC-only scripts (`cas.py`, `dico.py`) live at `workers/telemac/scripts/` instead - they drive the engine's own steering readers for `workflows/telemac/` and the catalog instrument, not the mesh library. |
| `shared/` | What every mesher needs and no mesher owns: `primitives.py` (the shared op namespace - `set_bed` over ONE surface, recording what painted the nodes, and `set_boundary_roles` over the domain's declared runs), `nodes.py` (projection, a raster or a fetched layer sampled at the nodes, slope, boundary contours, reading an accepted mesh's nodes off its record and a centreline in its metres) and `selafin_io.py` (SELAFIN in the daemon, both directions: the SELAFIN + `.cli` pair written as one artifact and refusing a boundary walk whose rings share a node, and the result read back - neither costs a container start). `formats/` holds the TIN writers a solver reads - `hgrid.gr3`, `fort.14`, SMS `2dm` - behind one topology pass. |

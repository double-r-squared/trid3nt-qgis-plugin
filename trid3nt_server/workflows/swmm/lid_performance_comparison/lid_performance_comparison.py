...
``vegetative_swale`` (conveyance attenuation), ``rainbarrel_vs_disconnect``
(rain barrel vs simple rooftop disconnection, both vs baseline), or
``infiltration_vs_permeable_pavement`` (an infiltration trench vs permeable pavement
on the same footprint, both vs baseline). Every number is a real parsed solver
output; the deck is a schematic stub.
"""
...
from trid3nt_server.mesh.swmm_mechanism_compare import build_lid_performance
...
logger = logging.getLogger(
    "trid3nt_server.workflows.swmm.lid_performance_comparison"
)
...
TEMPLATE_CARD = TemplateCard(
    question=(
        ...
        "green roof, a vegetative swale, a rain barrel vs rooftop disconnection, or an "
        "infiltration trench vs permeable pavement, compared to no LID?"
    ),
    required_inputs=[],
    knobs=(
        "lid_type=green_roof|vegetative_swale|rainbarrel_vs_disconnect|"
...
async def swmm_lid_performance_comparison(
    ...
    **_extra_ignored: Any,
    ...
    barrel vs rooftop disconnection / infiltration trench vs permeable pavement),
    overlaying the hydrographs.

    Use this when: the user asks how a GREEN ROOF, a VEGETATIVE SWALE, a RAIN
    BARREL, simple ROOFTOP DISCONNECTION, an INFILTRATION TRENCH, or PERMEABLE
    PAVEMENT reduces / delays runoff on a subcatchment (low-impact development /
    green-infrastructure detention).
    ...
    Surface-based; infiltration trench = Surface+Storage native-seepage no drain;
    permeable pavement = Surface+Pavement+Storage+underdrain) on a small SYNTHETIC
    subcatchment + one design storm, solved headless with the continuity honesty
    gate. Schematic (NOT a georeferenced AOI) - the with/without runoff overlay +
    reduction numbers are the product; no map layer. Off-scope: a real AOI mesh
    flood -> swmm_urban_flood.

    Params:
        lid_type: "green_roof", "vegetative_swale", "rainbarrel_vs_disconnect"
            (baseline vs rain barrel vs rooftop disconnection), or
            "infiltration_vs_permeable_pavement" (baseline vs infiltration trench
            vs permeable pavement on one footprint). input_mode: reserved.

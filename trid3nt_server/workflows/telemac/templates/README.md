...
| `agitation/` | `artemis_harbor_agitation` - swell at the harbour mouth to the agitation coefficient behind a declared structure, `field("KD")` off ARTEMIS and its profile along the transect the workflow measures through the structure on the incident direction; the water it solves over is the domain slot and the structure's centreline becomes the footprint the mesh subtracts through the shared structure ingestion. |
| `nearshore_waves/` | `tomawac_nearshore_waves` - what the offshore swell becomes at the shore: the TOMAWAC spectrum carried in across the seaward rim at the sea state the wave slot read off a buoy, shoaling and refracting over the charted bed until depth-induced breaking takes it down; the height, the period and the direction charted where the ask places them, and the breaking band drawn as the shape it is. Its domain is the water the coastline leaves inside the drawn window. |
| `wave_driven_currents/` | `tomawac_wave_driven_currents` - the current breaking waves drive along the shore: TELEMAC-2D coupled to TOMAWAC on one mesh, the wave field recomputed on the deck's own coupling period and its radiation stresses driving the flow, the speed and its two components charted beside the wave height where the ask places them. Its domain is the water the coastline leaves inside the drawn window. |

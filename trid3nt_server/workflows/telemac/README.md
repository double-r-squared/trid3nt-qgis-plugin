...
| `products/` | What an open-water run is answered with: the wave, agitation, 3D and coastal postprocessors, and the listing readers the primitives share. See its own map. |
| `helpers/` | What a declaration summons: the reach front, the declared forcing, the dredge fields, the saturation relations, where a derived release settles, and the typed failures. See its own map. |

...
One door and five trees. Nothing lives at this root but the door and the engine's
typed failures: the trees are named for what they hold - `modules/` the wrappers,
the module inputs their keywords are read out of and the primitive set their
outputs are, `templates/` one package per question, `authoring/` the

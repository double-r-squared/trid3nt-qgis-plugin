...
| `outputs.py` | The primitive set - `field`, `series`, `max_over_time`, `profile`, `extent`, `mesh`, `mass_balance` - and `drogues` and `column`, with the read of each off a solved run through `read_selafin`, the daemon's own SELAFIN reader (`mesh/shared/selafin_io.py`): the engine image solves and does nothing else, and a result that cannot be opened refuses as `TELEMAC_RESULT_READ_FAILED` naming the file rather than falling back to it. `deliver` puts each read into the format the template publishes it in. |

...
logger = logging.getLogger("trid3nt_server.data.publish_layer.publish_layer")
...
def _sediment_yield_log_style_params() -> str:
    ...
    from ..processing.compute_sediment_yield.compute_sediment_yield import SEDIMENT_YIELD_LOG_CLASSES, hex_to_rgba
...
def _write_durable_vector_geojson(
    ...
    try:
        ...
        from ..simulation.solver.solver import _get_runs_bucket
...
def _build_cog_with_overviews(raster_bytes: bytes) -> bytes | None:
    ...
    try:
        ...
        try:
            from ..processing.compute_hillshade.compute_hillshade import _get_gdaldem_bin, _translate_to_cog
...
def publish_layer(
    ...
    **_extra_ignored: Any,
    ...
    if layer_uri.startswith(("http://", "https://")) and "/cog/tiles/" in layer_uri:
        from trid3nt_server.data._uri_util import _unwrap_tile_template

...
IR: `trid3nt_contracts.message` is the internal history and tool-declaration
shape, and a provider adapter is the only place a provider's own nouns are
allowed to appear. Every adapter yields the same streamed events, so the turn
loop never branches on which model is answering.
...
| `adapter.py` | The turn builders, the tool-declaration builder and the provider-dispatch seam - what a provider adapter is required to be. |

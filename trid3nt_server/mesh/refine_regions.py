...
(:attr:`~trid3nt_server.mesh.spatial_roles.DrawnRoles.refine_regions`)
into ONE data-only :class:`MeshSizingSpec` the paradigm-native generators apply:

  * MODFLOW DISV / gridgen (worker-side ``workers/modflow/disv_grid``):
...
COPY-workers-only boundary. Refinement level maps a target cell size to

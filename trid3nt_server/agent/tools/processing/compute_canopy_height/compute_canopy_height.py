...
logger = logging.getLogger("trid3nt_server.agent.tools.processing.compute_canopy_height.compute_canopy_height")
...
#: prefix (the entrypoint's output; mirrored by ``workers/canopy``).
...
def _resolve_cog_from_result(run_result: Any, batch_run_id: str) -> str | None:
    ...
    from trid3nt_server.agent.tools.simulation.solver.solver import _get_runs_bucket, _try_get_completion_s3  # type: ignore[attr-defined]

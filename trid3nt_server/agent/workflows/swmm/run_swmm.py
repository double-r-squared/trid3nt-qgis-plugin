...
module owns the urban-flood engine's solver-dispatch surface:
  ...
  3. **A SWMM ``LocalSolverSpec``** (``swmm_local_spec``) + ``'swmm'`` registered
     ...
     (``workers/swmm/run_inp.py``): ``exec_kind="exec"`` (there is no
...
logger = logging.getLogger("trid3nt_server.agent.workflows.swmm.run_swmm")
...
def stage_swmm_manifest(staging: SWMMStaging) -> str:
    ...
    Writes:
      ...
      - ``.../manifest.json`` -- the manifest the SWMM worker entrypoint reads
        (``workers/swmm/entrypoint.py``): ``inputs[]`` carry the legacy
...
#: Repo root for PYTHONPATH injection (subprocess must resolve workers.*).
...
def swmm_local_spec() -> Any:
    ...
    in-repo CLI shim ``workers/swmm/run_inp.py`` with the venv python),
    ...
    def build_argv(run_id: str, rundir: Path, args: list[str]) -> list[str]:
        # python workers/swmm/run_inp.py <inp_filename...>. The .inp is
    ...
    # ``workers.swmm.*`` when the agent is installed in an isolated venv.

"""Engine template ``swmm_urban_flood`` - PySWMM quasi-2D urban-flood engine
(engine-door refactor - SWMM slice; was ``run_swmm_urban_flood``).
...
a SOUND BARRIER with RED walls / GREEN flap gates). ``swmm_urban_flood(...)``
takes the ``SWMMRunArgs`` forcing/structure fields, runs the deterministic
fetch -> build -> solve -> postprocess chain
(``model_swmm_urban_flood`` below, in this module), and returns a ``SWMMDepthLayerURI``
...
``sfincs_flood`` (SFINCS). It is a registered engine TEMPLATE tagged
...
from trid3nt_server.agent.workflows.swmm.run_swmm import SWMMWorkflowError

logger = logging.getLogger(
    "trid3nt_server.agent.workflows.swmm.urban_flood.urban_flood"
)
...
#: Curated door-listing card (the run_swmm door prefers this over signature
#: derivation). One-line question + the real required input + a knobs summary.
TEMPLATE_CARD = TemplateCard(
    question=(
        "peak overland flood depth + a depth animation for an urban / "
        "street-level design storm (PySWMM node-link storm sewer + pluvial "
        "overland; buildings as obstruction, walls / flap gates as controls)"
    ),
    required_inputs=["bbox"],
    knobs=(
        "return_period_yr, total_rain_depth_mm, storm_duration_hr, "
        "rain_interval_min, building_representation, infiltration_method, "
        "target_resolution_m, manning_overland, mass_balance_tolerance_pct, "
        "barriers (walls / flap gates), pollutants, dry_buildup_days, washoff_model"
    ),
)


_SWMM_URBAN_FLOOD_METADATA = AtomicToolMetadata(
    ...
    **_extra_ignored: Any,
    ...
    if bbox is None:
        return {
            ...
            "error_message": (
                "swmm_urban_flood requires a bbox "
    ...
    logger.info(
        "swmm_urban_flood bbox=%s return_period=%dyr duration=%.1fh "
    ...
    try:
        peak = await model_swmm_urban_flood(
            run_args,
        ...
        logger.warning("swmm_urban_flood failed: %s (%s)", exc.error_code, exc)
        return {
            "status": "error",
...
from trid3nt_server.agent.workflows.swmm.postprocess_swmm import (
...
def _enforce_min_urban_aoi(
    ...
    logger.info(
        "model_swmm_urban_flood: AOI floor applied - input bbox %s was "
...
# The composer.
# --------------------------------------------------------------------------- #
async def model_swmm_urban_flood(
    run_args: SWMMRunArgs,
    *,
    ...
    if emitter is not None:
        try:
            ...
            logger.warning("model_swmm_urban_flood: zoom-to emit failed: %s", exc)
    ...
    logger.info("model_swmm_urban_flood: DEM=%s (%s)", local_dem_path, dem_source)
    ...
    if _buildings_absent:
        logger.warning(
            "model_swmm_urban_flood: NO building obstructions applied - OSM "
    ...
    if run_args.total_rain_depth_mm is None:
        ...
        if depth_mm is not None:
            ...
            logger.info(
                "model_swmm_urban_flood: Atlas-14 depth=%.1f mm (%d-yr, %.0f-hr)",
                depth_mm,
                run_args.return_period_yr,
                run_args.storm_duration_hr,
            )
    ...
    if getattr(staging, "pollutants", None):
        try:
            ...
            logger.warning(
                "model_swmm_urban_flood: WQ postprocess/emit failed (non-fatal, "
    ...
    if _os.environ.get("TRID3NT_SWMM_REGISTRY_QUANTITIES", "").lower() in (
        ...
        try:
            from trid3nt_server.agent.workflows.swmm.postprocess_swmm import publish_swmm_quantities
            from trid3nt_server.agent.workflows.shared.register_published_manifest import register_manifest_layers
            ...
            logger.warning(
                "model_swmm_urban_flood registry-quantity publish failed "
                "(non-fatal): %s",
                exc,
            )
    ...
    logger.info(
        "model_swmm_urban_flood complete run_id=%s max_depth_m=%.4g "
    ...
    if emitter is not None:
        try:
            ...
            logger.warning(
                "model_swmm_urban_flood: authoritative zoom-to emit failed: %s", exc
            )
...
def _publish_peak_layer(
    ...
    try:
        ...
        logger.warning(
            "model_swmm_urban_flood: publish_layer FAILED for the peak "
...
async def _emit_frame_layers(
    ...
    if not frame_layers or emitter is None:
        if frame_layers:
            logger.info(
                "model_swmm_urban_flood: %d animation frames available but no "
    ...
    for lyr in frame_layers:
        ...
        if not (lyr.uri.startswith("gs://") or lyr.uri.startswith("s3://")):
            ...
            try:
                ...
                logger.warning(
                    "model_swmm_urban_flood: publish_layer FAILED for frame "
        ...
        try:
            ...
            logger.warning(
                "model_swmm_urban_flood: frame add_loaded_layer failed for %s: %s",
                emit_layer.layer_id,
                exc,
            )
    if emitted:
        logger.info(
            "model_swmm_urban_flood: emitted %d/%d animation frames as a "
...
async def _publish_and_emit_wq(
    ...
    logger.info(
        "model_swmm_urban_flood WQ run_id=%s metrics=%s",
        staging.run_id,
        {
            n: {
    ...
    for lyr in pol_layers:
        ...
        if lyr.uri.startswith("gs://") or lyr.uri.startswith("s3://"):
            try:
                ...
                logger.warning(
                    "model_swmm_urban_flood: WQ publish_layer FAILED for %s "
        ...
        try:
            ...
            logger.warning(
                "model_swmm_urban_flood: WQ add_loaded_layer failed for %s: %s",
                emit_layer.layer_id,
                exc,
            )
    ...
    try:
        from trid3nt_server.agent.tools.processing.charts_common import build_pollutograph_chart
        ...
        logger.warning(
            "model_swmm_urban_flood: pollutograph chart emit failed (non-fatal): %s",
            exc,
        )

...
logger = logging.getLogger("trid3nt_server.tools.processing.digitize_water_body.digitize_water_body")
...
#: Water polygons are an outline a reader locates themselves by.
...
def estimate_payload_mb(
    ...
    **_kw: Any,
    ...
    """The emitted FlatGeobuf's size in MB, scaled loosely with bbox area since
    shoreline complexity drives the vertex count, then floored and capped.
    """
    if bbox is None:
        return 0.5
    try:
...
def _default_window() -> tuple[str, str]:
    ...
    recent low-cloud scene is reliably found.
    """
...
def _digitize_water_fgb_bytes(
    ...
    """Search Sentinel-2, threshold NDWI over ``bbox`` and vectorize the mask to
    FlatGeobuf bytes; no scene and no water are both typed refusals.
    """
    import numpy as np
    import rasterio
...
@register_tool(
    ...
    supports_global_query=False,
    payload_mb_estimator_name="estimate_payload_mb",
    # Reads an external public STAC API and Azure blobs, so it carries
...
def digitize_water_body(
    ...
    **_extra_ignored: Any,
    ...
    Params:
        ...
        ndwi_threshold: water cutoff, default 0.0 (McFeeters 1996); lower it,
            say to -0.1, for turbid water. Range [-1, 1].
        max_cloud_cover: scene ceiling in percent, default 20.0.
        min_area_m2: drop polygons under this, default 1000, which removes
            few-pixel specks.

    Returns FlatGeobuf polygons carrying ``area_m2`` and ``water_index``. No
    scene, or no water over the threshold, is a typed refusal.
    """
    _validate_bbox(bbox)

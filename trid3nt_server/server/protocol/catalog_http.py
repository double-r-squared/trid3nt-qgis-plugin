"""The HTTP door: the read-only library, and the routes beside it.

One listener beside the WebSocket server, keyed by method and path. It serves
the library - every registered tool under its subsystem, every source row under
its class and kind, and one search over the corpus the model routes on - beside
the case, layer, probe, version and plugin-repository routes and the telemetry
summary. Every facet derives from a registry, and the routes are unauthenticated
with open CORS."""
...
# them never re-sorts. A tool's description is the docstring the model routes
# on, so the person and the model read the same words about the same surface.
...
def _library_tool_entry(name: str, entry: Any) -> dict[str, Any]:
...
def _library_data_entry(fetcher: str, row: Any) -> dict[str, Any]:
...
def _library_classes(
    fetchers: frozenset[str] | None = None,
) -> list[dict[str, Any]]:
...
def build_library_payload(*, use_cache: bool = True) -> dict[str, Any]:
    ...
    for name in sorted(TOOL_REGISTRY.keys()):
        ...
        by_subsystem.setdefault(_subsystem_of(entry), []).append(
            _library_tool_entry(name, entry))
...
async def build_library_search(query: str, top_k: int = 10) -> dict[str, Any]:
    ...
    try:
        k = max(1, min(_LIBRARY_SEARCH_TOP_K, int(top_k)))
    ...
    for ranked in found.get("results", []):
        ...
        if entry is None:
            ...
            continue
        hit = _library_tool_entry(name, entry)
        hit["kind"] = "tool"
...
async def _route_library(_req: _Request) -> _Reply:
    """``GET /api/library``: every registered tool under its subsystem and every
    source row under its class and kind."""
    return _json_reply(build_library_payload())


async def _route_library_search(req: _Request) -> _Reply:
    ...
    params = parse_qs(req.query, keep_blank_values=False)
    return _json_reply(
        await build_library_search((params.get("q") or [""])[0].strip()))
...
_ROUTES: dict[tuple[str, str], _Route] = {
    ...
    ("GET", "/api/library"): _Route(
        _route_library, "library listing failed"),
    ("GET", "/api/library/search"): _Route(
        _route_library_search, "library search failed"),

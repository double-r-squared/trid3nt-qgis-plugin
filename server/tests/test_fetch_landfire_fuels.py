...
from trid3nt_server.agent.tools.fetchers.hazard.fetch_landfire_fuels.fetch_landfire_fuels import (
    _LANDFIRE_YEAR,
    _LAYER_SERVICE,
    _LAYER_STYLE_PRESET,
    _LAYER_UNITS,
...
def test_bbox_to_pixel_size_clamps_to_bounds() -> None:
    ...
    from trid3nt_server.agent.tools.fetchers.hazard.fetch_landfire_fuels.fetch_landfire_fuels import _PX_MAX, _PX_MIN
...
def test_mocked_tiff_fetch_writes_to_cache() -> None:
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_landfire_fuels.fetch_landfire_fuels.read_through",
        side_effect=patched_rt,
    ), patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_landfire_fuels.fetch_landfire_fuels.requests.get",
        ...
        # validation.
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_landfire_fuels.fetch_landfire_fuels._is_all_nodata",
        return_value=False,
    ):

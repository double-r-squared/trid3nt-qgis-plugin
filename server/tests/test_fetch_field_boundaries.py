...
from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_field_boundaries.fetch_field_boundaries import (
    FTW_DATASETS,
    FieldsInputError,
...
def test_coverage_returns_vector_layer_with_polygons():
    ...
    gdf = _fields_gdf(3)
    with patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_field_boundaries.fetch_field_boundaries.read_through",
        _make_read_through_injector(fake),
    ), patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_field_boundaries.fetch_field_boundaries._read_fields_gdf",
        return_value=gdf,
    ) as mock_read:
...
def test_live_us_usda_iowa_returns_fields():
    from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_field_boundaries.fetch_field_boundaries import _read_fields_gdf, _select_dataset

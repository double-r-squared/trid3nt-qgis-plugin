"""SWMM engine-door slice (engine-door refactor, docs/specs/engine-rollout-contract.md).

Offline floor for the ``run_swmm`` door + ``swmm_urban_flood`` template:
...
(a) DOOR LISTING - direct-call the door on the live registry; assert the
    ...
    ``engine=swmm, tier=template`` tool, ``fidelity_brief`` +
    ``mismatch_redirect`` + ``next_action`` present). No solve.

(b) GATING EXCLUSION - ``swmm_urban_flood`` (tier=template) is EXCLUDED from the
    ...
    ``run_swmm`` (tier=door) is NOT excluded and is recognized as a
    gate-expander.

(c) REGISTRY MEMBERSHIP - the old ``run_swmm_urban_flood`` name is GONE;
    ``run_swmm`` is registered tier=door and ``swmm_urban_flood`` tier=template.
...
def test_old_swmm_name_gone_and_door_template_registered():
    reg = agent_tools.TOOL_REGISTRY
    assert "run_swmm_urban_flood" not in reg, (
        "the old registered name must be REPLACED (no alias)"
    )
    door = reg[_DOOR].metadata
    assert door.engine == "swmm" and door.tier == "door"
    tmpl = reg[_TEMPLATE].metadata
    assert tmpl.engine == "swmm" and tmpl.tier == "template"
    assert tmpl.source_class == "workflow_dispatch"


def test_setter_stays_general():
    reg = agent_tools.TOOL_REGISTRY
    m = reg["set_swmm_parameters"].metadata
    assert m.tier == "general", "set_swmm_parameters must stay tier=general"
...
def test_door_listing_envelope_shape():
    ...
    for door in ("run_sfincs", "run_modflow"):
        assert door in redirect_text, f"mismatch redirect must point at {door}"


def test_door_lists_only_registered_swmm_templates():
    reg = agent_tools.TOOL_REGISTRY
    env = run_swmm()
    listed = {c["tool_name"] for c in env["templates"]}
    assert _TEMPLATE in listed
    # every listed template is a REGISTERED engine=swmm tier=template (no
    # fabricated / not-yet-built template).
    for name in listed:
        m = reg[name].metadata
        assert m.engine == "swmm" and m.tier == "template", name
...
def test_door_listing_matches_registry_derivation():
    ...
    template names equals the set of engine=swmm tier=template registrations."""
    reg = agent_tools.TOOL_REGISTRY
    expected = sorted(
        n for n, e in reg.items()
        if getattr(e.metadata, "engine", None) == "swmm"
        and getattr(e.metadata, "tier", "general") == "template"
    )
    listed = sorted(c["tool_name"] for c in run_swmm()["templates"])
...
def test_template_excluded_door_present_in_index():
    ...
    try:
        ...
        assert _TEMPLATE not in idx.tool_names, (
            "swmm_urban_flood (tier=template) must be EXCLUDED from the default pool"
        )
...
def test_template_never_surfaces_in_retrieval():
    ...
    try:
        ...
        vis = tr.retrieve_visible_tools(
            "simulate urban street flooding around the buildings here", None, k=40
        )
...
def test_door_recognized_as_gate_expander():
    ...
    assert _DOOR in gx, "the SWMM door must be a registry-recognized gate-expander"

...
from trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels import (
    _LANDFIRE_YEAR,
    _LAYER_LABEL,
    _LAYER_SERVICE,
    _LAYER_STYLE_PRESET,
...
def test_bbox_to_pixel_size_clamps_min() -> None:
    ...
    from trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels import _PX_MIN
...
def test_bbox_to_pixel_size_clamps_max() -> None:
    ...
    from trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels import _PX_MAX
...
def test_estimate_payload_mb_large_bbox() -> None:
    ...
    from trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels import _PAYLOAD_MAX_MB
...
def test_mocked_tiff_fetch_writes_to_cache() -> None:
    ...
    with patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels.read_through",
        side_effect=patched_rt,
    ), patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels.requests.get",
        return_value=_make_response(tiff),
    ), patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usfs_canopy_fuels.fetch_usfs_canopy_fuels._is_all_nodata",
        return_value=False,
    ):

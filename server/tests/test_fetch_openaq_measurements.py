...
from trid3nt_server.tools.fetchers.weather.fetch_openaq_measurements.fetch_openaq_measurements import (
    DEFAULT_PARAMETERS,
    OpenAQAuthError,
    OpenAQInputError,
...
def test_no_key_raises_before_any_network_call():
    ...
    with patch.dict(os.environ, {}, clear=True), patch(
        "trid3nt_server.tools.fetchers.weather.fetch_openaq_measurements.fetch_openaq_measurements.httpx.Client"
    ) as mock_client:
        with pytest.raises(OpenAQMissingKeyError) as exc:
...
def test_mocked_happy_path_two_call_fanout():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.weather.fetch_openaq_measurements.fetch_openaq_measurements.read_through",

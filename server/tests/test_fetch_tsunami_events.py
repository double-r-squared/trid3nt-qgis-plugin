...
Live test (gated by TRID3NT_TEST_LIVE_NCEI_TSUNAMI=1): a real NCEI request for a
...
from trid3nt_server.agent.tools.fetchers.hazard.fetch_tsunami_events.fetch_tsunami_events import (
    CAUSE_CODES,
    DEFAULT_MIN_YEAR,
    ITEMS_PER_PAGE,
    MAX_PAGES,
...
def test_empty_result_raises_no_events():
    with mock.patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_tsunami_events.fetch_tsunami_events._http_get_json",
...
def test_full_tool_returns_layer_uri():
    ...
    with mock.patch(
        ...
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_tsunami_events.fetch_tsunami_events.read_through",
...
@pytest.mark.skipif(
    os.environ.get("TRID3NT_TEST_LIVE_NCEI_TSUNAMI") != "1",
    reason="set TRID3NT_TEST_LIVE_NCEI_TSUNAMI=1 to hit the real NCEI service",
)

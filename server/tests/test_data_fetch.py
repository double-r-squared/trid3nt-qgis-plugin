...
_ALL_FETCH_MODS = (dem_mod, bld_mod, pop_mod, geo_mod, lc_mod, pfd_mod)
...
def test_fetch_dem_happy_path_writes_through_cache(monkeypatch):
    ...
    _setattr_all_fetch(monkeypatch, "read_through", patched_read_through)
...
def test_fetch_3dep_full_coverage_passes(monkeypatch):
    ...
    data = dem_mod._fetch_3dep_dem_bytes(req, 10)
...
def test_fetch_3dep_south_edge_short_raises_partial_coverage(monkeypatch):
    ...
    with pytest.raises(dem_mod.DemPartialCoverageError) as exc:
        dem_mod._fetch_3dep_dem_bytes(req, 10)

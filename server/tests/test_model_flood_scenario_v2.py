...
async def test_none_path_forcing_spec_identical_to_baseline() -> None:
    ...
    with (
        ...
        patch("trid3nt_server.workflows.model_flood_scenario.lookup_precip_return_period", return_value=_precip_result()) as mock_lookup,
        patch("trid3nt_server.workflows.model_flood_scenario.build_sfincs_model", side_effect=_capture_build),
        ...
        patch("trid3nt_server.workflows.model_flood_scenario.publish_layer", side_effect=PublishLayerError("JOBS_CLIENT_UNAVAILABLE", "no qgis in test")),
    ):
...
async def test_raster_path_skips_atlas14_and_builds_observed_forcing() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with (
            ...
            patch("trid3nt_server.workflows.model_flood_scenario.fetch_landcover", return_value=_landcover_result()),
            patch("trid3nt_server.workflows.model_flood_scenario.fetch_river_geometry", return_value=_mock_layer_uri("rivers")),
            patch("trid3nt_server.workflows.model_flood_scenario.lookup_precip_return_period") as mock_lookup,
            patch("trid3nt_server.workflows.model_flood_scenario.build_sfincs_model", side_effect=_capture_build),
            patch("trid3nt_server.workflows.model_flood_scenario.run_solver", return_value=handle),
            patch("trid3nt_server.workflows.model_flood_scenario.wait_for_completion", side_effect=_wfc),
            patch(
...
def test_gs_forcing_read_path_unchanged_does_not_call_boto3() -> None:
    ...
    with (
        ...
        patch(
            "trid3nt_server.workflows.model_flood_scenario._to_vsigs",

...
from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm import (
...
def _fast_sleep(monkeypatch):
    ...
    monkeypatch.setattr(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm.time.sleep", lambda *_: None
    )
...
def test_50_way_response_yields_50_features(monkeypatch):
    ...
    with patch("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm.httpx.Client", FakeClient), \
         patch(
             "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm.read_through",
...
def test_504_maps_to_retryable_upstream_error(monkeypatch):
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm.httpx.Client",
...
def test_live_fort_myers_returns_primary_and_motorway():
    ...
    from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_roads_osm.fetch_roads_osm import _fetch_osm_roads_bytes

...
def test_fetch_bathy_rejects_land_only_dem():
    from trid3nt_server.workflows.swan.model_wave_scenario.model_wave_scenario import (
        SwanComposerError,
    ...
    with patch(
        "trid3nt_server.tools.fetchers.ocean.fetch_topobathy.fetch_topobathy.fetch_topobathy",
...
def test_fetch_bathy_accepts_real_bathymetry():
    from trid3nt_server.workflows.swan.model_wave_scenario.model_wave_scenario import _fetch_bathy_for_swan
...
def test_fetch_bathy_typed_error_when_topobathy_fails():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.ocean.fetch_topobathy.fetch_topobathy.fetch_topobathy", _boom
    ):
        with pytest.raises(SwanComposerError) as ei:
            _fetch_bathy_for_swan(bbox)
...
def test_read_swan_mat_fields_reads_hs_tp_dir(tmp_path: Path):
    from trid3nt_server.workflows.swan.postprocess_swan import read_swan_mat_fields
...
def test_compute_swan_wave_metrics_on_synthetic_grid():
    from trid3nt_server.workflows.swan.postprocess_swan import compute_swan_wave_metrics
...
def test_postprocess_swan_end_to_end_shape(tmp_path: Path):
    ...
    from trid3nt_server.workflows.swan import postprocess_swan as ps
...
def test_upsample_for_cog_preserves_values_and_mask():
    from trid3nt_server.workflows.swan.postprocess_swan import _upsample_for_cog
...
def test_written_hs_cog_has_overviews(tmp_path: Path):
    ...
    from trid3nt_server.workflows.swan.postprocess_swan import (
        _COG_MIN_DIM_PX,
        _write_hs_cog_4326,
    )
...
def test_composer_arg_assembly_and_dispatch(tmp_path: Path):
    ...
    from trid3nt_server.workflows.swan.model_wave_scenario import model_wave_scenario as comp
    from trid3nt_server.workflows.swan.run_swan import SwanStaging
...
def test_coerce_boundary_snaps_net_outgoing_dir_inward():
    ...
    from trid3nt_server.workflows.swan.run_swan import _coerce_boundary_inward

...
from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_usace_nsi.fetch_usace_nsi import (
...
def test_user_agent_header_sent_on_request():
    ...
    with patch("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_usace_nsi.fetch_usace_nsi.httpx.Client", FakeClient):
        _fetch_nsi_geojson(
...
def test_5_feature_response_writes_fgb_with_pelicun_columns():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_usace_nsi.fetch_usace_nsi._fetch_nsi_geojson",
        return_value=fake_geojson,
    ), patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_usace_nsi.fetch_usace_nsi.read_through",
...
def test_cache_miss_invokes_fetch_fn_then_hit_skips():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_usace_nsi.fetch_usace_nsi._fetch_nsi_bytes",

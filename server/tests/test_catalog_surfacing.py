...
- DEFAULT config (no arm flag): the 27 spec-served sources stay tier="general",
  ambient-declarable; registry == 208 (+4 pelicun CAND-S Assessment-API validation templates:
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 208  # registry does NOT shrink; only the pool does

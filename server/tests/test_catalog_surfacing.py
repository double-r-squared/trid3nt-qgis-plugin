...
- DEFAULT config (no arm flag): the 27 spec-served sources stay tier="general",
  ambient-declarable; registry == 176 (ADR 0105: -3 standalone composers, atop ADR 0095 -1 fetch_cama_flood_discharge
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 176  # registry does NOT shrink; only the pool does

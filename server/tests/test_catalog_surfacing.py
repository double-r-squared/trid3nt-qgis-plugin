...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 88  # ADR 0088: +fetch_goes_archive_animation +fetch_goes_active_fire (netcdf_cf_object mode)
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 87  # ADR 0088: +2 more animation-frames specs (archive/active_fire) also leave the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 87  # 88 specs minus the internal copernicus seam (ADR 0088)

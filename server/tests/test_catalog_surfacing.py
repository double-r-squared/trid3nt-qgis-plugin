...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 91  # ADR 0097: +fetch_dem (library_delegate py3dep + source="copernicus" cross-sibling dispatch)
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 90  # ADR 0097: +fetch_dem also leaves the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 90  # 91 specs minus the internal copernicus seam (ADR 0097 +fetch_dem)

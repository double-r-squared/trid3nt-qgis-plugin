...
- DEFAULT config (no arm flag): the 27 spec-served sources stay tier="general",
  ambient-declarable; registry == 235 (+1 SWMM RTK unit-hydrograph RDII template ADR 0190 row 4 (+1 ELMFIRE Hirsch initial-attack POC
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 235  # registry does NOT shrink; only the pool does

...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 35
    # The 35 stay ambient (tier=general) and IN the declarable pool.
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    # -34, not -35: fetch_copernicus_dem is tier="internal" (wave-11 absorption into
    # fetch_dem), so it is ALREADY out of the ambient pool in the None baseline; the
    # arm moves the remaining 34 general->catalog (29 + the 4 chained-resolution folds,
    # ADR 0063 + the 2 ADR 0064 folds openfema / storm_events).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 34
...
def test_arm3_specs_leave_pool_and_source_param():
    ...
    assert r["gridmet_tier"] == "catalog"
    assert r["any_spec_in_declarable"] is False  # every spec leaves the ambient pool
    # -34, not -35: fetch_copernicus_dem is tier="internal" (already out of the pool).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 34
    assert r["gridmet_in_index"] is True
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 34  # 35 specs minus the internal copernicus seam

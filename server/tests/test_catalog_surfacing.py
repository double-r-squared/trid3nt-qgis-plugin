...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 73  # ADR 0082: +fetch_landcover +fetch_flood_extent_observation
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 72  # ADR 0082: +2 specs leave pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 72  # 73 specs minus the internal copernicus seam (ADR 0082)

...
- DEFAULT config (no arm flag): the 27 spec-served sources stay tier="general",
  ambient-declarable; registry == 177 (ADR 0109 +1 hecras_muncie_flood; ADR 0105: -3 standalone composers, atop ADR 0095 -1 fetch_cama_flood_discharge
...
def test_default_config_identity():
    ...
    assert r["registry_size"] == 177
    assert r["n_specs"] == 94  # ADR 0111: +fetch_storm_tracks +fetch_goes_satellite (fetcher finale wave 2)
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    r = _run_arm("2")
    assert r["arm"] == "2"
    assert r["registry_size"] == 177  # registry does NOT shrink; only the pool does
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 93  # ADR 0111: +storm_tracks +goes_satellite also leave the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 93  # 94 specs minus the internal copernicus seam (ADR 0111 +storm_tracks +goes_satellite)

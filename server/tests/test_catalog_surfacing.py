...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    # -51, not -52: fetch_copernicus_dem is tier="internal" (wave-11 absorption into
    ...
    # station-sibling folds, the 2 ADR 0068 SLR-raster mapserver_export folds, and
    # the 2 ADR 0070 Overpass folds roads / pois).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 51
...
def test_arm3_specs_leave_pool_and_source_param():
    ...
    assert r["gridmet_tier"] == "catalog"
    assert r["any_spec_in_declarable"] is False  # every spec leaves the ambient pool
    # -51, not -52: fetch_copernicus_dem is tier="internal" (already out of the pool).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 51
    assert r["gridmet_in_index"] is True
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 51  # 52 specs minus the internal copernicus seam

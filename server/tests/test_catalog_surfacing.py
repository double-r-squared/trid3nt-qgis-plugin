...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 33
    # The 33 stay ambient (tier=general) and IN the declarable pool.
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    # -32, not -33: fetch_copernicus_dem is tier="internal" (wave-11 absorption into
    # fetch_dem), so it is ALREADY out of the ambient pool in the None baseline; the
    # arm moves the remaining 32 general->catalog (29 + the 4 chained-resolution folds, ADR 0063).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 32
...
def test_arm3_specs_leave_pool_and_source_param():
    ...
    assert r["gridmet_tier"] == "catalog"
    assert r["any_spec_in_declarable"] is False  # every spec leaves the ambient pool
    # -32, not -33: fetch_copernicus_dem is tier="internal" (already out of the pool).
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 32
    assert r["gridmet_in_index"] is True
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 32  # 33 specs minus the internal copernicus seam

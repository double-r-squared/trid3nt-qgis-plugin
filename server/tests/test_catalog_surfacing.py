...
def test_default_config_identity():
    ...
    assert r["n_specs"] == 86  # ADR 0087: +fetch_goes_animation +fetch_goes_blend_animation +fetch_viirs_day_fire
...
def test_arm2_specs_leave_pool_but_stay_indexed():
    ...
    assert r["declarable_size"] == _run_arm(None)["declarable_size"] - 85  # ADR 0087: +3 animation-frames specs also leave the arm-ON pool
...
def test_stratum_index_is_source_scoped(_stratum):
    ...
    assert len(idx.tool_names) == 85  # 86 specs minus the internal copernicus seam (ADR 0087)

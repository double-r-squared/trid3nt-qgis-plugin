...
from trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow import (
    SnotelError,
    SnotelInputError,
...
def test_happy_path_layeruri_shape() -> None:
    ...
    with patch("trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow._http_get", side_effect=fake_http_get), patch(
        "trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow.read_through", side_effect=_fake_read_through
    ):
...
def test_data_unreachable_degrades_to_locations() -> None:
    ...
    with patch("trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow._http_get", side_effect=fake_http_get), patch(
        "trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow.read_through", side_effect=capturing_read_through
    ):
...
def test_live_colorado_rockies() -> None:
    from trid3nt_server.tools.fetchers.soil.fetch_snotel_snow.fetch_snotel_snow import _fetch_snotel_snow_bytes

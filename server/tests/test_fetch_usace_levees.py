...
from trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees import (
    CONUS_BBOX,
    LAYER_TO_FS_ID,
    USACELeveeError,
...
def test_synthetic_response_writes_fgb_with_polygons():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees._fetch_nld_geojson",
        return_value=fake_geojson,
    ), patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees.read_through",
...
def test_global_sweep_omits_geometry_param():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees._fetch_nld_page",
...
def test_500_raises_typed_upstream_error():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees.httpx.Client", FakeClient
    ):
...
def test_cache_miss_invokes_fetch_then_hit_skips():
    ...
    with patch(
        "trid3nt_server.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees._fetch_nld_bytes",

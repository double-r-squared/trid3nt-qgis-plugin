...
from trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation import (
    GOES_PRODUCTS,
    SUPPORTED_PRODUCTS,
    VIIRS_PRODUCTS,
...
def test_review_gate_stops_without_fetching_frames():
    ...
    with patch.dict(
        ...
        "trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation._peek_frame_count",
        _fake_peek,
    ):
        result = _run(
            model_satellite_fire_animation(
...
def test_confirm_emits_postprocess_flood_frame_shape():
    ...
    with patch.dict(
        ...
        "trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation._safe_overlay_firms",
        _async_none,
    ), patch(
        "trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation._safe_overlay_perimeters",
        _async_none,
    ), patch(
        "trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation._publish_layers",
        _async_empty_dict,
    ):
        result = _run(
            model_satellite_fire_animation(
...
def test_confirm_empty_run_is_not_ok_honesty_floor():
    ...
    def _fake_goes(*a, **k):
        from trid3nt_server.agent.tools.fetchers.imagery.fetch_goes_animation.fetch_goes_animation import GOESAnimEmptyError
...
def test_coarse_geocode_localizes_from_firms_and_emits_aoi_pre_gate():
    ...
    review gate."""
    from trid3nt_server.agent.tools.fetchers.hazard.fetch_firms_active_fire.fetch_firms_active_fire import FirmsArgError  # noqa: F401
    ...
    def _no_wfigs(name, state=None, *a, **k):
        ...
        from trid3nt_server.agent.tools.fetchers.hazard.fetch_wfigs_incident.fetch_wfigs_incident import (
    ...
    with patch.dict(
        ...
        "trid3nt_server.agent.workflows.shared.model_satellite_fire_animation.model_satellite_fire_animation._read_firms_points",
        _fake_read_points,
    ):
        result = _run(
            model_satellite_fire_animation(

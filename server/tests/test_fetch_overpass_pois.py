...
from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois import (
    OVERPASS_ENDPOINTS,
    OverpassInputError,
...
def _fast_sleep(monkeypatch) -> None:
    monkeypatch.setattr(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois.time.sleep", lambda *_a, **_k: None
    )
...
def test_end_to_end_synthetic_features_and_cache_hit(monkeypatch):
    ...
    with patch("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois.httpx.Client", _Client), patch(
        "trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois.read_through",
...
def test_all_mirrors_504_raises_upstream_error(monkeypatch):
    ...
    with patch("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois.httpx.Client", _Client):
        from trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_overpass_pois.fetch_overpass_pois import _post_overpass

"""LANDLAB engine-door slice (engine-door refactor, docs/specs/engine-rollout-contract.md).

Offline floor for the ``run_landlab`` door + ``landlab_susceptibility`` template:
...
(a) DOOR LISTING - direct-call the door on the live registry; assert the
    ...
    ``engine=landlab, tier=template`` tool, ``fidelity_brief`` +
    ``mismatch_redirect`` + ``next_action`` present). No solve.

(b) GATING EXCLUSION - ``landlab_susceptibility`` (tier=template) is EXCLUDED
    from the default retrieval pool (absent from the discover index
    ``tool_names`` and from ``retrieve_visible_tools`` and from the fail-open
    floor); the door ``run_landlab`` (tier=door) is NOT excluded and is
    recognized as a gate-expander.

(c) REGISTRY MEMBERSHIP - the old ``run_landlab_susceptibility`` name is GONE;
    ``run_landlab`` is registered tier=door and ``landlab_susceptibility``
    tier=template.
...
from trid3nt_server.agent.tools.simulation.landlab.run_landlab.run_landlab import run_landlab

_DOOR = "run_landlab"
_TEMPLATE = "landlab_susceptibility"
...
def test_old_landlab_name_gone_and_door_template_registered():
    reg = agent_tools.TOOL_REGISTRY
    assert "run_landlab_susceptibility" not in reg, (
        "the old registered name must be REPLACED (no alias)"
    )
    door = reg[_DOOR].metadata
    assert door.engine == "landlab" and door.tier == "door"
    tmpl = reg[_TEMPLATE].metadata
    assert tmpl.engine == "landlab" and tmpl.tier == "template"
...
def test_door_listing_envelope_shape():
    ...
    # redirects name DOORS (+ the general model_debris_flow tool) and cover the
    # neighbouring hazards.
    redirect_text = " ".join(env["mismatch_redirect"].values())
    for target in ("run_sfincs", "run_openquake"):
        assert target in redirect_text, f"mismatch redirect must point at {target}"


def test_door_lists_only_registered_landlab_templates():
    reg = agent_tools.TOOL_REGISTRY
    env = run_landlab()
    listed = {c["tool_name"] for c in env["templates"]}
    assert _TEMPLATE in listed
    # every listed template is a REGISTERED engine=landlab tier=template (no
    # fabricated / not-yet-built template).
    for name in listed:
        m = reg[name].metadata
        assert m.engine == "landlab" and m.tier == "template", name
...
def test_door_listing_matches_registry_derivation():
    ...
    template names equals the set of engine=landlab tier=template registrations."""
    reg = agent_tools.TOOL_REGISTRY
    expected = sorted(
        n for n, e in reg.items()
        if getattr(e.metadata, "engine", None) == "landlab"
        and getattr(e.metadata, "tier", "general") == "template"
    )
    listed = sorted(c["tool_name"] for c in run_landlab()["templates"])
...
def test_template_excluded_door_present_in_index():
    ...
    try:
        ...
        assert _TEMPLATE not in idx.tool_names, (
            "landlab_susceptibility (tier=template) must be EXCLUDED from the default pool"
        )
...
def test_template_never_surfaces_in_retrieval():
    ...
    try:
        ...
        vis = tr.retrieve_visible_tools(
            "map landslide susceptibility and factor of safety across this catchment",
            None,
            k=40,
        )
...
def test_door_recognized_as_gate_expander():
    ...
    assert _DOOR in gx, "the Landlab door must be a registry-recognized gate-expander"
    edt = agent_server._engine_door_tool_names()
    assert _DOOR in edt, "the Landlab door must be an engine-door"

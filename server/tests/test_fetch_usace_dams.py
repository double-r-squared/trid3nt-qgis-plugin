...
from trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams import (
    CONUS_BBOX,
...
def test_user_agent_header_sent_on_request():
    ...
    with patch("trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams.httpx.Client", FakeClient):
        _fetch_nid_geojson_page(
...
def test_synthetic_response_writes_fgb_with_correct_count():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams._fetch_nid_all_features",
        return_value=fake_geojson,
    ), patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams.read_through",
...
def test_pagination_loop_stops_on_short_page():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams._fetch_nid_geojson_page",
...
def test_cache_miss_invokes_fetch_fn_then_hit_skips():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams._fetch_nid_bytes",
...
from trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams import (  # noqa: E402
    USACEDAMSAuthError,
...
def test_esri_499_token_required_raises_auth_error():
    with patch(
        "trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_dams.fetch_usace_dams.httpx.Client",

...
def la_catalogs(tmp_path, monkeypatch):
    ...
    from trid3nt_server.agent.tools.search import living_atlas_common as lac
    from trid3nt_server.agent.tools.search import living_atlas_index as lai
...
def test_search_authoritative_only_by_default(la_catalogs):
    from trid3nt_server.agent.tools.search.search_living_atlas import search_living_atlas
...
def no_probe(monkeypatch):
    ...
    fmod = importlib.import_module(
        "trid3nt_server.agent.tools.search.fetch_living_atlas_layer.fetch_living_atlas_layer"
    )
...
def test_fetch_image_service_dynamic_spec(la_catalogs, no_probe, monkeypatch):
    ...
    from trid3nt_server.agent.tools.search.fetch_living_atlas_layer import (
...
def test_harvest_normalizes_and_splits(tmp_path):
    ...
    script = Path(__file__).resolve().parents[2] / "scripts" / "harvest_living_atlas.py"

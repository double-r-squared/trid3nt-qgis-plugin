...
Three layers, mirroring the single-species contamination + archetype tests:
  ...
  * ``normalize_species_list`` honesty gates: empty / malformed / duplicate /
    sourceless species lists raise ``ContaminantPlumeInputError`` (never fabricate a
    contaminant).

  * The composer ``model_contaminant_plume`` full chain through a FAKE
...
def test_normalize_rejects_none() -> None:
    with pytest.raises(ContaminantPlumeInputError, match="non-empty species list"):
...
def test_normalize_rejects_malformed_species() -> None:
    with pytest.raises(ContaminantPlumeInputError, match="invalid species spec"):
        normalize_species_list([{"release_rate_kg_s": 0.01}])  # no name


def test_normalize_rejects_duplicate_names() -> None:
    with pytest.raises(ContaminantPlumeInputError, match="unique"):
        normalize_species_list(
            [
...
def test_normalize_rejects_sourceless_list() -> None:
    ...
    with pytest.raises(ContaminantPlumeInputError, match="positive release_rate_kg_s"):
        normalize_species_list(
            [
...
async def test_composer_requires_species(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ContaminantPlumeInputError):
...
async def test_composer_requires_exactly_one_source(
    ...
    _install_fake_tool("geocode_location", _fake_geocode, monkeypatch)
    with pytest.raises(ContaminantPlumeInputError, match="exactly one"):
        await model_contaminant_plume(
            location="Fort Myers",
            spill_location_latlon=(26.64, -81.87),
            species=TWO_SPECIES,
...
@pytest.mark.asyncio
async def test_composer_single_contaminant_yields_one_plume(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """N=1 known-answer (FOLD): the single-contaminant convenience pair routes
    through the SAME multi_species engine (archetype=multi_species, species len 1)
    and yields plumes[] of length 1 - the old run_modflow_job single behavior."""
    _install_fake_tool("geocode_location", _fake_geocode, monkeypatch)
    ...
    import trid3nt_server.agent.tools.simulation.run_modflow_multi_species_tool as tool
    ...
    assert len(result.plumes) == 1
    # Single-convenience normalized into a one-element multi_species species list.
    assert captured["run_args"].archetype == "multi_species"
    assert [s.name for s in captured["run_args"].species] == ["benzene"]
    assert captured["run_args"].species[0].release_rate_kg_s == pytest.approx(0.01)
    assert result.summary["n_species"] == 1
    assert result.plumes[0].max_concentration_mgl == pytest.approx(12.5)
...
async def test_composer_full_chain_two_plumes(
    ...
    """A fake run tool returning two plumes -> ContaminantPlumeResult with both layers
...
async def test_wrapper_missing_species_returns_user_input_required(
    ...
    _install_fake_tool("geocode_location", _fake_geocode, monkeypatch)
    out = await ms.modflow_contaminant_plume(
        spill_location_latlon=[26.64, -81.87], species=None, contaminant=None
    )
...
# --------------------------------------------------------------------------- #
# Registration + engine-door tier/engine tags (template, pool-excluded)
# --------------------------------------------------------------------------- #
...
def test_template_tagged_engine_and_tier() -> None:
    ...
    entry = TOOL_REGISTRY.get("modflow_contaminant_plume")
    assert entry is not None
    assert entry.metadata.engine == "modflow"
    assert entry.metadata.tier == "template"


def test_template_excluded_from_hazard_category() -> None:
    """Templates are pool-excluded: NOT categorized (only the run_modflow door is)."""
    from trid3nt_server.agent.categories import tools_for_category

    assert "modflow_contaminant_plume" not in tools_for_category("hazard_modeling")
...
def test_single_contaminant_convenience_normalizes_to_one_species() -> None:
    ...
    specs = normalize_species_list(None, contaminant="benzene", release_rate_kg_s=0.01)
    assert len(specs) == 1
    assert specs[0].name == "benzene"
    assert specs[0].release_rate_kg_s == pytest.approx(0.01)


def test_no_contaminant_and_no_species_raises() -> None:
    """Neither a single contaminant nor a species list -> honesty gate error."""
    with pytest.raises(ContaminantPlumeInputError):
        normalize_species_list(None)

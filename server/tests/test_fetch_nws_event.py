...
from trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event import (
    NWSError,
    NWSInputError,
    NWSUpstreamError,
...
def test_user_agent_header_sent_on_request():
    ...
    with patch("trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event.httpx.Client", FakeClient):
        from trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event import _fetch_nws_geojson
...
def test_fl_state_response_with_three_alerts_writes_fgb():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event._fetch_nws_geojson",
        return_value=fake_geojson,
    ), patch(
        "trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event.read_through",
...
def test_cache_miss_invokes_fetch_fn_then_hit_skips():
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event._fetch_nws_event_bytes",
...
def test_geojson_to_fgb_empty_collection_is_valid():
    ...
    from trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event import _geojson_to_fgb
...
def test_canonicalize_area_full_state_name(raw, code):
    from trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event import _canonicalize_area
...
def test_canonicalize_area_city_still_rejected():
    ...
    nationwide fallback."""
    from trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event import NWSInputError, _canonicalize_area
    with pytest.raises(NWSInputError, match="not a recognized"):

...
Live test (gated by TRID3NT_TEST_LIVE_NWPS=1): real NWPS request for the
...
from trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast import (
    GAUGE_DETAIL_URL,
    GAUGES_URL,
...
def test_fetch_happy_path_layeruri_shape() -> None:
    ...
    with patch(
        "trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast._http_get",
        return_value=_GAUGES_BODY,
    ), patch(
        "trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast.read_through",
        _stub_read_through,
    ):
...
def test_fetch_include_thresholds_enriches() -> None:
    ...
    import trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast as mod
...
@pytest.mark.skipif(
    os.environ.get("TRID3NT_TEST_LIVE_NWPS") != "1",
    reason="set TRID3NT_TEST_LIVE_NWPS=1 to run the live NWPS request",
)
def test_live_nwps_cedar_rapids() -> None:
    pytest.importorskip("geopandas")
    recs = _parse_gauges_json(
        __import__(
            "trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast", fromlist=["_http_get"]
...
def test_parse_stageflow_crest_and_series() -> None:
    from trid3nt_server.agent.tools.fetchers.hydrology.fetch_nws_river_forecast.fetch_nws_river_forecast import _parse_stageflow

"""``fetch_field_boundaries`` atomic tool -- agricultural FIELD-BOUNDARY vectors from Fields of The World (FTW) / fiboa PUBLISHED GeoParquet datasets. Serves EXISTING published polygons only (no on-demand imagery inference); raises a typed ``FIELDS_NO_COVERAGE`` error where the benchmark has no coverage for the bbox, never fabricates polygons.
"""
...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_field_boundaries.fetch_field_boundaries")
...
def _gdf_to_flatgeobuf_bytes(gdf: Any) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_ftw_fields_"
        ) as f:
            tmp_fgb = f.name
        try:

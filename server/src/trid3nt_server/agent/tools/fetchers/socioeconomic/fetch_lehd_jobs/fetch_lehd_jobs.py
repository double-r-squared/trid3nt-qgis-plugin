"""``fetch_lehd_jobs`` atomic tool -- Census LEHD LODES workplace-area employment, aggregated to census tract, as a choropleth FlatGeobuf.
"""
...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_lehd_jobs.fetch_lehd_jobs")
...
def _features_to_flatgeobuf(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_lehd_jobs_"
        ) as f:
            tmp_fgb = f.name
        try:

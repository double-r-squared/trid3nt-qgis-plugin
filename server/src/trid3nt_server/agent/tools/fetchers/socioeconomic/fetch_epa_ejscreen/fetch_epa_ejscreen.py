...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.socioeconomic.fetch_epa_ejscreen.fetch_epa_ejscreen")
...
def _features_to_flatgeobuf(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_epa_ejscreen_"
        ) as f:
            tmp_fgb = f.name
        try:
...
def fetch_epa_ejscreen(
    ...
    **Cross-tool dependencies:**
        ...
        - Feeds ``compute_zonal_statistics`` / ``spatial_query`` to find

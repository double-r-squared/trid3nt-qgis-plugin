...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.biodiversity.fetch_inaturalist_observations.fetch_inaturalist_observations")
...
def _records_to_flatgeobuf_bytes(records: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".fgb", delete=False, prefix="trid3nt_inat_") as f:
            tmp_fgb = f.name
        try:

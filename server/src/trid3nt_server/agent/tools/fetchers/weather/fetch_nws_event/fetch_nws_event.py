...
from trid3nt_server.agent.tools.fetchers.us_states import NWS_AREA_CODES, resolve_state_code
...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.weather.fetch_nws_event.fetch_nws_event")
...
def fetch_nws_event(
    ...
    **Parameters:**
    - ``area`` (str or tuple): 2-letter state code (``"FL"``) or full state
      ...
      ``fetch_administrative_boundaries`` + ``spatial_query``

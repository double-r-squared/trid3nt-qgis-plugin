"""``fetch_nws_alerts_conus`` atomic tool — NWS CONUS-wide active alerts.
"""
...
from trid3nt_server.agent.tools.fetchers.us_states import resolve_state_code, state_display_name
...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.weather.fetch_nws_alerts_conus.fetch_nws_alerts_conus")
...
def fetch_nws_alerts_conus(
    ...
    **When NOT to use:**
    - County- or bbox-scoped queries — use ``fetch_nws_event`` with a 5-digit
      ...
      ``spatial_query``) rather than trusting a rectangle.

...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.climate.fetch_us_drought_monitor.fetch_us_drought_monitor")
...
def _features_to_flatgeobuf(features: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_usdm_"
        ) as f:
            tmp_fgb = f.name
        try:
...
def fetch_us_drought_monitor(
    ...
    **Cross-tool dependencies:**
        - Feeds INTO: ``compute_zonal_statistics`` (assets/acres inside a
          drought footprint), ``spatial_query`` (admin/watershed-scoped

...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.climate.fetch_climate_normals.fetch_climate_normals")
...
def _records_to_fgb(records: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_normals_"
        ) as f:
            tmp_fgb = f.name
        try:

...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.hydrology.fetch_nwi_wetlands.fetch_nwi_wetlands")
...
def _features_to_flatgeobuf(features: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_nwi_wetlands_"
        ) as f:
            tmp_fgb = f.name
        try:

...
logger = logging.getLogger(
    "trid3nt_server.agent.tools.fetchers.hydrology.fetch_high_water_marks.fetch_high_water_marks"
)
...
# Generous (~0.5 deg pad) per-state bboxes -- self-contained (the asos twin that
# once owned this table is folded to a router spec; a small data copy keeps this
# coded fetcher off the router internals).
...
def _bbox_overlaps_state(
    bbox: tuple[float, float, float, float], sb: tuple[float, float, float, float]
) -> bool:
    w1, s1, e1, n1 = bbox
    w2, s2, e2, n2 = sb

...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.hazard.fetch_usace_levees.fetch_usace_levees")
...
def _geojson_to_fgb(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_usace_levees_"
        ) as f:
            tmp_fgb = f.name
        try:
...
def fetch_usace_levees(
    ...
    **_extra_ignored: Any,
    ...
    ``spatial_query`` (clip to a state/county polygon),

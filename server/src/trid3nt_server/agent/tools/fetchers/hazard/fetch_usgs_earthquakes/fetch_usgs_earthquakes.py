...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.hazard.fetch_usgs_earthquakes.fetch_usgs_earthquakes")
...
def _build_flatgeobuf(records: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_eq_"
        ) as f:
            tmp_fgb = f.name

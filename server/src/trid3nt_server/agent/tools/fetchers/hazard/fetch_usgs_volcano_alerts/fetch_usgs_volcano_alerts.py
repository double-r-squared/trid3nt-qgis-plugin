...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.hazard.fetch_usgs_volcano_alerts.fetch_usgs_volcano_alerts")
...
def _build_flatgeobuf(records: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_volc_"
        ) as f:
            tmp_fgb = f.name

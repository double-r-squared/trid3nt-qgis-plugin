"""``fetch_nifc_fire_perimeters`` atomic tool — NIFC current wildfire perimeters.
"""
...
logger = logging.getLogger("trid3nt_server.agent.tools.fetchers.hazard.fetch_nifc_fire_perimeters.fetch_nifc_fire_perimeters")
...
def _geojson_to_fgb(geojson: dict[str, Any]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_nifc_"
        ) as f:
            tmp_fgb = f.name
        try:

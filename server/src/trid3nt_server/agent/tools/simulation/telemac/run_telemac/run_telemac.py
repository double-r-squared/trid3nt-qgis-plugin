"""Engine door: ``run_telemac`` - the read-only TELEMAC river-transport concierge.

ROUTING - call this FIRST for ANY river DYE / TRACER / CONTAMINANT transport
question (a spill that TRAVELS DOWNSTREAM in surface water), then
SELECT-THEN-CALL the template it names:
  dye / tracer plume in a river reach, oil-spill slick drifting downstream,
  sewage / bacterial effluent decaying as it travels, a sediment / silt slurry
  settling on the bed, any "contaminant spilled into the river, how far does it
  go downstream" -> call ``run_telemac`` to LIST the available TELEMAC
  templates, then call the chosen ``telemac_*`` template directly.
...
turn (gate expansion), and (3) briefs the model on TELEMAC fidelity and redirects
off-engine asks to the right door.

Do NOT use for:
  - groundwater / aquifer contaminant plume or river<->aquifer seepage ->
    run_modflow
  - surface-water inundation / flooding DEPTH (not tracer transport) ->
    run_sfincs (riverine / coastal) or run_swmm (urban storm sewer)
  - tsunami / dam-break / surge run-up (adaptive finite volume) -> run_geoclaw
...
#: Static fidelity brief (narrated by the LLM). TELEMAC-2D full-physics
#: depth-averaged shallow water on an unstructured mesh - a river tracer plume
#: that travels downstream in surface water; idealized-bed demo, planning-grade.
_FIDELITY_BRIEF = (
    "TELEMAC-2D full-physics depth-averaged shallow-water engine (unstructured "
    "mesh over a REAL river reach). Models a DYE / TRACER / CONTAMINANT plume "
    "released at a mid-reach point source that is advected + diluted DOWNSTREAM "
    "in the SURFACE water (optional oil-slick, WAQTEL decay, or GAIA sediment "
    "modules by substance). Produces a peak concentration map + a time-stepped "
    "animated mesh. Idealized-bed planning-grade demo (no site bathymetry "
    "fetcher), NOT a calibrated regulatory transport model."
)

#: Off-engine redirection map (prose the LLM narrates; it names DOORS, not
#: templates). River surface-water tracer transport belongs here; everything
#: else points away.
_MISMATCH_REDIRECT = {
    "groundwater / aquifer contaminant plume or river-aquifer seepage": "run_modflow",
    "surface-water inundation / flooding depth (riverine / coastal)": "run_sfincs",
    "urban storm-sewer / pipe-network flooding": "run_swmm",
    "tsunami / dam-break / surge run-up (adaptive finite volume)": "run_geoclaw",
}

_NEXT_ACTION = (
    "SELECT-THEN-CALL: call the chosen telemac_* template directly with its "
    "required inputs."
)
...
def _list_templates() -> list[dict[str, Any]]:
    """Registry-driven TELEMAC template listing (deterministic, sorted).

    Enumerates ``TOOL_REGISTRY`` for entries tagged ``engine == "telemac"`` and
    ...
    for name in sorted(TOOL_REGISTRY.keys()):
        ...
        try:
            ...
            logger.warning("run_telemac door: card derivation failed for %s", name,
                           exc_info=True)
    return cards
...
def run_telemac() -> dict[str, Any]:
    """List the TELEMAC river-transport templates, then SELECT-THEN-CALL one.

    Read-only concierge for the TELEMAC-2D river-transport engine - call this
    FIRST for any river DYE / TRACER / CONTAMINANT transport question (a spill
    that travels DOWNSTREAM in surface water: dye / tracer plume, oil-spill
    slick, decaying sewage effluent, a settling sediment slurry). It EXECUTES
    NOTHING: it returns the available ``telemac_*`` templates (each with its
    one-line question, required inputs, and knobs) and makes them callable for

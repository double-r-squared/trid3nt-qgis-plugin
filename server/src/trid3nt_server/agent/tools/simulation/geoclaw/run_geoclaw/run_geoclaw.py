"""Engine door: ``run_geoclaw`` - the read-only GeoClaw inundation concierge.

ROUTING - call this FIRST for ANY tsunami / dam-break / surge run-up question,
then SELECT-THEN-CALL the template it names:
  tsunami run-up over terrain, dam / levee / embankment failure flood wave,
  reservoir release routed downstream, storm-surge run-up inundation, wave
  run-up + peak overland depth from an offshore earthquake -> call
  ``run_geoclaw`` to LIST the available GeoClaw templates, then call the chosen
  ``geoclaw_*`` template directly.
...
turn (gate expansion), and (3) briefs the model on GeoClaw fidelity and
redirects off-engine asks to the right door.

Do NOT use for:
  - pluvial / riverine / coastal compound flooding -> run_sfincs
...
#: Static fidelity brief (narrated by the LLM). GeoClaw / Clawpack adaptive-mesh
#: finite-volume shallow-water engine - tsunami / dam-break / surge run-up with
#: AMR; batch-only Fortran solver, planning-grade, not a calibrated model.
_FIDELITY_BRIEF = (
    "GeoClaw / Clawpack adaptive-mesh finite-volume shallow-water engine "
    "(TSUNAMI / DAM-BREAK / storm-SURGE run-up inundation; adaptive mesh "
    "refinement over terrain + bathymetry). Produces a peak run-up depth raster "
    "+ scrubber animation for the chosen source. Batch-only Fortran solver; "
    "planning-grade run-up envelope, NOT a calibrated regulatory model."
)

#: Off-engine redirection map (prose the LLM narrates; it names DOORS, not
#: templates). Tsunami / dam-break / surge run-up belongs here; everything else
#: points away.
_MISMATCH_REDIRECT = {
    "pluvial / riverine / coastal compound flooding": "run_sfincs",
    "urban storm-sewer / pipe-network flooding": "run_swmm",
...
_NEXT_ACTION = (
    "SELECT-THEN-CALL: call the chosen geoclaw_* template directly with its "
    "required inputs."
)
...
def _list_templates() -> list[dict[str, Any]]:
    """Registry-driven GeoClaw template listing (deterministic, sorted).

    Enumerates ``TOOL_REGISTRY`` for entries tagged ``engine == "geoclaw"`` and
    ...
    for name in sorted(TOOL_REGISTRY.keys()):
        ...
        try:
            ...
            logger.warning("run_geoclaw door: card derivation failed for %s", name,
                           exc_info=True)
    return cards
...
def run_geoclaw() -> dict[str, Any]:
    """List the GeoClaw run-up templates, then SELECT-THEN-CALL one.

    Read-only concierge for the GeoClaw inundation engine - call this FIRST for
    any tsunami / dam-break / surge run-up question (tsunami run-up over terrain,
    dam / levee / embankment failure flood wave, reservoir release routed
    downstream, storm-surge run-up inundation, wave run-up + peak overland depth
    from an offshore earthquake). It EXECUTES NOTHING: it returns the available
    ``geoclaw_*`` templates (each with its one-line question, required inputs,
    and knobs) and makes them callable for the rest of the turn. Then call the
    chosen template directly.

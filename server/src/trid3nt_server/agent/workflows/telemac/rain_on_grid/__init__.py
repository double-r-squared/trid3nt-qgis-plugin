...
template is built from:
  ...
  * ``cn_infiltration`` (ADR 0195) -- SCS curve-number infiltration: the native
  ...
  * ``mesh_acquisition`` (ADR 0196) -- the watershed-first meshing STEP promoted
    from the ADR 0193 sandbox: delineate the catchment at a pour point, mesh its
...
substrate; see ADR 0196 for the build plan.
"""

...
Backend: ``services/workers/hecras2025/subst/crux/freshtopo/culvert_reach_pipeline.py``
...
logger = logging.getLogger(
    "trid3nt_server.agent.workflows.hecras.culvert_embankment_flow.culvert_embankment_flow"
)
...
#: carries no hard dependency on services/workers (mirrors hecras_flood_2d).
_WORKERS_FRESHTOPO = (
    ...
    *,
    ...
    review_entries: list[SyntheticInput] = [
        ...
        SyntheticInput(param="barrel_diameter_m", value=round(float(barrel_diameter_m), 3), units="m",
                       basis="derived" if abs(barrel_diameter_m - _DEFAULT_DIAMETER_M) < 1e-9 else "user",
                       note="culvert rise/span -- UN-FETCHABLE engineering (labeled)"),
        SyntheticInput(param="opening_type", value=opening_type, basis="derived",
                       note="inlet-control chart/scale (entrance geometry)"),
        SyntheticInput(param="entrance_exit_loss", value=f"KIn={k_in}, KOut={k_out}", basis="derived",
                       note="culvert entrance/exit loss coefficients"),
        SyntheticInput(param="barrel_manning", value=round(float(barrel_manning), 4), basis="derived",
                       note="barrel roughness n"),

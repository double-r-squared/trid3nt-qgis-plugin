...
logger = logging.getLogger(
    "trid3nt_server.agent.workflows.swmm.dual_drainage.dual_drainage"
)
...
async def model_swmm_dual_drainage(
    ...
    *,
    ...
    from trid3nt_server.agent.workflows.swmm.postprocess_swmm import postprocess_swmm
    from trid3nt_server.agent.workflows.swmm.urban_flood.urban_flood import (
    ...
    if dem_path is None:
        async with substep(emitter, "fetch_dem"):
            dem_path, dem_source = await asyncio.to_thread(_fetch_dem_for_urban, bbox)
    else:
        dem_source = "supplied"

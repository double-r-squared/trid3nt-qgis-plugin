...
logger = logging.getLogger(
    "trid3nt_server.agent.workflows.modflow.model_groundwater_contamination_scenario.model_groundwater_contamination_scenario"
)
...
class Case2Result(GraceModel):
    ...
    ``summary`` dict mirrors those plus the derived release rate / duration -- all computed, none free-generated.
...
def _extract_contaminant(text: str) -> str | None:
    ...
    from trid3nt_server.agent.tools.processing.aggregate_claims_across_sources.aggregate_claims_across_sources import _extract_contaminants
    ...
    **_extra_ignored: Any,
    ...
    Do NOT use this for:
        ...
        - A spill with explicit numeric parameters already in hand (call
          ``run_modflow`` directly with the forcing fields).
        - Ingesting a news event WITHOUT modeling it (compose
          ``web_fetch`` + ``geocode_location`` in the ``code_exec_request``
          playground to derive params for review -- see
    ...
    Cross-tool dependencies:
        ...
        location + scale extraction), ``geocode_location`` (location ->
        spill point), ``run_modflow`` (deck build -> mf6 -> postprocess ->
        publish -> ``PlumeLayerURI``).
    """

...
from trid3nt_server.agent.workflows.shared.mesh_layer import make_sfincs_mesh_layer_uri
from trid3nt_server.agent.workflows.sfincs.postprocess_flood import (
    FLOOD_DEPTH_STYLE_PRESET,
    PostprocessError,
    postprocess_flood,
)
from trid3nt_server.agent.workflows.sfincs.postprocess_waves import (
...
def _estimate_frame_count(
    *, output_interval_min: float | None, duration_hr: float
    ...
    """
    from trid3nt_server.agent.workflows.sfincs.postprocess_flood import MAX_FLOOD_FRAMES
...
def _upload_local_forcing_files_to_s3(
    ...
    *,
    ...
    from trid3nt_server.agent.tools.simulation.solver.solver import SolverDispatchError as _DeckBuildError
...
def _compose_and_upload_flood_build_spec(
    *,
    ...
    path when the offload is enabled. Runs the §4 NLCD validation gate
...
def _resolve_quadtree_rivers_uri(
    *,
    ...
    building footprints + the land/pluvial river branch). A successful
...
async def model_flood_scenario(
    ...
    *,
    ...
    try:
        ...
        # scenario-coverage auto-wire precedence ladder. Each
    ...
    # Auto vertical scaling per case: size the Batch
    ...
    # The combined quadtree solved (Step 5.5 returned its solve
    ...
    # NATE (coastal North Star): when the COMBINED quadtree job ran,
    ...
    if not register_only:
        ...
        # --- Step 9c: postprocess + emit the SnapWave WAVE field ---
        ...
        if quadtree_run_result is not None:
            try:
                # surface the wave postprocess as a nested child row. A

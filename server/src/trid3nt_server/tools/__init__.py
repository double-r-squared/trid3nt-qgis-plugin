...
class RegisteredTool:
    ...
    - ``module`` - the ``__module__`` attribute at registration time, useful
      for diagnostics (`"trid3nt_server.tools.passthroughs"` etc.).
    """
...
#
# Importing ``trid3nt_server.tools`` should populate ``TOOL_REGISTRY`` with
...
from . import compute_model_residuals  # noqa: E402,F401 -- observed-vs-modeled residuals: registers compute_model_residuals (bilinear-samples a MODEL raster, e.g. a MODFLOW simulated-head or plume-concentration COG, at OBSERVATION points -> per-point observed/simulated/residual FlatGeobuf with a diverging red-blue legend + n_points/mean_error/bias/rmse/mae/min-max-residual/interpretation on the LayerURI subclass; observations_layer_uri override or bbox-driven internal fetch_usgs_groundwater_levels shared-core fetch; honest ALWAYS-populated units_warning distinguishing USGS depth-to-water pcodes 72019/61055 from elevation-referenced pcodes 72150/62610/62611, filtering mixed fetches to the elevation-referenced subset; typed no-observations/all-nodata errors + small-n caveat)

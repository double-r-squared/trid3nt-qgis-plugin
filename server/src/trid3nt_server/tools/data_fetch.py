...
- The three layer-producing tools return ``LayerURI`` (from
  ``trid3nt_contracts.execution``) so downstream visualization seams (map-
  command ``load-layer``) consume them with zero translation.
- ``geocode_location`` returns a plain ``dict`` for now — there is no
  ``GeocodedLocation`` pydantic model in ``trid3nt-contracts`` yet (FROZEN).
...
logger = logging.getLogger("trid3nt_server.tools.data_fetch")

...
3. ``TRID3NT_OPENAQ_API_KEY`` env var (local dev convenience).
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.weather.fetch_openaq_measurements")
...
def _resolve_api_key(
    ...
    raise OpenAQMissingKeyError(
        ...
        "the TRID3NT_OPENAQ_API_KEY env var. Register a free key at "
...
def _rows_to_flatgeobuf_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_openaq_"
        ) as f:
            tmp_fgb = f.name
        try:

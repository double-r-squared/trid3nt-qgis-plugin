...
1. AUTHORITATIVE: the USACE NID ArcGIS REST server at
   ...
   ``TRID3NT_USACE_NID_TOKEN`` env), the SAME pattern eBird / ERA5 / GTSM
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.hazard.fetch_usace_dams")
...
def _resolve_nid_token(
    ...
    3. ``TRID3NT_USACE_NID_TOKEN`` env var (dev convenience).
...
def _geojson_to_fgb(geojson: dict[str, Any]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_usace_dams_"
        ) as f:
            tmp_fgb = f.name
        try:
...
def fetch_usace_dams(
    ...
    **_extra_ignored: Any,
    ...
    What it does:
        ...
        (kwarg / per-Case ``secret_ref`` / ``TRID3NT_USACE_NID_TOKEN`` env) it

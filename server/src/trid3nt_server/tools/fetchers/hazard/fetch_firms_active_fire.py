...
MAP_KEY (FR-AS-11 / OQ surfaced):
    ...
    ``TRID3NT_FIRMS_MAP_KEY`` env var, falling back to the literal ``"demo"``
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.hazard.fetch_firms_active_fire")
...
# vault -> TRID3NT_FIRMS_MAP_KEY env -> "demo" literal fallback.
#
...
def _resolve_map_key(
    ...
    # 3. Env var fallback.
    env_key = os.environ.get("TRID3NT_FIRMS_MAP_KEY")
    if env_key:
        return env_key
...
def _parse_firms_csv_to_fgb(csv_text: str) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_firms_"
        ) as tf:
            tmp_path = tf.name
        try:
...
def _fetch_firms_csv(
    ...
    # user to set TRID3NT_FIRMS_MAP_KEY rather than retry.
...
def fetch_firms_active_fire(
    ...
    **What it does:** Calls the NASA FIRMS Web Service AREA API
    ...
    (``TRID3NT_FIRMS_MAP_KEY`` env var). Does not support global queries

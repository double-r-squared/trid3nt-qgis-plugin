...
2. ``secret_ref`` (a ``SecretRecord`` per ``trid3nt_contracts.secrets``)
   → ``Persistence.get_secret_value()`` (the production per-Case path
   landed by Wave 2 sibling job-0124).
3. ``TRID3NT_COPERNICUS_CDS_API_KEY`` env var (local dev convenience).
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.climate.fetch_era5_reanalysis")
...
def _resolve_api_key(
    ...
    # 3. Env var fallback.
    env_key = os.environ.get("TRID3NT_COPERNICUS_CDS_API_KEY")
    if env_key:
        return env_key
...
def _da_to_cog_bytes(da: Any, variable: str) -> bytes:
    ...
    out_fd, out_path = tempfile.mkstemp(suffix=".tif", prefix="trid3nt_era5_")
    os.close(out_fd)
    try:
...
def _fetch_era5_bytes(
    ...
    if variable == _DERIVED_WIND_SPEED:
        u_var, v_var = _WIND_SPEED_COMPONENTS
        u_fd, u_path = tempfile.mkstemp(suffix=".nc", prefix="trid3nt_era5_cds_u_")
        os.close(u_fd)
        v_fd, v_path = tempfile.mkstemp(suffix=".nc", prefix="trid3nt_era5_cds_v_")
        os.close(v_fd)
        try:
    ...
    nc_fd, nc_path = tempfile.mkstemp(
        suffix=".nc", prefix="trid3nt_era5_cds_"
    )
    os.close(nc_fd)
    try:
...
def fetch_era5_reanalysis(
    ...
    **Parameters:**
    ...
    - ``api_key`` (str | None): explicit CDS key; overrides all other resolution
      ...
      ``TRID3NT_COPERNICUS_CDS_API_KEY`` → ``~/.cdsapirc``.

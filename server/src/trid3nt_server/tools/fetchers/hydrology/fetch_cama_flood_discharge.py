...
Source resolution priority (kickoff-extended for migration reality):
  ...
  2. ``TRID3NT_CAMA_FLOOD_BASE_URL`` env var.
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.hydrology.fetch_cama_flood_discharge")
...
def _resolve_base_url(base_url: str | None) -> str:
    ...
    env_url = os.environ.get("TRID3NT_CAMA_FLOOD_BASE_URL")
    if env_url:
    ...
    try:
        ...
        if saw_html_sentinel:
            raise CaMaFloodUnreachableError(
                ...
                "Dropbox password. Configure TRID3NT_CAMA_FLOOD_BASE_URL or "
...
def _netcdf_to_cog_bytes(
    ...
    try:
        ...
        out_fd, out_path = tempfile.mkstemp(
            suffix=".tif", prefix="trid3nt_cama_"
        )
        os.close(out_fd)
        try:
    ...
    try:
        for yr in years:
            fd, nc_path = tempfile.mkstemp(
                suffix=f"_{yr}.nc", prefix="trid3nt_cama_"
            )
            os.close(fd)
        ...
        if len(nc_paths) == 1:
            ...
            try:
                ...
                combo_fd, combo_path = tempfile.mkstemp(
                    suffix=".nc", prefix="trid3nt_cama_combo_"
                )
                os.close(combo_fd)
                try:
...
def fetch_cama_flood_discharge(
    ...
    **Parameters:**
    ...
    - ``base_url``: optional mirror base URL. Falls back to
      ``TRID3NT_CAMA_FLOOD_BASE_URL`` env var, then the legacy Tokyo path.

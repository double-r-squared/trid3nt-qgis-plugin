...
3. ``TRID3NT_EBIRD_API_KEY`` env var (local dev convenience).
...
logger = logging.getLogger("trid3nt_server.tools.fetchers.biodiversity.fetch_ebird_observations")
...
def _resolve_api_key(
    ...
    # 3. Env var fallback.
    env_key = os.environ.get("TRID3NT_EBIRD_API_KEY")
    if env_key:
        return env_key

    raise EBirdMissingKeyError(
        "no eBird API key available: pass api_key=..., secret_ref=..., "
        "or set the TRID3NT_EBIRD_API_KEY env var. Register at "
...
def _records_to_flatgeobuf_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_ebird_"
        ) as fgb_f:
            tmp_fgb = fgb_f.name
        try:

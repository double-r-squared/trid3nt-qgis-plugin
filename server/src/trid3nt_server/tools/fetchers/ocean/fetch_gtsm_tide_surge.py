...
logger = logging.getLogger("trid3nt_server.tools.fetchers.ocean.fetch_gtsm_tide_surge")
...
def _records_to_flatgeobuf_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="trid3nt_gtsm_"
        ) as fgb_f:
            tmp_fgb = fgb_f.name
        try:
...
def _fetch_gtsm_bytes(
    ...
    zip_fd, zip_path = tempfile.mkstemp(
        suffix=".zip", prefix="trid3nt_gtsm_cds_"
    )
    os.close(zip_fd)
    nc_paths: list[str] = []
    try:

...
logger = logging.getLogger("trid3nt_server.tools.discovery.search_tools.search_tools")
...
def _default_corpus_path() -> Path:
    """Resolve the residual ``data/tool_query_corpus.yaml`` under the package.

    Post engine-door restructure this file is the RESIDUAL corpus: it holds
    only the query entries for tools registered OUTSIDE the ``tools/`` tree
    ...
    # tools/discovery/search_tools/search_tools.py -> trid3nt_server/ is parents[3]
    here = Path(__file__).resolve()
    return here.parents[3] / "data" / "tool_query_corpus.yaml"
...
def _compose_corpus_from_tree() -> dict[str, list[str]]:
    ...
    """
    here = Path(__file__).resolve()
    tools_dir = here.parents[2]  # .../trid3nt_server/tools
    ...
    residual = here.parents[3] / "data" / "tool_query_corpus.yaml"

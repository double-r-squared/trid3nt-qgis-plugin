...
logger = logging.getLogger("trid3nt_server.tools.discovery.qgis_discovery.qgis_discovery")
...
# True)`` (or the ``TRID3NT_QGIS_ALLOWLIST=all`` env flip) returns the full
...
def curated_allowlist() -> tuple[frozenset[str], frozenset[str]]:
    ...
    module constants above; ops can override via ``TRID3NT_QGIS_ALLOWLIST``:

    - ``TRID3NT_QGIS_ALLOWLIST=all`` (or ``*``) -> sentinel: both sets empty,
      which ``_apply_curated_allowlist`` reads as "return everything" (the
      same effect as the ``include_all=True`` call-site escape hatch).
    - ``TRID3NT_QGIS_ALLOWLIST=native:*,gdal:*,grass:r.watershed,...`` -> a
    ...
    """
    raw = (os.environ.get("TRID3NT_QGIS_ALLOWLIST") or "").strip()
    if not raw:
...
def _get_worker_submitter():
    ...
    """
    from trid3nt_server.tools.meta.passthroughs import passthroughs
...
def list_qgis_algorithms(
    ...
    **_extra_ignored: Any,
    ...
    Curated default:
        ...
        full unfiltered catalog (or set ``TRID3NT_QGIS_ALLOWLIST=all`` ops-side).

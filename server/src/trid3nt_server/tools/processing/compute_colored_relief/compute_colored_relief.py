...
from trid3nt_server.tools.processing.compute_hillshade.compute_hillshade import _gdaldem_subprocess_env, _translate_to_cog

__all__ = ["compute_colored_relief"]

logger = logging.getLogger("trid3nt_server.tools.processing.compute_colored_relief.compute_colored_relief")
...
def compute_colored_relief(
    ...
    **_extra_ignored: Any,
    ...
    Returns:
        ``LayerURI`` for a 3/4-band Cloud-Optimized GeoTIFF (cache bucket,
        TTL 30d; same CRS/extent as the DEM; RGB 0-255 bands, not
        elevation).
    """

...
logger = logging.getLogger("trid3nt_server.tools.processing.compute_canopy_height.compute_canopy_height")
...
async def _run_canopy_chain(
    *,
    ...
    from trid3nt_server.tools.simulation.solver.solver import run_solver, select_compute_class, wait_for_completion
...
def _resolve_cog_from_result(run_result: Any, batch_run_id: str) -> str | None:
    ...
    from trid3nt_server.tools.simulation.solver.solver import _get_runs_bucket, _try_get_completion_s3  # type: ignore[attr-defined]
...
async def _fetch_naip_rgb_uri(bbox: tuple[float, float, float, float]) -> str:
    ...
    """
    from trid3nt_server.tools.fetchers.imagery.fetch_naip.fetch_naip import fetch_naip

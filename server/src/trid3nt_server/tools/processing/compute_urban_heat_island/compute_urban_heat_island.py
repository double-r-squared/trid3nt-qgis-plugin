...
logger = logging.getLogger("trid3nt_server.tools.processing.compute_urban_heat_island.compute_urban_heat_island")
    ...
    **_extra_ignored: Any,
    ...
    with tempfile.TemporaryDirectory(prefix="trid3nt_uhi_") as tmpdir:
        ...
        if landcover_uri is not None:
            ...
            try:
                from trid3nt_server.tools.fetchers.terrain.fetch_esri_landcover_10m.fetch_esri_landcover_10m import fetch_esri_landcover_10m
        ...
        try:
            ...
            if lst_uri is not None:
                ...
                try:
                    from trid3nt_server.tools.fetchers.climate.fetch_modis_lst.fetch_modis_lst import fetch_modis_lst

...
LOG = logging.getLogger("trid3nt.agent.sandbox_hardening")
...
def hardening_enabled() -> bool:
    """True unless ``TRID3NT_SANDBOX_HARDENED`` is explicitly falsey.
...
def build_child_env(base_env: dict[str, str], *, cap_seconds: int) -> dict[str, str]:
    ...
    Always-passed sandbox knobs the executor + guard read:
      ...
      * ``AWS_EC2_METADATA_DISABLED=true`` + ``TRID3NT_SANDBOX_HARDENED=1`` so the
    ...
    # Sandbox knobs.
    child["TRID3NT_SANDBOX_TIMEOUT"] = str(cap_seconds)
    child["TRID3NT_SANDBOX_HARDENED"] = "1"
    ...
    child["TRID3NT_SANDBOX_NET_ALLOW"] = "localhost,127.0.0.1,::1,0.0.0.0"
...
def rlimit_spec() -> dict[str, int]:
    ...
    """
    return {
        "as_bytes": _int_env("TRID3NT_SANDBOX_RLIMIT_AS_MB", 2048) * 1024 * 1024,
        "cpu_seconds": _int_env("TRID3NT_SANDBOX_RLIMIT_CPU_SECONDS", 120),
        "nproc": _int_env("TRID3NT_SANDBOX_RLIMIT_NPROC", 256),
        "fsize_bytes": _int_env("TRID3NT_SANDBOX_RLIMIT_FSIZE_MB", 512) * 1024 * 1024,
    }
...
def _bwrap_mode() -> str:
    ...
    """
    raw = (os.environ.get("TRID3NT_SANDBOX_BWRAP") or "").strip().lower()
    if raw in ("1", "true", "yes", "on", "require"):
        return "1"
    if raw in ("0", "false", "no", "off"):
        return "0"
    if not raw:
        backend = (os.environ.get("TRID3NT_STORAGE_BACKEND") or "").strip().lower()
...
def _bwrap_bin() -> str | None:
    override = (os.environ.get("TRID3NT_SANDBOX_BWRAP_BIN") or "").strip()
    if override:
...
class JailUnavailable(RuntimeError):
    """``TRID3NT_SANDBOX_BWRAP=1`` required the jail but ``bwrap`` is absent."""
...
def _editable_source_roots() -> list[str]:
    ...
    The TRID3NT packages (``trid3nt_contracts`` for chart-emission shaping that the
...
def wrap_with_jail(
    ...
    *,
    ...
    Returns ``None`` when the jail is disabled (``TRID3NT_SANDBOX_BWRAP=0``) or
    ``auto`` + no ``bwrap`` present (caller falls back to Layer-A-only). Raises
    :class:`JailUnavailable` when ``TRID3NT_SANDBOX_BWRAP=1`` but ``bwrap`` is
    ...
    if bwrap is None:
        if mode == "1":
            raise JailUnavailable(
                "TRID3NT_SANDBOX_BWRAP=1 requires the bubblewrap (bwrap) binary, "
                "which was not found on PATH (set TRID3NT_SANDBOX_BWRAP_BIN or "

...
(``services/agent/sandbox_runner.py`` with ``TRID3NT_SANDBOX_LOCAL=1``) so dev/test
...
(GCS endpoints + the Atlas host, supplied via ``TRID3NT_SANDBOX_NET_ALLOW``). A
...
MAX_OUTPUT_CHARS = int(os.environ.get("TRID3NT_SANDBOX_MAX_OUTPUT", "65536"))
...
MAX_DATAFRAME_ROWS = int(os.environ.get("TRID3NT_SANDBOX_MAX_DF_ROWS", "5000"))
...
MAX_RESULT_BYTES = int(os.environ.get("TRID3NT_SANDBOX_MAX_RESULT_BYTES", str(2 * 1024 * 1024)))
...
NET_ALLOW_SUFFIXES = tuple(
    s.strip().lower()
    for s in os.environ.get("TRID3NT_SANDBOX_NET_ALLOW", DEFAULT_NET_ALLOW).split(",")
    if s.strip()
)
...
#     TRID3NT_SANDBOX_ENVELOPE_V1 {"status": "ok", ...}
...
ENVELOPE_MARKER = "TRID3NT_SANDBOX_ENVELOPE_V1"
...
def convert_result(result: Any) -> dict[str, Any]:
    ...
    Returns a dict with a ``kind`` discriminator:
      - ``matplotlib.figure.Figure``  -> {"kind": "chart", "chart_emission": {...},
                                          ...
                                          trid3nt_contracts is importable, else a
...
def _convert_figure(fig: Any) -> dict[str, Any]:
    ...
    try:
        import trid3nt_contracts.chart_contracts as _cc  # noqa: PLC0415, F401

...
override ``TRID3NT_CASE_QGS_BUCKET`` lets a future infra job split the
...
def _get_qgs_bucket() -> str:
    ...
    Order: ``TRID3NT_CASE_QGS_BUCKET`` env var > ``DEFAULT_CASE_QGS_BUCKET``.
    The bucket name is NOT prefixed with ``gs://`` — callers concatenate.
    """
    return os.environ.get("TRID3NT_CASE_QGS_BUCKET", DEFAULT_CASE_QGS_BUCKET)


def _get_template_qgs_uri() -> str:
    """Resolve the template ``.qgs`` URI to seed-from on first publish.

    Order: ``TRID3NT_CASE_QGS_TEMPLATE`` env var > ``DEFAULT_PROJECT_QGS_URI``
    (the canonical single-tenant project from ``tools.publish_layer``).
    """
    return os.environ.get("TRID3NT_CASE_QGS_TEMPLATE", DEFAULT_PROJECT_QGS_URI)
    ...
    *,
    ...
    Args:
        ...
        template_qgs_uri: optional override for the template path. Defaults
            to ``TRID3NT_CASE_QGS_TEMPLATE`` env > ``DEFAULT_PROJECT_QGS_URI``.

...
override (``TRID3NT_SANDBOX_EXECUTOR``) lets tests / a container point elsewhere.
"""
...
LOG = logging.getLogger("trid3nt.agent.sandbox_runner")
...
# the executor's TRID3NT_SANDBOX_TIMEOUT. The runner's OUTER subprocess timeout is
...
# backstop only.
WALLCLOCK_CAP_SECONDS = int(os.environ.get("TRID3NT_SANDBOX_TIMEOUT", "60"))
...
SUBPROCESS_GRACE_SECONDS = int(os.environ.get("TRID3NT_SANDBOX_SUBPROC_GRACE", "10"))
...
MAX_ENVELOPE_BYTES = int(os.environ.get("TRID3NT_SANDBOX_MAX_ENVELOPE_BYTES", str(8 * 1024 * 1024)))
...
def _executor_path() -> Path:
    override = os.environ.get("TRID3NT_SANDBOX_EXECUTOR", "").strip()
    if override:
...
def run_sandbox_local(
    ...
    *,
    ...
    Enforces:
      - the executor's in-process 60s SIGALRM cap (via TRID3NT_SANDBOX_TIMEOUT), AND
    ...
    workdir = tempfile.mkdtemp(prefix="trid3nt_sandbox_")
    ...
    if hardened:
        ...
        # TRID3NT_SANDBOX_BWRAP=1 demanded it.
        ...
        child_env["TRID3NT_SANDBOX_TIMEOUT"] = str(cap)
...
MAX_ENVELOPE_FIELD_CHARS = int(
    os.environ.get("TRID3NT_SANDBOX_MAX_ENVELOPE_FIELD_CHARS", str(256 * 1024))
)
...
def _parse_envelope(stdout: str, stderr: str, returncode: int | None) -> dict[str, Any]:
    ...
    for line in reversed(stdout.splitlines()):
        line = line.strip()
        # Tolerate the ``TRID3NT_SANDBOX_ENVELOPE_V1 {...}`` marker prefix: take

...
job-def via the per-solver ``TRID3NT_AWS_BATCH_JOB_DEF_OPENQUAKE`` env knob.
...
logger = logging.getLogger("trid3nt_server.workflows.openquake.model_seismic_hazard_scenario.model_seismic_hazard_scenario")
...
async def _emit_oq_curve_charts(
    ...
    *,
    ...
    from trid3nt_server.tools.processing.charts_common import build_hazard_curve_chart, build_uhs_chart
...
async def model_seismic_hazard_scenario(
    ...
    *,
    ...
    """
    from trid3nt_server.tools.simulation.solver.solver import run_solver, wait_for_completion
...
_TRID3NT_REPO_ROOT_OQ = str(Path(__file__).resolve().parents[6])
...
def openquake_local_spec() -> Any:
    ...
    """
    from trid3nt_server.tools.simulation.solver.solver import LOCAL_EXEC_WORKFLOW_NAME, LocalSolverSpec

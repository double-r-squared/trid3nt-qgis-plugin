...
from trid3nt_server.pipeline_emitter import begin_substeps, current_emitter, substep
...
async def model_river_seepage_scenario(
    ...
    *,
    ...
    from trid3nt_server.tools.simulation.run_river_seepage_tool.run_river_seepage_tool import (
...
TEMPLATE_CARD = TemplateCard(
    question=(
        "groundwater <-> river seepage exchange: is a reach gaining or losing, "
        "how much leaks between the aquifer and the river"
    ),
    required_inputs=["location (or spill_location_latlon)", "contaminant"],
    knobs=(
        "release_rate_kg_s, duration_days, along_river_source, "
        "fetch_dem_for_streambed, aquifer_k_ms, porosity"
    ),
)


_RUN_RIVER_SEEPAGE_SCENARIO_METADATA = AtomicToolMetadata(
    name="modflow_river_seepage",
...
async def modflow_river_seepage(
    location: str | None = None,
    ...
    **_extra_ignored: Any,
    ...
    if spill_location_latlon is not None:
        try:
            from trid3nt_server.tool_arg_normalizer import coerce_latlon

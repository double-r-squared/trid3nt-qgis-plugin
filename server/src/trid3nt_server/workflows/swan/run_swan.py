...
downloads the SWAN output (``swan_out.mat``) and postprocesses it.
  ...
  2. **SWAN solver registration** (``register_swan_solver``). Adds ``'swan'`` to
     ...
     per-solver Batch job-def resolves from ``TRID3NT_AWS_BATCH_JOB_DEF_SWAN`` and
...
logger = logging.getLogger("trid3nt_server.workflows.swan.run_swan")
...
def stage_swan_manifest(
    ...
    *,
    ...
    ``TRID3NT_CACHE_BUCKET`` staging bucket the SFINCS/GeoClaw decks upload to.
...
def register_swan_solver() -> None:
    """Register ``'swan'`` in ``tools.simulation.solver.SOLVER_WORKFLOW_REGISTRY``.
...
#: Default SWAN image under local-docker (env TRID3NT_SWAN_IMAGE).

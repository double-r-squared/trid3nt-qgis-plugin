...
``run_pelicun_with_buildings``). Tagged ``engine="pelicun", tier="template"``:
EXCLUDED from the default retrieval pool and surfaced only by the ``run_pelicun``
door's gate expansion (SELECT-THEN-CALL). Metadata
(``source_class="workflow_dispatch"``, ``cacheable=False``,
``ttl_class="live-no-cache"``) is unchanged from the rename.

**Pattern this encapsulates**

The canonical ``pelicun_damage_assessment`` atomic tool accepts an
...
This composer enforces the correct pattern:
    ...
    2. ``pelicun_damage_assessment(hazard_raster_uri, assets_uri=buildings_uri, ...)``
...
Registered as ``pelicun_damage_with_buildings`` via the standard workflow-dispatch
...
logger = logging.getLogger("trid3nt_server.workflows.pelicun.damage_with_buildings.damage_with_buildings")
...
# ---------------------------------------------------------------------------
# S3 -> local staging (established download-then-open pattern).
# ---------------------------------------------------------------------------
...
def _stage_cog_to_local(cog_uri: str) -> tuple[str, bool]:
    ...
    ``AWS_ENDPOINT_URL`` endpoint is never applied, so the read hits real AWS
    or fails) AND risks the orphaned-``MemoryFile`` read-corruption class (a
    GC'd MemoryFile => truncated s3 read). The established pattern (see
    ``damage_assessment._download_uri_to_local`` /
    ``postprocess_pelicun._download_uri_to_local``) is download-then-open:
    read the object fully via the shared boto3 reader (which honours
    ``AWS_ENDPOINT_URL`` for MinIO) into an on-disk temp file, then open the
    disk path.

    Returns ``(local_path, is_temp)``. When ``is_temp`` is True the caller
    MUST unlink ``local_path`` after reading it.
    """
...
# values are building counts per cell.  ``pelicun_damage_assessment``
...
def density_cog_to_point_fgb(cog_uri: str) -> str:
    ...
    download-then-open pattern, never a raw ``rasterio.open("s3://...")``).
    For each cell with
    ...
    # Download-then-open: stage an s3:// COG to a local temp file via the
    # shared boto3/MinIO reader, then open the disk path. Reading the full band
    # inside the ``with`` block (then unlinking) avoids the orphaned-MemoryFile
...
async def _pelicun_damage_with_buildings_chain(
    ...
    Args:
        hazard_raster_uri: single-band flood-depth COG in metres. This MUST be the
            ...
            ``sfincs_flood`` / ``run_model_nws_flood_event_scenario`` call
            ...
            been modeled yet, run ``sfincs_flood`` FIRST and pass its
            result here.
    ...
    Returns:
        ...
        shape returned by ``pelicun_damage_assessment`` directly, with
    ...
    #
    # ``pelicun_damage_assessment`` consumes a vector FlatGeobuf of asset
    ...
    # ------------------------------------------------------------------
    logger.info(
        "pelicun_damage_with_buildings: step 2 — pelicun_damage_assessment "
        "hazard=%s assets=%s",
        hazard_raster_uri,
        assets_fgb_path,
    )
...
_METADATA = AtomicToolMetadata(
    name="pelicun_damage_with_buildings",
...
#: Curated door-listing card (the run_pelicun door prefers this over signature
#: derivation). One-line question + the real required inputs + a knobs summary.
TEMPLATE_CARD = TemplateCard(
    question=(
        "spatially-distributed per-cell structural damage + repair-cost losses "
        "over a hazard raster, AUTO-FETCHING a building-density grid for the "
        "bbox (no explicit asset inventory needed; Pelicun HAZUS v6.1 flood)"
    ),
    required_inputs=["hazard_raster_uri", "bbox"],
    knobs="cell_size_m, fragility_set, realization_count",
)
...
async def pelicun_damage_with_buildings(
    ...
    **_extra_ignored: Any,
    ...
    When to use:
        ...
        - A prior ``sfincs_flood`` has produced a flood depth COG
    ...
    When NOT to use:
        ...
        - Cases with a custom asset layer already in hand (pass it directly to
          ``pelicun_damage_assessment(assets_uri=...)`` instead).
        - CONUS bboxes where ``fetch_usace_nsi`` is available — NSI carries
          ...
          ``pelicun_damage_assessment(assets_uri=<NSI URI>)`` for CONUS.
    ...
    Returns:
        ...
        the same damage properties as ``pelicun_damage_assessment``:
    ...
    LLM guidance:
        - Pair with ``sfincs_flood`` output: pass its flood depth
    ...
    Cross-tool dependencies:
        ...
        Step 3 — ``pelicun_damage_assessment(hazard_raster_uri,
    ...
    """
    return await _pelicun_damage_with_buildings_chain(
        hazard_raster_uri=hazard_raster_uri,
        bbox=bbox,
        cell_size_m=cell_size_m,
        fragility_set=fragility_set,

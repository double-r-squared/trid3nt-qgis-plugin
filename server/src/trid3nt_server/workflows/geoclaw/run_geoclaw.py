...
logger = logging.getLogger("trid3nt_server.workflows.geoclaw.run_geoclaw")
...
def _dem_uri_to_local(dem_uri: str) -> tuple[str, bool]:
    ...
    if dem_uri.startswith("s3://"):
        from trid3nt_server.tools.simulation.solver.solver import _get_s3_client, _split_object_uri

        _scheme, bucket, key = _split_object_uri(dem_uri)
        s3 = _get_s3_client()
        fd, path = _tf.mkstemp(suffix=".tif", prefix="trid3nt_geoclaw_bathy_")
        os.close(fd)
    ...
    try:
        ...
        with rasterio.open(src_local) as src:
            ...
            fd, out_path = _tf.mkstemp(
                suffix=".tif", prefix="trid3nt_geoclaw_topo4326_"
            )
            os.close(fd)
...
#: Default GeoClaw image under local-docker (env TRID3NT_GEOCLAW_IMAGE).

...
from trid3nt_server.layer_uri_emit import emit_layer_uri, publish_input_layer
...
from trid3nt_server.tools.fetchers.hydrology.fetch_river_geometry.fetch_river_geometry import fetch_river_geometry
...
from trid3nt_server.tools.fetchers.ocean.fetch_topobathy.fetch_topobathy import TopobathyError, fetch_topobathy
...
from trid3nt_server.workflows.shared.mesh_layer import make_sfincs_mesh_layer_uri
from trid3nt_server.workflows.sfincs.postprocess_flood import (
    FLOOD_DEPTH_STYLE_PRESET,
    PostprocessError,
    postprocess_flood,
)
from trid3nt_server.workflows.sfincs.postprocess_waves import (
...
logger = logging.getLogger("trid3nt_server.workflows.sfincs.model_flood_scenario.model_flood_scenario")
...
def _estimate_frame_count(
    *, output_interval_min: float | None, duration_hr: float
    ...
    """
    from trid3nt_server.workflows.sfincs.postprocess_flood import MAX_FLOOD_FRAMES
...
# bnd FGB the parametric/CO-OPS surge wrote under /tmp/trid3nt-sfincs-forcing/)
...
def _upload_local_forcing_files_to_s3(
    ...
    *,
    ...
    (``/tmp/trid3nt-sfincs-forcing/bzs-*.csv``, ``bnd-*.fgb``). The SIMPLE-SFINCS
    ...
    from trid3nt_server.tools.simulation.solver.solver import SolverDispatchError as _DeckBuildError
    ...
    for member_name, member in surge_forcing.items():
        ...
        for key in _FORCING_FILE_URI_KEYS:
            ...
            try:
                if s3 is None:
                    from trid3nt_server.tools.simulation.solver.solver import _get_s3_client
...
_WAVE_HS_M_AT_100YR = float(os.getenv("TRID3NT_WAVE_HS_M_AT_100YR", "4.0"))
_WAVE_HS_M_FLOOR = float(os.getenv("TRID3NT_WAVE_HS_M_FLOOR", "0.5"))
_WAVE_HS_M_CEIL = float(os.getenv("TRID3NT_WAVE_HS_M_CEIL", "9.0"))
...
_WAVE_BND_MIN_DEPTH_M = float(os.getenv("TRID3NT_WAVE_BND_MIN_DEPTH_M", "5.0"))
...
_WAVE_BND_TARGET_DEPTH_M = float(os.getenv("TRID3NT_WAVE_BND_TARGET_DEPTH_M", "10.0"))
...
_WAVE_BND_SEAWARD_STEP_FRAC = float(
    os.getenv("TRID3NT_WAVE_BND_SEAWARD_STEP_FRAC", "0.04")
)
_WAVE_BND_SEAWARD_MAX_STEPS = int(os.getenv("TRID3NT_WAVE_BND_SEAWARD_MAX_STEPS", "10"))
...
def _compose_and_upload_deckbuild_spec(
    *,
    ...
    # (/tmp/trid3nt-sfincs-forcing/...). Upload any LOCAL forcing file URIs under
    ...
    if not build_spec_uri.startswith("s3://"):
        raise _DeckBuildError(
            ...
            "(TRID3NT_STORAGE_BACKEND=s3). Composed build_spec URI was "
...
def _sfincs_build_offload_enabled() -> bool:
    ...
    Gated OFF by default (``TRID3NT_SFINCS_BUILD_OFFLOAD`` unset) so live behavior
    ...
    return (os.environ.get("TRID3NT_SFINCS_BUILD_OFFLOAD") or "").strip().lower() in {
        "1",
        "on",
        "true",
        "yes",
    ...
    *,
    ...
    if scheme != "s3":
        raise _DeckBuildError(
            ...
            "worker can read the job_spec (TRID3NT_STORAGE_BACKEND=s3). Staying inert."
        )
    cache_bucket = os.environ.get("TRID3NT_CACHE_BUCKET") or CACHE_BUCKET
    spec_id = new_ulid()
...
def _resolve_surge_forcing_from_fetchers(
    ...
    *,
    ...
    if not surge_forcing:
        return surge_forcing
    from trid3nt_server.workflows.sfincs.sfincs_forcing_adapter import (
...
def _synthesize_parametric_surge_forcing(
    ...
    *,
    ...
    from trid3nt_server.workflows.sfincs.sfincs_forcing_adapter import _staging_dir, _unique  # local: lean top
...
def _autowire_coastal_surge_forcing(
    ...
    *,
    ...
    try:
        from trid3nt_server.tools.fetchers.ocean.fetch_noaa_coops_tides.fetch_noaa_coops_tides import fetch_noaa_coops_tides
    ...
    try:
        from trid3nt_server.tools.fetchers.ocean.fetch_gtsm_tide_surge.fetch_gtsm_tide_surge import fetch_gtsm_tide_surge
...
def _resolve_spiderweb_forcing(
    ...
    *,
    ...
    from trid3nt_server.tools.fetchers.weather.fetch_storm_tracks.fetch_storm_tracks import fetch_storm_tracks
    from trid3nt_server.workflows.sfincs.sfincs_builder import _stage_gcs_local
    from trid3nt_server.workflows.sfincs import sfincs_spiderweb as _spw
...
def _resolve_building_obstacle_uri(
    ...
    try:
        from trid3nt_server.tools.fetchers.socioeconomic.fetch_buildings.fetch_buildings import fetch_buildings  # local: keep top imports lean
...
def _autowire_river_discharge_forcing(
    ...
    *,
    ...
    try:
        from trid3nt_server.tools.fetchers.hydrology.fetch_noaa_nwm_streamflow.fetch_noaa_nwm_streamflow import fetch_noaa_nwm_streamflow
    ...
    try:
        ...
        from trid3nt_server.tools.fetchers.hydrology.fetch_usgs_nwis_gauges.fetch_usgs_nwis_gauges import fetch_usgs_nwis_gauges
...
def _resolve_infiltration_uri(
    ...
    try:
        from trid3nt_server.tools.fetchers.soil.fetch_gcn250_curve_numbers.fetch_gcn250_curve_numbers import fetch_gcn250_curve_numbers
...
async def model_flood_scenario(
    ...
    *,
    ...
    if quadtree_run_result is not None:
        ...
        from trid3nt_server.tools.simulation.solver.solver import EmitterBinding, set_emitter_binding
        ...
        # byte-identical.
        from trid3nt_server.tools.simulation.solver.solver import solve_progress_vcpus

...
logger = logging.getLogger("trid3nt_server.workflows.run_geoclaw")
...
def geoclaw_local_spec() -> "Any":
    ...
    def build_argv(run_id: str, rundir: Path, args: list[str]) -> list[str]:
        ...
        for k, v in env_pairs:
            ...
            # already-built geoclaw worker images still read the old GRACE2_*
            # names at runtime. Emit the legacy twin with the same value so
            # those images keep working. REMOVE once the geoclaw image is
            # rebuilt from this tree (services/workers/geoclaw reads TRID3NT_*).

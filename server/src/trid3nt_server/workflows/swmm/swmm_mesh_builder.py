...
from trid3nt_server.workflows.swmm.swmm_hyetograph import HyetographResult, build_nested_hyetograph
...
from trid3nt_server.workflows.sfincs.sfincs_builder import load_manning_mapping  # noqa: E402
...
def _env_resolution_ladder(default: tuple[float, ...]) -> tuple[float, ...]:
    raw = os.environ.get("TRID3NT_SWMM_RES_LADDER")
    if raw is None or not raw.strip():
        return default
    try:
        vals = sorted(
        ...
        logger.warning(
            "swmm autoscale: env TRID3NT_SWMM_RES_LADDER=%r unparseable; using %s", raw, default
        )
        return default
...
#: than wedging the always-on box. Env: ``TRID3NT_SWMM_SOLVE_BUDGET_S``.
SWMM_SOLVE_BUDGET_S: float = _env_float("TRID3NT_SWMM_SOLVE_BUDGET_S", 300.0)

#: Fraction of the budget reserved for non-solve overhead (DEM fetch, deck
#: write, postprocess). Env: ``TRID3NT_SWMM_OVERHEAD_FRACTION``.
SWMM_OVERHEAD_FRACTION: float = _env_float("TRID3NT_SWMM_OVERHEAD_FRACTION", 0.35)
...
#: Env: ``TRID3NT_SWMM_PERF_P`` / ``TRID3NT_SWMM_PERF_A``.
SWMM_PERF_P: float = _env_float("TRID3NT_SWMM_PERF_P", 1.10)
SWMM_PERF_A: float = _env_float("TRID3NT_SWMM_PERF_A", 0.40687)
...
#: Env: ``TRID3NT_SWMM_RES_LADDER`` (comma-separated).
...
SWMM_MIN_CELL_CAP: int = int(_env_float("TRID3NT_SWMM_MIN_CELL_CAP", 200))

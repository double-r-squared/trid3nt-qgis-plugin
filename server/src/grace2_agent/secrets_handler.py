...
- Secrets live one-per-file under the file-persistence data root
  (``GRACE2_DEV_PERSISTENCE_DIR``, default ``~/.grace2/dev_persistence``)
...
def _vault_root() -> Path:
    ...
    Mirrors the file-persistence selection (``GRACE2_DEV_PERSISTENCE_DIR``,
    default ``~/.grace2/dev_persistence``) via the same helper

...
the mesh never fights the AOI camera):
  ...
  * Regular-grid OUTLINE - the NEW preview for the ``regular_grid`` paradigm
    ...
    :mod:`trid3nt_server.agent.mesh.grid_geometry`
...
logger = logging.getLogger("trid3nt_server.agent.mesh.mesh_preview")
...
def swmm_mesh_to_geojson(build: Any, *, cap: int = 6000) -> tuple[dict, dict]:
    ...
    """
    from trid3nt_server.agent.mesh.raster_cell_mesh import _active_cells_from_deck
...
def regular_grid_outline_feature_collection(
    ...
    *,
    ...
    """
    from trid3nt_server.agent.mesh.grid_geometry import regular_grid_from_bbox
